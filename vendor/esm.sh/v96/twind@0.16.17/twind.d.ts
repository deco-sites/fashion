import {
  ...
  SimplePseudos,
} from "https://esm.sh/v96/csstype@3.1.1/index.d.ts";
...
declare type MaybeObjInterpolationGeneric<T> = T[] | [
  TemplateStringsArray,
  ...T[],
];
declare type MaybeTokenInterpolation = MaybeObjInterpolationGeneric<Token>;
...
interface Preflight {
  (
    preflight: CSSRules,
    context: Context,
  ): MaybeThunk<CSSRules | undefined | void>;
}
...
interface DirectiveHandler {
  ...
  (
    ...
    context: Context,
    id: string,
  ): InlineDirective | CSSRules | string | Falsy;
}
interface Rule {
  /**
...
declare type CSSSimplePseudos = {
  [K in SimplePseudos as `&${string & K}`]?:
    | CSSRulesThunk
    | MaybeArray<CSSRules>;
};
...
declare type CSSAtKeyframes = Record<
  string,
  CSSProperties | ((context: Context) => CSSProperties)
>;
/**
...
declare type CSSRuleValue =
  | CSSAtMedia
  | CSSAtSupports
  | CSSAtKeyframes
  | CSSRulesThunk
  | MaybeArray<
    CSSProperties | CSSRules | FontFace | string | Falsy | TypescriptCompat
  >;
...
interface ThemeHelper {
  <Section extends keyof Theme>(
    section: Section,
  ): (context: Context) => Record<string, ThemeSectionType<Theme[Section]>>;
  <Section extends keyof Theme>(
    keypath: `${Section}.${string}`,
  ): (context: Context) => ThemeSectionType<Theme[Section]> | undefined;
  <Section extends keyof Theme>(
    keypath: `${Section}.${string}`,
    defaultValue: NonNullable<ThemeSectionType<Theme[Section]>>,
...
declare type Unwrap<T> = T extends string[] ? string
  : T extends Record<string, infer R> ? R
  : T;
declare type ThemeSectionType<T> = T extends ThemeSection<infer R> ? Unwrap<R>
...
declare type ThemeSectionResolver<T = string> = (
  theme: ThemeResolver,
  context: ThemeSectionResolverContext,
) => ThemeSectionRecord<T>;
declare type ThemeSection<T = string> =
  | ThemeSectionRecord<T>
  | ThemeSectionResolver<T>;
...
declare type ThemeFontSize = string | [size: string, lineHeight: string] | [
  size: string,
  options: {
    lineHeight?: string;
...
declare type FromTheme<Section extends keyof Theme> = Interpolate<
  `theme(${Section})`
>;
...
declare type SimplePseudoClasses =
  | "active"
  | "any-link"
  | "autofill"
  | "checked"
  ...
  | "valid"
  | "visited";
declare type SimplePseudoElements =
  | "after"
  | "before"
  | "cue"
  | "cue-region"
...
> = Suffix extends Empty ? Prefix extends Empty ? never : Prefix
  : Prefix extends Empty ? Suffix
  : Prefix extends Suffix ? never
  : Suffix extends `-${infer S}` ? `-${Prefix}${Separator}${S}`
...
interface CorePlugins {
  ...
  overflow:
    | Join<"" | "x" | "y", "auto" | "hidden" | "visible" | "scroll">
    | "ellipsis"
    | "clip";
  ...
  row:
    ...
    | Join<
      "start",
      "auto" | Range<1, 13> | Interpolate<"nonzero"> | FromTheme<"gridRowStart">
    >
    | Join<
      "end",
      "auto" | Range<1, 13> | Interpolate<"nonzero"> | FromTheme<"gridRowEnd">
    >
    | FromTheme<"gridRow">;
  auto:
  ...
  space:
    | Negatable<Join<"x" | "y", FromTheme<"space">>>
    | Join<"x" | "y", "reverse">;
  w: FromTheme<"width">;
  ...
  placeholder:
    | FromTheme<"placeholderColor">
    | Join<"opacity", FromTheme<"placeholderOpacity">>;
  underline: "";
  "no-underline": "";
  "line-through": "";
  uppercase: "";
  ...
  ring:
    ...
    | Join<
      "offset",
      FromTheme<"ringOffsetWidth"> | FromTheme<"ringOffsetColor">
    >;
...
declare type ToString<T> = T extends string ? T
  : T extends number ? `${T}`
  : never;
...
declare type CoreCompletionTokens =
  | `${FromTheme<"screens">}:`
 ...
 * @param data to use
 */
declare const directive: <Data, T>(
 ...
 */
declare const cssomSheet: (
  { nonce, target }?: SheetConfig<CSSStyleSheet>,
) => Sheet<CSSStyleSheet>;
/**
