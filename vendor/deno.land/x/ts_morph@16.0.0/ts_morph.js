import {
  ...
  EventContainer,
  FileUtils,
  getCompilerOptionsFromTsConfig as getCompilerOptionsFromTsConfig$1,
...
class AdvancedIterator {
  ...
  get previous() {
    ...
    return this
      .buffer[(this.bufferIndex + this.buffer.length - 1) % this.buffer.length];
  }
  get peek() {
    if (this.isDone) {
      throw new errors.InvalidOperationError(
...
function getNotFoundErrorMessageForNameOrFindFunction(
  findName,
  nameOrFindFunction,
) {
...
function getParentSyntaxList(node, sourceFile) {
  ...
  for (const child of parent.getChildren(sourceFile)) {
    ...
    if (
      child.kind === SyntaxKind.SyntaxList && child.pos <= pos &&
      child.end >= end
    ) {
      return child;
    }
...
function checkNodeIsAmbient(node) {
  const isThisAmbient =
    (node.getCombinedModifierFlags() & ts.ModifierFlags.Ambient) ===
      ts.ModifierFlags.Ambient;
  return isThisAmbient || Node.isInterfaceDeclaration(node) ||
...
function printNode(node, sourceFileOrOptions, secondOverloadOptions) {
  var _a, _b;
  const isFirstOverload = sourceFileOrOptions == null ||
    sourceFileOrOptions.kind !== SyntaxKind.SourceFile;
  const options = getOptions();
  const sourceFile = getSourceFile();
  const printer = ts.createPrinter({
    newLine: (_a = options.newLineKind) !== null && _a !== void 0
      ? _a
  ...
  if (sourceFile == null) {
    ...
    return printer.printNode(
      (_b = options.emitHint) !== null && _b !== void 0
        ? _b
  ...
  function getSourceFile() {
    ...
    function getScriptKind() {
      var _a;
      return (_a = options.scriptKind) !== null && _a !== void 0
        ? _a
        : ScriptKind.TSX;
    }
    ...
    if (this._formatCodeSettings == null) {
      this._formatCodeSettings = {
        ...
        insertSpaceAfterOpeningAndBeforeClosingNonemptyBraces:
          this._settings.insertSpaceAfterOpeningAndBeforeClosingNonemptyBraces,
      };
    }
  ...
  getUserPreferences() {
    if (this._userPreferences == null) {
      this._userPreferences = {
        quotePreference: this.getQuoteKind() === QuoteKind.Double
          ? "double"
          : "single",
        providePrefixAndSuffixTextForRename: this
...
function fillDefaultEditorSettings(settings, manipulationSettings) {
  setValueIfUndefined(
    settings,
    "convertTabsToSpaces",
    manipulationSettings.getIndentationText() !== IndentationText.Tab,
  );
  setValueIfUndefined(
    settings,
    "newLineCharacter",
    manipulationSettings.getNewLineKindAsString(),
  );
  setValueIfUndefined(settings, "indentStyle", ts.IndentStyle.Smart);
  setValueIfUndefined(
    settings,
    "indentSize",
    manipulationSettings.getIndentationText().length,
  );
  setValueIfUndefined(
    settings,
    "tabSize",
  ...
  switch (kind) {
    ...
    default:
      throw new errors.NotImplementedError(
        `Not implemented newline kind: ${kind}`,
      );
  }
}
...
class SourceFileReferenceContainer {
  constructor(sourceFile) {
    ...
    this.resolveUnresolved = () => {
      ...
      if (this.unresolvedLiterals.length === 0) {
        this.sourceFile._context.compilerFactory.onSourceFileAdded(
          this.resolveUnresolved,
          false,
        );
      }
  ...
  getLiteralsReferencingOtherSourceFilesEntries() {
    this.sourceFile._context.lazyReferenceCoordinator.refreshSourceFileIfDirty(
      this.sourceFile,
    );
  ...
  populateReferences() {
    this.sourceFile._context.compilerFactory.forgetNodesCreatedInBlock(
      (remember) => {
  ...
  getSourceFileForLiteral(literal) {
    ...
    if (Node.isImportDeclaration(parent) || Node.isExportDeclaration(parent)) {
      ...
      grandParent != null && Node.isImportEqualsDeclaration(grandParent)
    ) {
      ...
      this.sourceFile._context.logger.warn(
        `Unknown import string literal parent: ${parent.getKindName()}`,
      );
    }
    return undefined;
  }
  ...
  return class extends Base {
    ...
    getDeclareKeywordOrThrow() {
      return errors.throwIfNullOrUndefined(
        ...
        "Expected to find a declare keyword.",
      );
    }
    getDeclareKeyword() {
...
function getFormattingKindText(formattingKind, opts) {
  switch (formattingKind) {
    ...
    default:
      throw new errors.NotImplementedError(
        `Not implemented formatting kind: ${formattingKind}`,
      );
  }
}
...
function hasBody(node) {
  ...
  return Node.isInterfaceDeclaration(node) || Node.isClassDeclaration(node) ||
...
function getTextFromTextChanges(sourceFile, textChanges) {
  ...
  function getSortedTextChanges() {
    return textChanges.map((edit, index) => ({
      edit: toWrappedTextChange(edit),
      index,
    })).sort((a, b) => {
...
function getNewInsertCode(opts) {
  ...
  const {
    ...
    parent,
    getSeparator,
    previousFormattingKind,
    nextFormattingKind,
  } = opts;
  const indentationText = (_a = opts.indentationText) !== null && _a !== void 0
    ? _a
    : parent.getChildIndentationText();
  const newLineKind = parent._context.manipulationSettings
    .getNewLineKindAsString();
  return getFormattingKindTextWithIndent(previousFormattingKind) +
    getChildCode() + getFormattingKindTextWithIndent(nextFormattingKind);
  ...
  function getFormattingKindTextWithIndent(formattingKind) {
    ...
    if (
      formattingKind === FormattingKind.Newline ||
      formattingKind === FormattingKind.Blankline
    ) {
...
function getEndIndexFromArray(array) {
  var _a;
  return (_a = array === null || array === void 0 ? void 0 : array.length) !==
        null && _a !== void 0
    ? _a
    : 0;
}
...
function getInsertPosFromIndex(index, syntaxList, children) {
  if (index === 0) {
    ...
    if (Node.isSourceFile(parent)) {
      ...
      const colonToken = parent.getFirstChildByKindOrThrow(
    ...
    const openBraceToken = parentContainer.getFirstChildByKindOrThrow(
...
function getEndPosFromIndex(index, parent, children, fullText) {
  ...
  if (index === children.length) {
    if (Node.isSourceFile(parent)) {
      ...
      const parentContainer = getParentContainerOrThrow(parent);
      const closeBraceToken = parentContainer.getLastChildByKind(
...
function getNodesToReturn(oldChildren, newChildren, index, allowCommentNodes) {
  const oldChildCount = typeof oldChildren === "number"
    ? oldChildren
    : oldChildren.length;
...
function getRangeWithoutCommentsFromArray(array, index, length, expectedKind) {
  ...
  while (index < array.length && children.length < length) {
    ...
    if (
      ...
      if (childKind !== expectedKind) {
        throw new errors.NotImplementedError(
          `Unexpected! Inserting syntax kind of ${
            getSyntaxKindName(expectedKind)
          }` +
  ...
  if (children.length !== length) {
    throw new errors.NotImplementedError(
      `Unexpected! Inserted ${length} child/children, but ${children.length} were inserted.`,
    );
  }
  return children;
}
...
(function (CommentNodeKind) {
  ...
  CommentNodeKind[CommentNodeKind["ObjectLiteralElement"] = 3] =
...
class CommentNodeParser {
  ...
  static getContainerBodyPos(container, sourceFile) {
    ...
    function getTokenEnd(node, kind) {
      var _a;
      return (_a = node.getChildren(sourceFile).find((c) =>
              c.kind === kind
            )) === null || _a === void 0
        ? void 0
        : _a.end;
...
function* getNodes(container, sourceFile) {
  ...
  if (childNodes.length === 0) {
    const bodyStartPos = CommentNodeParser.getContainerBodyPos(
      container,
      sourceFile,
    );
  ...
  function* getCommentNodes(pos, stopAtJsDoc) {
    ...
    const maxEnd = sourceFileText.length === pos || sourceFileText[pos] === "}"
      ? pos
      : StringUtils.getLineStartFromPos(sourceFileText, pos);
    ...
    function skipTrailingLine() {
      ...
      while (pos < lineEnd) {
        ...
        if (commentKind != null) {
          ...
          !StringUtils.isWhitespace(sourceFileText[pos]) &&
          sourceFileText[pos] !== ","
        ) {
          return;
        } else {
    ...
    function skipSingleLineComment() {
      pos += 2;
      while (
        pos < sourceFileText.length && sourceFileText[pos] !== "\n" &&
  ...
  function getContainerChildren() {
    if (
      ...
      ts.isModuleBlock(container) || ts.isCaseClause(container) ||
  ...
  function getCreationFunction() {
    const ctor = getCtor();
    return (fullStart, pos, end, kind) =>
      new ctor(fullStart, pos, end, kind, sourceFile, container);
    function getCtor() {
      ...
      if (
        ts.isInterfaceDeclaration(container) || ts.isTypeLiteralNode(container)
      ) {
      ...
      throw new errors.NotImplementedError(
        `Not implemented comment node container type: ${
          getSyntaxKindName(container.kind)
        }`,
      );
    }
  }
...
class ExtendedParser {
  ...
  static getCompilerForEachChildren(node, sourceFile) {
    if (CommentNodeParser.shouldParseChildren(node)) {
      ...
      if (result == null) {
        ...
        mergeInComments(
          result,
          CommentNodeParser.getOrParseChildren(node, sourceFile),
        );
  ...
  static getCompilerChildren(node, sourceFile) {
    ...
    function isStatementMemberOrPropertyHoldingSyntaxList() {
      ...
      return CommentNodeParser.getContainerBodyPos(parent, sourceFile) ===
        node.pos;
    }
  }
}
function mergeInComments(nodes, otherNodes) {
  ...
  for (const child of otherNodes) {
    if (
      child.kind !== SyntaxKind.SingleLineCommentTrivia &&
      child.kind !== SyntaxKind.MultiLineCommentTrivia
    ) {
      continue;
    }
...
class NodeHandlerHelper {
  ...
  handleForValues(handler, currentNode, newNode, newSourceFile) {
    if (this.compilerFactory.hasCompilerNode(currentNode)) {
      handler.handleNode(
        this.compilerFactory.getExistingNodeFromCompilerNode(currentNode),
        newNode,
        newSourceFile,
      );
    } else if (currentNode.kind === SyntaxKind.SyntaxList) {
      const sourceFile = this.compilerFactory.getExistingNodeFromCompilerNode(
        currentNode.getSourceFile(),
      );
      handler.handleNode(
        this.compilerFactory.getNodeFromCompilerNode(currentNode, sourceFile),
        newNode,
        newSourceFile,
      );
    }
  ...
  getCompilerChildren(currentNode, newNode, newSourceFile) {
    ...
    return [
      ExtendedParser.getCompilerChildren(
        currentCompilerNode,
        currentSourceFile,
      ),
  ...
  getChildrenFast(currentNode, newNode, newSourceFile) {
    ...
    return [
      ExtendedParser.getCompilerForEachChildren(
        currentCompilerNode,
        currentSourceFile,
      ),
...
class StraightReplacementNodeHandler {
  ...
  handleNode(currentNode, newNode, newSourceFile) {
    if (currentNode.getKind() !== newNode.kind) {
      ...
      if (
        kinds.includes(ts.SyntaxKind.Identifier) &&
        kinds.includes(ts.SyntaxKind.PrivateIdentifier)
      ) {
        currentNode.forget();
        return;
      }
      throw new errors.InvalidOperationError(
        `Error replacing tree! Perhaps a syntax error was inserted ` +
          `(Current: ${currentNode.getKindName()} -- New: ${
            getSyntaxKindName(newNode.kind)
          }).`,
      );
    }
  ...
  handleChildren(currentNode, newNode, newSourceFile) {
    const [currentChildren, newChildren] = this.helper.getChildrenFast(
      currentNode,
      newNode,
      newSourceFile,
    );
...
class ChangeChildOrderParentHandler {
  constructor(compilerFactory, opts) {
    this.compilerFactory = compilerFactory;
    this.straightReplacementNodeHandler = new StraightReplacementNodeHandler(
      compilerFactory,
    );
    this.helper = new NodeHandlerHelper(compilerFactory);
  ...
  handleNode(currentNode, newNode, newSourceFile) {
    const [currentChildren, newChildren] = this.helper.getCompilerChildren(
      currentNode,
      newNode,
      newSourceFile,
    );
    const currentChildrenInNewOrder = this.getChildrenInNewOrder(
      currentChildren,
    );
    errors.throwIfNotEqual(
      newChildren.length,
      currentChildrenInNewOrder.length,
      "New children length should match the old children length.",
    );
    for (let i = 0; i < newChildren.length; i++) {
      this.helper.handleForValues(
        this.straightReplacementNodeHandler,
        currentChildrenInNewOrder[i],
        newChildren[i],
        newSourceFile,
      );
    }
...
class DefaultParentHandler {
  constructor(compilerFactory, opts) {
    ...
    this.replacingNodes = (_a = opts.replacingNodes) === null || _a === void 0
      ? void 0
      : _a.map((n) => n.compilerNode);
    this.customMappings = opts.customMappings;
  }
  handleNode(currentNode, newNode, newSourceFile) {
    const [currentChildren, newChildren] = this.helper
      .getCompilerChildrenAsIterators(currentNode, newNode, newSourceFile);
    let count = this.childCount;
    this.handleCustomMappings(newNode);
    while (
      !currentChildren.done && !newChildren.done &&
      !this.isFirstChild(currentChildren.peek, newChildren.peek)
    ) {
      this.helper.handleForValues(
        this.straightReplacementNodeHandler,
    ...
    if (!newChildren.done) {
      throw new Error(
        "Error replacing tree: Should not have children left over.",
      );
    }
    this.compilerFactory.replaceCompilerNode(currentNode, newNode);
  }
  handleCustomMappings(newParentNode) {
    ...
    for (const mapping of customMappings) {
      this.compilerFactory.replaceCompilerNode(
        mapping.currentNode,
        mapping.newNode,
      );
    }
...
class ForgetChangedNodeHandler {
  ...
  handleChildren(currentNode, newNode, newSourceFile) {
    const [currentNodeChildren, newNodeChildrenArray] = this.helper
      .getChildrenFast(currentNode, newNode, newSourceFile);
    ...
    for (const currentNodeChild of currentNodeChildren) {
      ...
      if (nextNodeChildResult.done) {
        const existingNode = this.compilerFactory
          .getExistingNodeFromCompilerNode(currentNodeChild);
...
class ParentFinderReplacementNodeHandler
  extends StraightReplacementNodeHandler {
  constructor(compilerFactory, parentNodeHandler, changingParent) {
    ...
    this.changingParentParent = this.changingParent.getParentSyntaxList() ||
      this.changingParent.getParent();
    this.parentsAtSamePos = this.changingParentParent != null &&
      this.changingParentParent.getPos() === this.changingParent.getPos();
  }
  handleNode(currentNode, newNode, newSourceFile) {
  ...
  isParentNode(newNode, newSourceFile) {
    const positionsAndKindsEqual =
      areNodesEqual(newNode, this.changingParent) &&
      areNodesEqual(
        getParentSyntaxList(newNode, newSourceFile) || newNode.parent,
        this.changingParentParent,
      );
    ...
    return getAncestorLength(this.changingParent.compilerNode) ===
      getAncestorLength(newNode);
...
class RangeHandler {
  ...
  handleNode(currentNode, newNode, newSourceFile) {
    const currentSourceFile = currentNode._sourceFile.compilerNode;
    const children = this.helper.getChildrenFast(
      currentNode,
      newNode,
      newSourceFile,
    );
    const currentNodeChildren = new AdvancedIterator(
      ArrayUtils.toIterator(children[0]),
    );
    const newNodeChildren = new AdvancedIterator(
      ArrayUtils.toIterator(children[1]),
    );
    while (
      !currentNodeChildren.done && !newNodeChildren.done &&
      newNodeChildren.peek.getEnd() <= this.start
    ) {
      this.straightReplace(
    ...
    while (
      ...
      (currentNodeChildren.peek.getStart(currentSourceFile) < this.start ||
        currentNodeChildren.peek.getStart(currentSourceFile) === this.start &&
          newNodeChildren.peek.end > this.end)
    ) {
      this.rangeHandlerReplace(
        currentNodeChildren.next(),
        newNodeChildren.next(),
        newSourceFile,
      );
...
class RangeParentHandler {
  ...
  handleNode(currentNode, newNode, newSourceFile) {
    const currentSourceFile = currentNode._sourceFile.compilerNode;
    const [currentNodeChildren, newNodeChildren] = this.helper
      .getCompilerChildrenAsIterators(currentNode, newNode, newSourceFile);
    this.handleCustomMappings(newNode, newSourceFile);
    while (
      !currentNodeChildren.done && !newNodeChildren.done &&
      newNodeChildren.peek.getStart(newSourceFile) < this.start
    ) {
      this.straightReplace(
        currentNodeChildren.next(),
        newNodeChildren.next(),
    ...
    if (this.replacingLength != null) {
      ...
      while (
        ...
        (getRealEnd(currentNodeChildren.peek, currentSourceFile) <=
            replacingEnd ||
      ...
      if (
        ...
        oldNodes.every((node, i) => node.kind === newNodes[i].kind)
      ) {
        for (let i = 0; i < oldNodes.length; i++) {
          const node = this.compilerFactory.getExistingNodeFromCompilerNode(
            oldNodes[i],
          );
          if (node != null) {
  ...
  handleCustomMappings(newParentNode, newSourceFile) {
    ...
    for (const mapping of customMappings) {
      mapping.currentNode._context.compilerFactory.replaceCompilerNode(
        mapping.currentNode,
        mapping.newNode,
      );
    }
...
function getRealEnd(node, sourceFile) {
  if (
    ...
    return getPreviousMatchingPos(
      ...
      node.end,
      (charCode) =>
        charCode !== CharCodes.ASTERISK &&
        !StringUtils.isWhitespaceCharCode(charCode),
    );
  }
  return node.end;
}
...
class RenameNodeHandler extends StraightReplacementNodeHandler {
  handleNode(currentNode, newNode, newSourceFile) {
    ...
    if (
      currentNodeKind === SyntaxKind.ShorthandPropertyAssignment &&
      newNodeKind === SyntaxKind.PropertyAssignment
    ) {
      ...
      currentNodeKind === SyntaxKind.ExportSpecifier &&
      newNodeKind === SyntaxKind.ExportSpecifier &&
      currentNode.compilerNode.propertyName == null &&
      newNode.propertyName != null
    ) {
      handleImportOrExportSpecifier(this.compilerFactory);
      return;
    } else if (
      currentNodeKind === SyntaxKind.ImportSpecifier &&
      newNodeKind === SyntaxKind.ImportSpecifier &&
      currentNode.compilerNode.propertyName == null &&
      newNode.propertyName != null
    ) {
      handleImportOrExportSpecifier(this.compilerFactory);
    ...
    function handleImportOrExportSpecifier(compilerFactory) {
      ...
      const newIdentifier = newPropertyName.escapedText ===
          currentIdentifier.compilerNode.escapedText
        ? newName
        : newPropertyName;
...
class TryOrForgetNodeHandler {
  ...
  handleNode(currentNode, newNode, newSourceFile) {
    ...
    try {
      ...
      currentNode._context.logger.warn(
        "Could not replace tree, so forgetting all nodes instead. Message: " +
          ex,
      );
      currentNode.getChildSyntaxListOrThrow().forget();
      currentNode._context.compilerFactory.replaceCompilerNode(
        currentNode,
        newNode,
      );
    }
...
class UnwrapParentHandler {
  ...
  handleNode(currentNode, newNode, newSourceFile) {
    ...
    let index = 0;
    while (
      !currentChildren.done && !newChildren.done && index++ < this.childIndex
    ) {
      this.helper.handleForValues(
        this.straightReplacementNodeHandler,
        currentChildren.next(),
    ...
    const currentChild = this.compilerFactory.getExistingNodeFromCompilerNode(
      currentChildren.next(),
    );
    const childSyntaxList = currentChild.getChildSyntaxListOrThrow();
    for (
      const child of ExtendedParser.getCompilerChildren(
        childSyntaxList.compilerNode,
        childSyntaxList._sourceFile.compilerNode,
      )
    ) {
      this.helper.handleForValues(
        this.straightReplacementNodeHandler,
...
class NodeHandlerFactory {
  getDefault(opts) {
    const { parent: changingParent, isFirstChild, childCount, customMappings } =
      opts;
    const sourceFile = changingParent.getSourceFile();
    const compilerFactory = sourceFile._context.compilerFactory;
    const replacingNodes = opts.replacingNodes == null
      ? undefined
      : [...opts.replacingNodes];
    const parentHandler = new DefaultParentHandler(compilerFactory, {
      childCount,
      isFirstChild,
      replacingNodes,
      customMappings,
    ...
    if (changingParent === sourceFile) {
      ...
      return new ParentFinderReplacementNodeHandler(
        compilerFactory,
        parentHandler,
        changingParent,
      );
  ...
  getForParentRange(opts) {
    ...
    const sourceFile = changingParent.getSourceFile();
    const compilerFactory = sourceFile._context.compilerFactory;
    const parentHandler = new RangeParentHandler(compilerFactory, {
      start,
      end,
      replacingLength,
      replacingNodes,
  ...
  getForChildIndex(opts) {
    const { parent, childIndex, childCount, replacingNodes, customMappings } =
      opts;
    const parentChildren = parent.getChildren();
    errors.throwIfOutOfRange(
      childIndex,
      [0, parentChildren.length],
      "opts.childIndex",
    );
    if (childCount < 0) {
      errors.throwIfOutOfRange(childCount, [
  ...
  getForChangingChildOrder(opts) {
    ...
    const sourceFile = changingParent.getSourceFile();
    const compilerFactory = sourceFile._context.compilerFactory;
    const changeChildOrderParentHandler = new ChangeChildOrderParentHandler(
      compilerFactory,
      { oldIndex, newIndex },
    );
    if (changingParent === sourceFile) {
  ...
  getForUnwrappingNode(unwrappingNode) {
    const changingParent = unwrappingNode.getParentSyntaxList() ||
      unwrappingNode.getParentOrThrow();
    const childIndex = unwrappingNode.getChildIndex();
    const sourceFile = changingParent.getSourceFile();
    const compilerFactory = sourceFile._context.compilerFactory;
    const unwrapParentHandler = new UnwrapParentHandler(
      compilerFactory,
      childIndex,
    );
    if (changingParent === sourceFile) {
...
function getSpacingBetweenNodes(opts) {
  ...
  const previousSiblingFormatting = getSiblingFormatting(
    parent,
    previousSibling,
  );
  const nextSiblingFormatting = getSiblingFormatting(parent, nextSibling);
  if (
    previousSiblingFormatting === FormattingKind.Blankline ||
    nextSiblingFormatting === FormattingKind.Blankline
  ) {
    return newLineKind + newLineKind;
  } else if (
    previousSiblingFormatting === FormattingKind.Newline ||
    nextSiblingFormatting === FormattingKind.Newline
  ) {
    return newLineKind;
  } else if (
    previousSiblingFormatting === FormattingKind.Space ||
    nextSiblingFormatting === FormattingKind.Space
  ) {
    return " ";
  } else {
    return "";
...
class ChangingChildOrderTextManipulator {
  ...
  getNewText(inputText) {
    ...
    const movingNodeStart = getPosAtNextNonBlankLine(
      fullText,
      movingNode.getPos(),
    );
    const movingNodeText = fullText.substring(
      movingNodeStart,
      movingNode.getEnd(),
    );
    ...
    function fillMiddleText() {
      ...
      if (lowerIndex === oldIndex) {
        startPos = getPosAtNextNonBlankLine(
          fullText,
          children[lowerIndex].getEnd(),
        );
        endPos = children[upperIndex].getEnd();
      } else {
        startPos = getPosAtNextNonBlankLine(
          fullText,
          children[lowerIndex].getPos(),
        );
    ...
    function fillSuffixText() {
      ...
      finalText += fullText.substring(
        getPosAtNextNonBlankLine(fullText, children[upperIndex].getEnd()),
      );
    }
...
function getTextForError(newText, pos, length = 0) {
  ...
  endPos = endPos === -1
    ? newText.length
    : Math.min(newText.length, endPos + 100);
  let text = "";
...
class InsertionTextManipulator {
  ...
  getNewText(inputText) {
    ...
    return inputText.substring(0, insertPos) + newText +
      inputText.substring(insertPos + replacingLength);
  }
  getTextForError(newText) {
    return getTextForError(
      newText,
      this.opts.insertPos,
      this.opts.newText.length,
    );
  }
}
  ...
  getNewText(inputText) {
    ...
    function getRemovalPos() {
      ...
      if (removePrecedingSpaces || removePrecedingNewLines) {
        return getPreviousMatchingPos(
          fullText,
          pos,
          getCharRemovalFunction(
            removePrecedingSpaces,
            removePrecedingNewLines,
          ),
        );
      }
      return pos;
    ...
    function getRemovalEnd() {
      ...
      if (removeFollowingSpaces || removeFollowingNewLines) {
        return getNextMatchingPos(
          fullText,
          end,
          getCharRemovalFunction(
            removeFollowingSpaces,
    ...
    function getCharRemovalFunction(removeSpaces, removeNewLines) {
      return (char) => {
        if (
          removeNewLines &&
          (char === CharCodes.CARRIAGE_RETURN || char === CharCodes.NEWLINE)
        ) {
          return false;
        }
...
class RemoveChildrenWithFormattingTextManipulator {
  ...
  getNewText(inputText) {
    ...
    const fullText = sourceFile.getFullText();
    const newLineKind = sourceFile._context.manipulationSettings
      .getNewLineKindAsString();
    ...
    function getRemovalPos() {
      if (previousSibling != null) {
        ...
        return isNewLineAtPos(fullText, trailingEnd)
          ? trailingEnd
          : previousSibling.getEnd();
      }
      const firstPos = getPreviousNonWhitespacePos(
        fullText,
        firstChild.getPos(),
      );
      ...
      return firstChild.isFirstNodeOnLine()
        ? firstPos
        : firstChild.getNonWhitespaceStart();
    }
    function getRemovalEnd() {
      ...
      if (previousSibling != null && nextSibling != null) {
        ...
        if (
          ...
          return getPosAtStartOfLineOrNonWhitespace(
            fullText,
            nextSibling.getNonWhitespaceStart(),
          );
        }
      ...
      if (isNewLineAtPos(fullText, triviaEnd)) {
        ...
        return getPosAtEndOfPreviousLine(
          fullText,
          getPosAtNextNonBlankLine(fullText, triviaEnd),
        );
      }
      if (previousSibling == null) {
        return triviaEnd;
...
class RenameLocationTextManipulator {
  ...
  getNewText(inputText) {
    const renameLocations = [...this.renameLocations].sort((a, b) =>
      b.getTextSpan().getStart() - a.getTextSpan().getStart()
    );
  ...
  getTextForError(newText) {
    ...
    return "..." +
      newText.substring(this.renameLocations[0].getTextSpan().getStart());
  }
}
...
function getReplacementText(node) {
  ...
  return StringUtils.indent(text, -1, {
    ...
    indentSizeInSpaces: sourceFile._context.manipulationSettings
      ._getIndentSizeInSpaces(),
  ...
  function getInnerBraceRange() {
    ...
    function getBodyNodeOrThrow() {
      if (Node.isModuleDeclaration(node)) {
        ...
        throw new errors.NotImplementedError(
          `Not implemented unwrap scenario for ${node.getKindName()}.`,
        );
      }
    }
  }
...
function doManipulation(sourceFile, textManipulator, nodeHandler, newFilePath) {
  ...
  try {
    const replacementSourceFile = sourceFile._context.compilerFactory
    ...
    if (diagnostics.length > 0) {
      throwError(
        "Manipulation error: " + "A syntax error was inserted." + "\n\n" +
          sourceFile._context.project.formatDiagnosticsWithColorAndContext(
            diagnostics,
            { newLineChar: "\n" },
          ) +
          "\n" + errorDetails,
...
function insertIntoParentTextRange(opts) {
  ...
  doManipulation(
    ...
    new InsertionTextManipulator({
      insertPos,
      newText,
      replacingLength: (_a = opts.replacing) === null || _a === void 0
        ? void 0
        : _a.textLength,
    }),
    new NodeHandlerFactory().getForParentRange({
      ...
      replacingLength: (_b = opts.replacing) === null || _b === void 0
        ? void 0
        : _b.textLength,
      replacingNodes: (_c = opts.replacing) === null || _c === void 0
        ? void 0
        : _c.nodes,
...
function insertIntoCommaSeparatedNodes(opts) {
  ...
  const separator = opts.useNewLines
    ? parent._context.manipulationSettings.getNewLineKindAsString()
    : " ";
  const parentNextSibling = parent.getNextSibling();
  const isContained = parentNextSibling != null &&
    (parentNextSibling.getKind() === SyntaxKind.CloseBraceToken ||
      parentNextSibling.getKind() === SyntaxKind.CloseBracketToken);
  let { newText } = opts;
  if (previousNode != null) {
    ...
    const nextEndStart = nextNode == null
      ? (isContained ? parentNextSibling.getStart(true) : parent.getEnd())
      : nextNode.getStart(true);
    ...
    insertIntoParentTextRange({
      ...
      parent,
      replacing: {
        textLength: parent.getNextSiblingOrThrow().getStart() - parent.getPos(),
      },
    });
  }
  function prependCommaAndSeparator() {
    ...
    let text = "";
    if (
      previousNodeNextSibling != null &&
      previousNodeNextSibling.getKind() === SyntaxKind.CommaToken
    ) {
    ...
    function appendCommentNodeTexts() {
      const lastCommentRangeEnd = getLastCommentRangeEnd(previousNode) ||
        previousNode.getEnd();
      text += originalSourceFileText.substring(
        previousNonCommentNode.getEnd(),
        lastCommentRangeEnd,
      );
    }
    ...
    function getLastCommentRangeEnd(node) {
      ...
      return lastCommentRange === null || lastCommentRange === void 0
        ? void 0
        : lastCommentRange.getEnd();
    }
  }
...
function insertIntoBracesOrSourceFile(opts) {
  ...
  doManipulation(
    parent._sourceFile,
    new InsertionTextManipulator({ insertPos, replacingLength, newText }),
    new NodeHandlerFactory().getForParentRange({
...
function insertIntoBracesOrSourceFileWithGetChildren(opts) {
  ...
  return getRangeWithoutCommentsFromArray(
    opts.getIndexedChildren(),
    opts.index - previousJsDocCount,
    opts.structures.length,
    opts.expectedKind,
  );
...
function removeChildren(opts) {
  ...
  doManipulation(
    ...
    new NodeHandlerFactory().getForChildIndex({
      parent: children[0].getParentSyntaxList() ||
        children[0].getParentOrThrow(),
...
function removeChildrenWithFormatting(opts) {
  ...
  doManipulation(
    children[0]._sourceFile,
    new RemoveChildrenWithFormattingTextManipulator({
      children,
      getSiblingFormatting,
    }),
    new NodeHandlerFactory().getForChildIndex({
...
function removeCommaSeparatedChild(child) {
  ...
  const isRemovingFirstChild =
    childrenToRemove[0] === syntaxList.getFirstChild();
  addNextCommaIfAble();
  addPreviousCommaIfAble();
  removeChildren({
    children: childrenToRemove,
    removePrecedingSpaces: !isRemovingFirstChild ||
      syntaxList.getChildren().length === childrenToRemove.length &&
        childrenToRemove[0].isFirstNodeOnLine(),
  ...
  function addPreviousCommaIfAble() {
    if (
      syntaxList.getLastChild() !==
        childrenToRemove[childrenToRemove.length - 1]
    ) {
      return;
    }
    const precedingComma = child.getPreviousSiblingIfKind(
...
function replaceNodeText(opts) {
  doManipulation(
    ...
    new NodeHandlerFactory().getForForgetChanged(
      opts.sourceFile._context.compilerFactory,
    ),
  );
}
...
function replaceSourceFileTextStraight(opts) {
  ...
  doManipulation(
    ...
    new NodeHandlerFactory().getForStraightReplacement(
      sourceFile._context.compilerFactory,
    ),
  );
}
function replaceSourceFileTextForRename(opts) {
  ...
  doManipulation(
    ...
    nodeHandlerFactory.getForTryOrForget(
      nodeHandlerFactory.getForRename(sourceFile._context.compilerFactory),
    ),
  );
}
...
function ArgumentedNode(Base) {
  return class extends Base {
    getArguments() {
      var _a, _b;
      return (_b = (_a = this.compilerNode.arguments) === null || _a === void 0
              ? void 0
              : _a.map((a) => this._getNodeFromCompilerNode(a))) !== null &&
          _b !== void 0
        ? _b
        : [];
    }
    ...
    insertArguments(index, argumentTexts) {
      ...
      insertIntoCommaSeparatedNodes({
        parent: this.getFirstChildByKindOrThrow(SyntaxKind.OpenParenToken)
          .getNextSiblingIfKindOrThrow(SyntaxKind.SyntaxList),
    ...
    removeArgument(argOrIndex) {
      ...
      const argToRemove = typeof argOrIndex === "number"
        ? getArgFromIndex(argOrIndex)
        : argOrIndex;
  ...
  return class extends Base {
    ...
    getAsyncKeywordOrThrow() {
      return errors.throwIfNullOrUndefined(
        ...
        "Expected to find an async keyword.",
      );
    }
    setIsAsync(value) {
  ...
  return class extends Base {
    ...
    getAwaitKeywordOrThrow() {
      return errors.throwIfNullOrUndefined(
        ...
        "Expected to find an await token.",
      );
    }
    setIsAwaited(value) {
...
function getBodyText(writer, textOrWriterFunction) {
  writer.newLineIfLastNot();
  if (
    typeof textOrWriterFunction !== "string" || textOrWriterFunction.length > 0
  ) {
    writer.indent(() => {
      printTextFromStringOrWriter(writer, textOrWriterFunction);
    });
...
function getBodyTextWithoutLeadingIndentation(body) {
  ...
  const endPos = Math.max(
    startPos,
    textArea._getTrailingTriviaNonWhitespaceEnd(),
  );
  ...
  return StringUtils.removeIndentation(fullText, {
    indentSizeInSpaces: body._context.manipulationSettings
      ._getIndentSizeInSpaces(),
...
class Node {
  ...
  _storeTextForForgetting() {
    const sourceFileCompilerNode = this._sourceFile &&
  ...
  getSymbolOrThrow() {
    return errors.throwIfNullOrUndefined(
      this.getSymbol(),
      "Could not find the node's symbol.",
    );
  }
  getSymbol() {
  ...
  getLocalOrThrow(name) {
    return errors.throwIfNullOrUndefined(
      this.getLocal(name),
      `Expected to find local symbol with name: ${name}`,
    );
  }
  getLocal(name) {
    ...
    return tsSymbol == null
      ? undefined
      : this._context.compilerFactory.getSymbol(tsSymbol);
  }
  getLocals() {
    const locals = this._getCompilerLocals();
    if (locals == null) {
    ...
    return ArrayUtils.from(locals.values()).map((symbol) =>
      this._context.compilerFactory.getSymbol(symbol)
    );
  }
  _getCompilerLocals() {
    this._ensureBound();
    ...
    if (this._childStringRanges == null) {
      ...
      for (const descendant of this._getCompilerDescendantsIterator()) {
        if (isStringKind(descendant.kind)) {
          this._childStringRanges.push([
            descendant.getStart(this._sourceFile.compilerNode),
            descendant.getEnd(),
          ]);
        }
      }
  ...
  asKindOrThrow(kind) {
    return errors.throwIfNullOrUndefined(
      this.asKind(kind),
      () =>
        `Expected the node to be of kind ${
          getSyntaxKindName(kind)
        }, but it was ${getSyntaxKindName(this.getKind())}.`,
    );
  }
  isKind(kind) {
  ...
  getFirstDescendantOrThrow(condition) {
    return errors.throwIfNullOrUndefined(
      ...
      "Could not find a descendant that matched the specified condition.",
    );
  }
  ...
  getPreviousSiblingOrThrow(condition) {
    return errors.throwIfNullOrUndefined(
      this.getPreviousSibling(condition),
      "Could not find the previous sibling.",
    );
  }
  ...
  getNextSiblingOrThrow(condition) {
    return errors.throwIfNullOrUndefined(
      this.getNextSibling(condition),
      "Could not find the next sibling.",
    );
  }
  ...
  getPreviousSiblings() {
    return this._getCompilerPreviousSiblings().map((n) =>
      this._getNodeFromCompilerNode(n)
    );
  }
  getNextSiblings() {
    return this._getCompilerNextSiblings().map((n) =>
      this._getNodeFromCompilerNode(n)
    );
  }
  getChildren() {
    return this._getCompilerChildren().map((n) =>
      this._getNodeFromCompilerNode(n)
    );
  }
  ...
  *_getChildrenInCacheIterator() {
    ...
    for (const child of children) {
      if (this._context.compilerFactory.hasCompilerNode(child)) {
        yield this._context.compilerFactory.getExistingNodeFromCompilerNode(
          child,
        );
  ...
  getChildSyntaxListOrThrow() {
    return errors.throwIfNullOrUndefined(
      ...
      "A child syntax list was expected.",
    );
  }
  getChildSyntaxList() {
    let node = this;
    if (Node.isBodyable(node) || Node.isBodied(node)) {
      do {
        const bodyNode = Node.isBodyable(node)
          ? node.getBody()
          : node.getBody();
        if (bodyNode == null) {
          return undefined;
        ...
        node.compilerNode.statements == null
      );
    }
    if (
  ...
  forEachDescendant(cbNode, cbNodeArray) {
    ...
    function forEachChildForNode(node) {
      const result = node.forEachChild(
        (innerNode) => {
  ...
  *_getDescendantsIterator() {
    ...
    function handleNode(thisNode, node) {
      if (handleStatements(thisNode, node)) {
        ...
        if (arrowFunction.body.kind !== SyntaxKind.Block) {
          statements.push(
            thisNode._getNodeFromCompilerNode(arrowFunction.body),
          );
        } else {
  ...
  getStart(includeJsDocComments) {
    return this.compilerNode.getStart(
      this._sourceFile.compilerNode,
      includeJsDocComments,
    );
  }
  getFullStart() {
  ...
  _getTrailingTriviaNonWhitespaceEnd() {
    return getPreviousNonWhitespacePos(
      this._sourceFile.getFullText(),
      this.getTrailingTriviaEnd(),
    );
  }
  ...
  getTrailingTriviaEnd() {
    ...
    return getNextMatchingPos(
      this._sourceFile.getFullText(),
      searchStart,
      (char) => char !== CharCodes.SPACE && char !== CharCodes.TAB,
    );
    function getSearchStart() {
      return trailingComments.length > 0
        ? trailingComments[trailingComments.length - 1].getEnd()
        : end;
    }
  }
  getText(includeJsDocCommentOrOptions) {
    const options = typeof includeJsDocCommentOrOptions === "object"
      ? includeJsDocCommentOrOptions
      : undefined;
    const includeJsDocComments = includeJsDocCommentOrOptions === true ||
      (options != null && options.includeJsDocComments);
    const trimLeadingIndentation = options != null &&
      options.trimLeadingIndentation;
    const startPos = this.getStart(includeJsDocComments);
    const text = this._sourceFile.getFullText().substring(
      startPos,
      this.getEnd(),
    );
    if (trimLeadingIndentation) {
      return StringUtils.removeIndentation(text, {
        isInStringAtPos: (pos) =>
          this._sourceFile.isInStringAtPos(pos + startPos),
        indentSizeInSpaces: this._context.manipulationSettings
          ._getIndentSizeInSpaces(),
      });
    } else {
      return text;
  ...
  getNodeProperty(propertyName) {
    ...
    if (property == null) {
      ...
      return property.map((p) =>
        isNode(p) ? this._getNodeFromCompilerNode(p) : p
      );
    ...
    function isNode(value) {
      return typeof value.kind === "number" && typeof value.pos === "number" &&
  ...
  *_getAncestorsIterator(includeSyntaxLists) {
    ...
    function getParent(node) {
      return includeSyntaxLists
        ? node.getParentSyntaxList() || node.getParent()
        : node.getParent();
    }
  }
  getParent() {
  ...
  getParentOrThrow() {
    return errors.throwIfNullOrUndefined(
      this.getParent(),
      "Expected to find a parent.",
    );
  }
  getParentWhileOrThrow(condition) {
    return errors.throwIfNullOrUndefined(
      this.getParentWhile(condition),
      "The initial parent did not match the provided condition.",
    );
  }
  ...
  getParentWhileKindOrThrow(kind) {
    return errors.throwIfNullOrUndefined(
      ...
      `The initial parent was not a syntax kind of ${getSyntaxKindName(kind)}.`,
    );
  }
  ...
  getLastToken() {
    const lastToken = this.compilerNode.getLastToken(
      this._sourceFile.compilerNode,
    );
    if (lastToken == null) {
      throw new errors.NotImplementedError(
  ...
  getParentSyntaxListOrThrow() {
    return errors.throwIfNullOrUndefined(
      ...
      "Expected the parent to be a syntax list.",
    );
  }
  getParentSyntaxList() {
    ...
    const syntaxList = getParentSyntaxList(
      this.compilerNode,
      this._sourceFile.compilerNode,
    );
  ...
  getIndentationLevel() {
    const indentationText = this._context.manipulationSettings
      .getIndentationText();
  ...
  getIndentationText(offset = 0) {
    return this._getIndentationTextForLevel(
      this.getIndentationLevel() + offset,
    );
  }
  getChildIndentationText(offset = 0) {
    return this._getIndentationTextForLevel(
      this.getChildIndentationLevel() + offset,
    );
  }
  _getIndentationTextForLevel(level) {
    return this._context.manipulationSettings.getIndentationText().repeat(
      level,
    );
  }
  getStartLinePos(includeJsDocComments) {
    ...
    return getPreviousMatchingPos(
      ...
      (char) =>
        char === CharCodes.NEWLINE || char === CharCodes.CARRIAGE_RETURN,
    );
  }
  getStartLineNumber(includeJsDocComments) {
    return StringUtils.getLineNumberAtPos(
      this._sourceFile.getFullText(),
      this.getStartLinePos(includeJsDocComments),
    );
  }
  getEndLineNumber() {
    const sourceFileText = this._sourceFile.getFullText();
    const endLinePos = getPreviousMatchingPos(
      sourceFileText,
      this.getEnd(),
      (char) =>
        char === CharCodes.NEWLINE || char === CharCodes.CARRIAGE_RETURN,
  ...
  replaceWithText(textOrWriterFunction, writer) {
    const newText = getTextFromStringOrWriter(
      writer || this._getWriterWithQueuedIndentation(),
  ...
  prependWhitespace(textOrWriterFunction) {
    insertWhiteSpaceTextAtPos(
      this,
      this.getStart(true),
      textOrWriterFunction,
      nameof(this, "prependWhitespace"),
    );
  }
  appendWhitespace(textOrWriterFunction) {
    insertWhiteSpaceTextAtPos(
      ...
      this.getEnd(),
      textOrWriterFunction,
      nameof(this, "appendWhitespace"),
    );
  }
  formatText(settings = {}) {
    const formattingEdits = this._context.languageService
      .getFormattingEditsForRange(this._sourceFile.getFilePath(), [
        this.getStart(true),
        this.getEnd(),
      ], settings);
  ...
  transform(visitNode) {
    ...
    if (this.getKind() === ts.SyntaxKind.SourceFile) {
      ts.transform(
        compilerNode,
        [transformerFactory],
        this._context.compilerOptions.get(),
      );
      ...
      const traversal = {
        ...
        visitChildren() {
          node = ts.visitEachChild(
            node,
            (child) => innerVisit(child, context),
            context,
          );
          return node;
        },
    ...
    function handleTransformation(oldNode, newNode) {
      ...
      while (
        (lastTransformation = transformations[transformations.length - 1]) &&
        lastTransformation.start > start
      ) {
        transformations.pop();
      }
      const wrappedNode = compilerFactory.getExistingNodeFromCompilerNode(
        oldNode,
      );
    ...
    function getTransformedText(replaceRange) {
      ...
      for (const transform of transformations) {
        ...
        finalText += printer.printNode(
          ts.EmitHint.Unspecified,
          transform.compilerNode,
          compilerSourceFile,
        );
  ...
  getLeadingCommentRanges() {
    return this._leadingCommentRanges ||
      (this._leadingCommentRanges = this._getCommentsAtPos(
        this.getFullStart(),
        (text, pos) => {
          const comments = ts.getLeadingCommentRanges(text, pos) || [];
          if (
            this.getKind() === SyntaxKind.SingleLineCommentTrivia ||
            this.getKind() === SyntaxKind.MultiLineCommentTrivia
          ) {
  ...
  getTrailingCommentRanges() {
    return this._trailingCommentRanges ||
      (this._trailingCommentRanges = this._getCommentsAtPos(
        this.getEnd(),
  ...
  _getCommentsAtPos(pos, getComments) {
    ...
    return (getComments(this._sourceFile.getFullText(), pos) || []).map((r) =>
      new CommentRange(r, this._sourceFile)
    );
  }
  getChildrenOfKind(kind) {
    return this._getCompilerChildrenOfKind(kind).map((c) =>
      this._getNodeFromCompilerNode(c)
    );
  }
  getFirstChildByKindOrThrow(kind) {
    return errors.throwIfNullOrUndefined(
      this.getFirstChildByKind(kind),
      `A child of the kind ${getSyntaxKindName(kind)} was expected.`,
    );
  }
  ...
  getFirstChildIfKindOrThrow(kind) {
    return errors.throwIfNullOrUndefined(
      ...
      `A first child of the kind ${getSyntaxKindName(kind)} was expected.`,
    );
  }
  getFirstChildIfKind(kind) {
    const firstChild = this._getCompilerFirstChild();
    return firstChild != null && firstChild.kind === kind
      ? this._getNodeFromCompilerNode(firstChild)
      : undefined;
  }
  ...
  getLastChildIfKindOrThrow(kind) {
    return errors.throwIfNullOrUndefined(
      ...
      `A last child of the kind ${getSyntaxKindName(kind)} was expected.`,
    );
  }
  getLastChildIfKind(kind) {
    const lastChild = this._getCompilerLastChild();
    return lastChild != null && lastChild.kind === kind
      ? this._getNodeFromCompilerNode(lastChild)
      : undefined;
  }
  getChildAtIndexIfKindOrThrow(index, kind) {
    return errors.throwIfNullOrUndefined(
      this.getChildAtIndexIfKind(index, kind),
      `Child at index ${index} was expected to be ${getSyntaxKindName(kind)}`,
    );
  }
  ...
  getPreviousSiblingIfKindOrThrow(kind) {
    return errors.throwIfNullOrUndefined(
      ...
      `A previous sibling of kind ${getSyntaxKindName(kind)} was expected.`,
    );
  }
  getNextSiblingIfKindOrThrow(kind) {
    return errors.throwIfNullOrUndefined(
      this.getNextSiblingIfKind(kind),
      `A next sibling of kind ${getSyntaxKindName(kind)} was expected.`,
    );
  }
  ...
  getNextSiblingIfKind(kind) {
    const nextSibling = this._getCompilerNextSibling();
    return nextSibling != null && nextSibling.kind === kind
      ? this._getNodeFromCompilerNode(nextSibling)
      : undefined;
  }
  getParentIfOrThrow(condition) {
    return errors.throwIfNullOrUndefined(
      this.getParentIf(condition),
      "The parent did not match the provided condition.",
    );
  }
  ...
  getParentIfKindOrThrow(kind) {
    return errors.throwIfNullOrUndefined(
      ...
      `The parent was not a syntax kind of ${getSyntaxKindName(kind)}.`,
    );
  }
  ...
  getFirstAncestorByKindOrThrow(kind) {
    return errors.throwIfNullOrUndefined(
      ...
      `Expected an ancestor with a syntax kind of ${getSyntaxKindName(kind)}.`,
    );
  }
  getFirstAncestorByKind(kind) {
    for (
      const parent of this._getAncestorsIterator(kind === SyntaxKind.SyntaxList)
    ) {
  ...
  getFirstAncestorOrThrow(condition) {
    return errors.throwIfNullOrUndefined(
      ...
      `Expected to find an ancestor that matched the provided condition.`,
    );
  }
  ...
  _getCompilerChildren() {
    return ExtendedParser.getCompilerChildren(
      this.compilerNode,
      this._sourceFile.compilerNode,
    );
  }
  _getCompilerForEachChildren() {
    return ExtendedParser.getCompilerForEachChildren(
      this.compilerNode,
      this._sourceFile.compilerNode,
    );
  }
  _getCompilerChildrenFast() {
    return hasParsedTokens(this.compilerNode)
      ? this._getCompilerChildren()
      : this._getCompilerForEachChildren();
  }
  _getCompilerChildrenOfKind(kind) {
    const children = useParseTreeSearchForKind(this, kind)
      ? this._getCompilerForEachChildren()
      : this._getCompilerChildren();
  ...
  *_getCompilerDescendantsOfKindIterator(kind) {
  ...
  _getCompilerDescendantsIterator() {
    return getCompilerDescendantsIterator(
      this.compilerNode,
      this._sourceFile.compilerNode,
    );
  }
  ...
  _getNodeFromCompilerNodeIfExists(compilerNode) {
    return compilerNode == null
      ? undefined
      : this._getNodeFromCompilerNode(compilerNode);
  }
  _ensureBound() {
  ...
  static hasExpression(node) {
    var _a, _b;
    return ((_b = (_a = node).getExpression) === null || _b === void 0
      ? void 0
      : _b.call(_a)) != null;
  }
  static hasName(node) {
    var _a, _b;
    return typeof ((_b = (_a = node).getName) === null || _b === void 0
      ? void 0
      : _b.call(_a)) === "string";
  }
  static hasBody(node) {
    var _a, _b;
    return ((_b = (_a = node).getBody) === null || _b === void 0
      ? void 0
      : _b.call(_a)) != null;
  }
  ...
  static is(kind) {
    return (node) => {
      return (node === null || node === void 0 ? void 0 : node.getKind()) ==
        kind;
    };
  }
  static isNode(value) {
  ...
  static isCommentStatement(node) {
    var _a;
    return ((_a = node === null || node === void 0
            ? void 0
            : node.compilerNode) === null || _a === void 0
      ? void 0
      : _a._commentKind) === CommentNodeKind.Statement;
  }
  static isCommentClassElement(node) {
    var _a;
    return ((_a = node === null || node === void 0
            ...
            : node.compilerNode) === null || _a === void 0
      ? void 0
      : _a._commentKind) === CommentNodeKind.ClassElement;
  }
  static isCommentTypeElement(node) {
    var _a;
    return ((_a = node === null || node === void 0
            ...
            : node.compilerNode) === null || _a === void 0
      ? void 0
      : _a._commentKind) === CommentNodeKind.TypeElement;
  }
  static isCommentObjectLiteralElement(node) {
    var _a;
    return ((_a = node === null || node === void 0
            ...
            : node.compilerNode) === null || _a === void 0
      ? void 0
      : _a._commentKind) === CommentNodeKind.ObjectLiteralElement;
  }
  static isCommentEnumMember(node) {
    var _a;
    return ((_a = node === null || node === void 0
            ...
            : node.compilerNode) === null || _a === void 0
      ? void 0
      : _a._commentKind) == CommentNodeKind.EnumMember;
  }
...
Node.isClassStaticBlockDeclaration = Node.is(
...
Node.isExpressionWithTypeArguments = Node.is(
...
Node.isImportTypeAssertionContainer = Node.is(
...
Node.isNoSubstitutionTemplateLiteral = Node.is(
...
Node.isPartiallyEmittedExpression = Node.is(
...
Node.isShorthandPropertyAssignment = Node.is(
...
function getWrappedCondition(thisNode, condition) {
  return condition == null
    ? undefined
    : ((c) => condition(thisNode._getNodeFromCompilerNode(c)));
}
function insertWhiteSpaceTextAtPos(
  node,
  insertPos,
  textOrWriterFunction,
  methodName,
) {
  const parent = Node.isSourceFile(node)
    ? node.getChildSyntaxListOrThrow()
    : node.getParentSyntaxList() || node.getParentOrThrow();
  const newText = getTextFromStringOrWriter(
    node._getWriterWithQueuedIndentation(),
    textOrWriterFunction,
  );
  if (!/^[\s\r\n]*$/.test(newText)) {
    throw new errors.InvalidOperationError(
      `Cannot insert non-whitespace into ${methodName}.`,
    );
  }
  insertIntoParentTextRange({
    parent,
...
function useParseTreeSearchForKind(thisNodeOrSyntaxKind, searchingKind) {
  return searchingKind >= SyntaxKind.FirstNode &&
    searchingKind < SyntaxKind.FirstJSDocNode &&
...
class SyntaxList extends Node {
  ...
  insertChildText(index, textOrWriterFunction) {
    ...
    const newLineKind = this._context.manipulationSettings
      .getNewLineKindAsString();
    ...
    let insertText = getTextFromStringOrWriter(
      isInline
        ? parent._getWriterWithQueuedChildIndentation()
        : parent._getWriterWithChildIndentation(),
      textOrWriterFunction,
    );
    ...
    if (isInline) {
      ...
      if (index === 0 && Node.isSourceFile(parent)) {
        ...
        if (
          !Node.isSourceFile(parent) && index === initialChildCount &&
          insertText.endsWith("\n")
        ) {
...
function renameNode(node, newName, options) {
  ...
  const renameLocations = node._context.languageService.findRenameLocations(
    node,
    options,
  );
  const renameLocationsBySourceFile = new KeyValueCache();
  for (const renameLocation of renameLocations) {
    const locations = renameLocationsBySourceFile.getOrCreate(
  ...
  for (
    const [sourceFile, locations] of renameLocationsBySourceFile.getEntries()
  ) {
...
function setBodyTextForNode(body, textOrWriterFunction) {
  const newText = getBodyText(
    body._getWriterWithIndentation(),
    textOrWriterFunction,
  );
  const openBrace = body.getFirstChildByKindOrThrow(SyntaxKind.OpenBraceToken);
  const closeBrace = body.getFirstChildByKindOrThrow(
    SyntaxKind.CloseBraceToken,
  );
  insertIntoParentTextRange({
...
function BodyableNode(Base) {
  return class extends Base {
    getBodyOrThrow() {
      return errors.throwIfNullOrUndefined(
        this.getBody(),
        "Expected to find the node's body.",
      );
    }
    getBody() {
    ...
    getBodyText() {
      ...
      return body == null
        ? undefined
        : getBodyTextWithoutLeadingIndentation(body);
    }
    setBodyText(textOrWriterFunction) {
      this.addBody();
    ...
    addBody() {
      ...
      insertIntoParentTextRange({
        ...
        newText: this._getWriterWithQueuedIndentation().space().block()
          .toString(),
        replacing: {
          textLength: (_a = semiColon === null || semiColon === void 0
                  ? void 0
                  : semiColon.getFullWidth()) !== null && _a !== void 0
            ? _a
            : 0,
        },
      });
...
function ChildOrderableNode(Base) {
  return class extends Base {
    setOrder(order) {
      const childIndex = this.getChildIndex();
      const parent = this.getParentSyntaxList() ||
        this.getParentSyntaxListOrThrow();
...
function DecoratableNode(Base) {
  return class extends Base {
    getDecorator(nameOrFindFunction) {
      return getNodeByNameOrFindFunction(
        this.getDecorators(),
        nameOrFindFunction,
      );
    }
    ...
    getDecorators() {
      return getCompilerNodeDecorators(this.compilerNode).map((d) =>
        this._getNodeFromCompilerNode(d)
      );
    }
    addDecorator(structure) {
      return this.insertDecorator(
        getEndIndexFromArray(getCompilerNodeDecorators(this.compilerNode)),
        structure,
      );
    }
    addDecorators(structures) {
      return this.insertDecorators(
        getEndIndexFromArray(getCompilerNodeDecorators(this.compilerNode)),
        structures,
      );
    }
    ...
    insertDecorators(index, structures) {
      ...
      const decoratorCode = getNewInsertCode({
        ...
        previousFormattingKind: previousDecorator == null
          ? FormattingKind.None
          : formattingKind,
        nextFormattingKind: previousDecorator == null
          ? formattingKind
          : FormattingKind.None,
      });
      insertIntoParentTextRange({
        parent: (_d = (_b = (_a = decorators[0]) === null || _a === void 0
                      ? void 0
                      : _a.getParentSyntaxListOrThrow()) !== null &&
                  _b !== void 0
                ? _b
                : (_c = this.getModifiers()[0]) === null || _c === void 0
                ? void 0
                : _c.getParentSyntaxListOrThrow()) !== null && _d !== void 0
          ? _d
          : this,
        insertPos: decorators[index - 1] == null
          ? this.getStart()
          : decorators[index - 1].getEnd(),
...
function getCompilerNodeDecorators(node) {
  ...
  return ts.canHaveDecorators(node)
    ? (_a = ts.getDecorators(node)) !== null && _a !== void 0 ? _a : []
    : [];
}
function getDecoratorLines(node, structures) {
  ...
  for (const structure of structures) {
    ...
    const structurePrinter = node._context.structurePrinterFactory
      .forDecorator();
  ...
  return class extends Base {
    getDotDotDotTokenOrThrow() {
      return errors.throwIfNullOrUndefined(
        ...
        "Expected to find a dot dot dot token (...).",
      );
    }
    getDotDotDotToken() {
  ...
  return class extends Base {
    ...
    getExclamationTokenNodeOrThrow() {
      return errors.throwIfNullOrUndefined(
        ...
        "Expected to find an exclamation token.",
      );
    }
  ...
  return class extends Base {
    ...
    getExportKeyword() {
      if (Node.isVariableDeclaration(this)) {
        ...
        return variableStatement === null || variableStatement === void 0
          ? void 0
          : variableStatement.getExportKeyword();
      }
    ...
    getExportKeywordOrThrow() {
      return errors.throwIfNullOrUndefined(
        ...
        "Expected to find an export keyword.",
      );
    }
    hasDefaultKeyword() {
    ...
    getDefaultKeyword() {
      if (Node.isVariableDeclaration(this)) {
        ...
        return variableStatement === null || variableStatement === void 0
          ? void 0
          : variableStatement.getDefaultKeyword();
      }
      if (!Node.isModifierable(this)) {
        return throwForNotModifierableNode();
      }
    ...
    getDefaultKeywordOrThrow() {
      return errors.throwIfNullOrUndefined(
        ...
        "Expected to find a default keyword.",
      );
    }
    isExported() {
      ...
      return sourceFileSymbol.getExports().some((e) =>
        e === thisSymbol || e.getAliasedSymbol() === thisSymbol
      );
    }
    isDefaultExport() {
    ...
    isNamedExport() {
      ...
      return !isDefaultExport() &&
        sourceFileSymbol.getExports().some((e) =>
          e === thisSymbol || e.getAliasedSymbol() === thisSymbol
        );
      function isDefaultExport() {
        ...
        return thisSymbol === defaultExportSymbol ||
          thisSymbol === defaultExportSymbol.getAliasedSymbol();
      }
    }
  };
}
  ...
  return class extends Base {
    setIsDefaultExport(value) {
      ...
      if (Node.hasName(this) && shouldWriteAsSeparateStatement.call(this)) {
        const parentSyntaxList = this.getFirstAncestorByKindOrThrow(
      ...
      function shouldWriteAsSeparateStatement() {
        if (
          Node.isEnumDeclaration(this) || Node.isModuleDeclaration(this) ||
          Node.isTypeAliasDeclaration(this)
        ) {
          return true;
        }
...
class Printer {
  ...
  getTextWithQueuedChildIndentation(writer, textOrWriterFunc) {
    const queuedChildIndentationWriter = this
      .getNewWriterWithQueuedChildIndentation(writer);
...
function printTextWithSeparator(printer, writer, structures, separator) {
  ...
  if (structures instanceof Function || typeof structures === "string") {
    ...
    for (let i = 0; i < structures.length; i++) {
      ...
      const pos = getAppendCommaPos(
        WriterUtils.getLastCharactersToPos(writer, startPos),
      );
...
class ClassDeclarationStructurePrinter extends NodePrinter {
  ...
  printTextInternal(writer, structure) {
    ...
    writer.inlineBlock(() => {
      this.factory.forPropertyDeclaration().printTexts(
        writer,
      ...
      if (!ArrayUtils.isNullOrEmpty(structure.methods)) {
        ...
        this.factory.forMethodDeclaration({ isAmbient }).printTexts(
          writer,
          structure.methods,
        );
      }
  ...
  printHeader(writer, structure) {
    ...
    this.factory.forTypeParameterDeclaration().printTextsWithBrackets(
      writer,
  ...
  printCtors(writer, structure, isAmbient) {
    ...
    for (const ctor of structure.ctors) {
      this.conditionalSeparator(writer, isAmbient);
      this.factory.forConstructorDeclaration({ isAmbient }).printText(
        writer,
        ctor,
      );
    }
  ...
  printGetAndSet(writer, structure, isAmbient) {
    var _a, _b;
    const getAccessors = [
      ...(_a = structure.getAccessors) !== null && _a !== void 0 ? _a : [],
    ];
    const setAccessors = [
      ...(_b = structure.setAccessors) !== null && _b !== void 0 ? _b : [],
    ];
    const getAccessorWriter = this.factory.forGetAccessorDeclaration({
      isAmbient,
    });
    const setAccessorWriter = this.factory.forSetAccessorDeclaration({
      isAmbient,
    });
    for (const getAccessor of getAccessors) {
      this.conditionalSeparator(writer, isAmbient);
      getAccessorWriter.printText(writer, getAccessor);
      const setAccessorIndex = setAccessors.findIndex((item) =>
        item.name === getAccessor.name
      );
...
(function (StructureKind) {
  ...
  StructureKind[StructureKind["ConstructorOverload"] = 6] =
    "ConstructorOverload";
  ...
  StructureKind[StructureKind["JsxSpreadAttribute"] = 21] =
    "JsxSpreadAttribute";
  StructureKind[StructureKind["JsxElement"] = 22] = "JsxElement";
  StructureKind[StructureKind["JsxSelfClosingElement"] = 23] =
  ...
  StructureKind[StructureKind["VariableDeclaration"] = 40] =
    "VariableDeclaration";
...
const Structure = {
  ...
  isAssertEntry(structure) {
    return (structure === null || structure === void 0
      ? void 0
      : structure.kind) === StructureKind.AssertEntry;
  },
  ...
  isCallSignature(structure) {
    return (structure === null || structure === void 0
      ? void 0
      : structure.kind) === StructureKind.CallSignature;
  },
  isJSDocable(structure) {
    switch (
      structure === null || structure === void 0 ? void 0 : structure.kind
    ) {
  ...
  isClassStaticBlock(structure) {
    return (structure === null || structure === void 0
      ? void 0
      : structure.kind) === StructureKind.ClassStaticBlock;
  },
  ...
  isConstructSignature(structure) {
    return (structure === null || structure === void 0
      ? void 0
      : structure.kind) === StructureKind.ConstructSignature;
  },
  isDecorator(structure) {
    return (structure === null || structure === void 0
      ? void 0
      : structure.kind) === StructureKind.Decorator;
  },
  isEnum(structure) {
    return (structure === null || structure === void 0
      ? void 0
  ...
  isEnumMember(structure) {
    return (structure === null || structure === void 0
      ? void 0
      : structure.kind) === StructureKind.EnumMember;
  },
  ...
  isExportSpecifier(structure) {
    return (structure === null || structure === void 0
      ? void 0
      : structure.kind) === StructureKind.ExportSpecifier;
  },
  isFunctionDeclarationOverload(structure) {
    return (structure === null || structure === void 0
      ? void 0
      : structure.kind) === StructureKind.FunctionOverload;
  },
  ...
  isGetAccessor(structure) {
    return (structure === null || structure === void 0
      ? void 0
      : structure.kind) === StructureKind.GetAccessor;
  },
  ...
  isImportDeclaration(structure) {
    return (structure === null || structure === void 0
      ? void 0
      : structure.kind) === StructureKind.ImportDeclaration;
  },
  isImportSpecifier(structure) {
    return (structure === null || structure === void 0
      ? void 0
      : structure.kind) === StructureKind.ImportSpecifier;
  },
  ...
  isJSDoc(structure) {
    return (structure === null || structure === void 0
      ? void 0
      : structure.kind) === StructureKind.JSDoc;
  },
  isJSDocTag(structure) {
    return (structure === null || structure === void 0
      ? void 0
      : structure.kind) === StructureKind.JSDocTag;
  },
  isJsxAttribute(structure) {
    return (structure === null || structure === void 0
      ? void 0
      : structure.kind) === StructureKind.JsxAttribute;
  },
  isJsxElement(structure) {
    return (structure === null || structure === void 0
      ? void 0
      : structure.kind) === StructureKind.JsxElement;
  },
  ...
  isJsxSpreadAttribute(structure) {
    return (structure === null || structure === void 0
      ? void 0
      : structure.kind) === StructureKind.JsxSpreadAttribute;
  },
  ...
  isMethod(structure) {
    return (structure === null || structure === void 0
      ? void 0
      : structure.kind) === StructureKind.Method;
  },
  isMethodSignature(structure) {
    return (structure === null || structure === void 0
      ? void 0
      : structure.kind) === StructureKind.MethodSignature;
  },
  isModule(structure) {
    return (structure === null || structure === void 0
      ? void 0
      : structure.kind) === StructureKind.Module;
  },
  ...
  isParameter(structure) {
    return (structure === null || structure === void 0
      ? void 0
      : structure.kind) === StructureKind.Parameter;
  },
  ...
  isPropertyAssignment(structure) {
    return (structure === null || structure === void 0
      ? void 0
      : structure.kind) === StructureKind.PropertyAssignment;
  },
  ...
  isPropertySignature(structure) {
    return (structure === null || structure === void 0
      ? void 0
      : structure.kind) === StructureKind.PropertySignature;
  },
  isSetAccessor(structure) {
    return (structure === null || structure === void 0
      ? void 0
      : structure.kind) === StructureKind.SetAccessor;
  },
  ...
  isSpreadAssignment(structure) {
    return (structure === null || structure === void 0
      ? void 0
      : structure.kind) === StructureKind.SpreadAssignment;
  },
  ...
  isTypeParameter(structure) {
    return (structure === null || structure === void 0
      ? void 0
      : structure.kind) === StructureKind.TypeParameter;
  },
  ...
  isVariableStatement(structure) {
    return (structure === null || structure === void 0
      ? void 0
      : structure.kind) === StructureKind.VariableStatement;
  },
};
...
function forTypeElementMemberedNode(structure, callback) {
  return forAll(
    structure.callSignatures,
    callback,
    StructureKind.CallSignature,
  ) ||
    forAll(
      structure.constructSignatures,
      callback,
...
class ClassMemberStructurePrinter extends Printer {
  ...
  printText(writer, member) {
    if (
      typeof member === "string" || member instanceof Function || member == null
    ) {
    ...
    switch (member.kind) {
      case StructureKind.Method:
        ...
        this.factory.forMethodDeclaration(this.options).printText(
          writer,
          member,
        );
        break;
      ...
      case StructureKind.GetAccessor:
        ...
        this.factory.forGetAccessorDeclaration(this.options).printText(
          writer,
          member,
        );
        break;
      case StructureKind.SetAccessor:
        ...
        this.factory.forSetAccessorDeclaration(this.options).printText(
          writer,
          member,
        );
        break;
      case StructureKind.Constructor:
        ...
        this.factory.forConstructorDeclaration(this.options).printText(
          writer,
          member,
        );
        break;
...
class ClassStaticBlockDeclarationStructurePrinter extends NodePrinter {
  ...
  printTextInternal(writer, structure) {
    ...
    writer.space().inlineBlock(() => {
      this.factory.forStatementedNode({ isAmbient: false }).printText(
        writer,
        structure,
      );
    });
...
class ConstructorDeclarationStructurePrinter extends NodePrinter {
  ...
  printTextInternal(writer, structure) {
    ...
    if (this.options.isAmbient) {
      ...
      writer.space().inlineBlock(() => {
        this.factory.forStatementedNode(this.options).printText(
          writer,
          structure,
        );
      });
  ...
  printHeader(writer, structure) {
    ...
    this.factory.forParameterDeclaration().printTextsWithParenthesis(
      writer,
...
class PropertyDeclarationStructurePrinter extends NodePrinter {
  ...
  printTextInternal(writer, structure) {
    ...
    writer.conditionalWrite(
      structure.hasExclamationToken && !structure.hasQuestionToken,
      "!",
    );
    this.factory.forTypedNode(":").printText(writer, structure);
    this.factory.forInitializerExpressionableNode().printText(
      writer,
      structure,
    );
    writer.write(";");
...
class DecoratorStructurePrinter extends NodePrinter {
  ...
  printTypeArguments(writer, structure) {
    if (
      structure.typeArguments == null || structure.typeArguments.length === 0
    ) {
      return;
    }
    writer.write("<");
    for (let i = 0; i < structure.typeArguments.length; i++) {
      writer.conditionalWrite(i > 0, ", ");
      writer.write(
        this.getTextWithQueuedChildIndentation(
          writer,
          structure.typeArguments[i],
        ),
      );
    }
    writer.write(">");
  ...
  printArguments(writer, structure) {
    ...
    writer.write("(");
    const args = structure.arguments instanceof Array
      ? structure.arguments
      : [structure.arguments];
    ...
    function getText(jsdocPrinter) {
      ...
      if (typeof structure === "function") {
        ...
        if (structure.tags && structure.tags.length > 0) {
          ...
          jsdocPrinter.factory.forJSDocTag({ printStarsOnNewLine: false })
            .printTexts(tempWriter, structure.tags);
        }
      }
  ...
  printTextInternal(writer, structure) {
    ...
    function getText(tagPrinter) {
      ...
      if (typeof structure === "function") {
        ...
        tempWriter.unsafeInsert(
          0,
          `@${structure.tagName}` +
            (currentText.length > 0 &&
                !StringUtils.startsWithNewLine(currentText)
              ? " "
              : ""),
        );
      }
...
class EnumMemberStructurePrinter extends NodePrinter {
  ...
  printTextInternal(writer, structure) {
    ...
    this.factory.forJSDoc().printDocs(writer, structure.docs);
    if (
      isValidVariableName(structure.name) ||
      StringUtils.isQuoted(structure.name)
    ) {
      writer.write(structure.name);
    } else {
  ...
  printTexts(writer, structures) {
    ...
    for (let i = 0; i < structures.length; i++) {
      ...
      if (i > 0) {
        ...
        if (
          this.options.isAmbient ||
          previousStructure.hasDeclareKeyword &&
            currentStructure.hasDeclareKeyword
        ) {
          writer.newLine();
        } else {
          writer.blankLine();
...
class ParameterDeclarationStructurePrinter extends NodePrinter {
  ...
  printTextInternal(writer, structure) {
    ...
    writer.write(structure.name);
    writer.conditionalWrite(structure.hasQuestionToken, "?");
    this.factory.forTypedNode(":", structure.hasQuestionToken).printText(
      writer,
      structure,
    );
    this.factory.forInitializerExpressionableNode().printText(
...
class IndexSignatureDeclarationStructurePrinter extends NodePrinter {
  ...
  printTextInternal(writer, structure) {
    ...
    this.factory.forModifierableNode().printText(writer, structure);
    writer.write(
      `[${structure.keyName || "key"}: ${structure.keyType || "string"}]`,
    );
    this.factory.forReturnTypedNode().printText(writer, structure);
    writer.write(";");
  }
...
class TypeElementMemberedNodeStructurePrinter extends Printer {
  ...
  printText(writer, structure) {
    this.factory.forCallSignatureDeclaration().printTexts(
      writer,
      structure.callSignatures,
    );
    this.conditionalSeparator(writer, structure.constructSignatures);
    this.factory.forConstructSignatureDeclaration().printTexts(
      writer,
      structure.constructSignatures,
    );
    this.conditionalSeparator(writer, structure.indexSignatures);
    this.factory.forIndexSignatureDeclaration().printTexts(
      writer,
      structure.indexSignatures,
    );
    this.conditionalSeparator(writer, structure.properties);
    this.factory.forPropertySignature().printTexts(
      writer,
      structure.properties,
    );
  ...
  conditionalSeparator(writer, structures) {
    if (
      !ArrayUtils.isNullOrEmpty(structures) &&
...
class TypeElementMemberStructurePrinter extends Printer {
  ...
  printText(writer, members) {
    if (
      typeof members === "string" || members instanceof Function ||
      members == null
    ) {
    ...
    switch (members.kind) {
      ...
      case StructureKind.IndexSignature:
        ...
        break;
      case StructureKind.ConstructSignature:
        this.factory.forConstructSignatureDeclaration().printText(
          writer,
          members,
        );
        break;
...
class JsxAttributeDeciderStructurePrinter extends NodePrinter {
  printTextInternal(writer, structure) {
    if (isJsxAttribute()) {
      ...
      this.factory.forJsxSpreadAttribute().printTextWithoutTrivia(
        writer,
        structure,
      );
    } else {
...
class ExportAssignmentStructurePrinter extends NodePrinter {
  ...
  printTextInternal(writer, structure) {
    ...
    writer.write(
      this.getTextWithQueuedChildIndentation(writer, structure.expression),
    ).write(";");
  }
}
...
class ExportDeclarationStructurePrinter extends NodePrinter {
  ...
  printTextInternal(writer, structure) {
    const hasModuleSpecifier = structure.moduleSpecifier != null &&
      structure.moduleSpecifier.length > 0;
    const hasNamedImport = structure.namedExports != null &&
      structure.namedExports.length > 0;
    ...
    if (structure.namedExports != null && structure.namedExports.length > 0) {
      writer.space();
      this.factory.forNamedImportExportSpecifier().printTextsWithBraces(
        writer,
        structure.namedExports,
      );
    ...
    if (structure.assertElements) {
      writer.space();
      this.factory.forAssertEntry().printAssertClause(
        writer,
        structure.assertElements,
      );
    }
    writer.write(";");
  }
...
class ImportDeclarationStructurePrinter extends NodePrinter {
  ...
  printTextInternal(writer, structure) {
    const hasNamedImport = structure.namedImports != null &&
    ...
    if (structure.defaultImport != null) {
      ...
      writer.conditionalWrite(
        hasNamedImport || structure.namespaceImport != null,
        ",",
      );
    }
    ...
    writer.conditionalWrite(
      structure.defaultImport != null || hasNamedImport ||
        structure.namespaceImport != null,
      " from",
    );
    writer.write(" ");
    writer.quote(structure.moduleSpecifier);
...
class ModuleDeclarationStructurePrinter extends NodePrinter {
  constructor(factory, options) {
    super(factory);
    this.options = options;
    this.blankLineFormattingWriter = new BlankLineFormattingStructuresPrinter(
      this,
    );
  }
  printTexts(writer, structures) {
  ...
  printTextInternal(writer, structure) {
    ...
    this.factory.forModifierableNode().printText(writer, structure);
    if (
      structure.declarationKind == null ||
      structure.declarationKind !== ModuleDeclarationKind.Global
    ) {
      writer.write(
        `${structure.declarationKind || "namespace"} ${structure.name}`,
      );
    } else {
      writer.write("global");
    }
    if (
      structure.hasDeclareKeyword &&
      StringUtils.isQuoted(structure.name.trim()) &&
      structure.hasOwnProperty(nameof(structure, "statements")) &&
      structure.statements == null
    ) {
      writer.write(";");
    } else {
      writer.write(" ");
...
class NamedImportExportSpecifierStructurePrinter extends NodePrinter {
  ...
  printTextsWithBraces(writer, structures) {
    ...
    if (
      ...
      !StringUtils.startsWithNewLine(specifierText)
    ) {
      writer.space();
    }
    ...
    if (
      formatSettings.insertSpaceAfterOpeningAndBeforeClosingNonemptyBraces &&
      !StringUtils.endsWithNewLine(specifierText)
    ) {
      writer.space();
    }
    writer.write("}");
...
class SourceFileStructurePrinter extends NodePrinter {
  ...
  printTextInternal(writer, structure) {
    ...
    writer.conditionalNewLine(
      !writer.isAtStartOfFirstLineOfBlock() && !writer.isLastNewLine(),
    );
  }
}
...
class StatementedNodeStructurePrinter extends Printer {
  ...
  printText(writer, structure) {
    this.factory.forStatement(this.options).printTexts(
      writer,
...
class StatementStructurePrinter extends Printer {
  ...
  printText(writer, statement) {
    if (
      typeof statement === "string" || statement instanceof Function ||
      statement == null
    ) {
    ...
    switch (statement.kind) {
      case StructureKind.Function:
        ...
        this.factory.forFunctionDeclaration(this.options).printText(
          writer,
          statement,
        );
        break;
      case StructureKind.Class:
        ensureBlankLine();
        this.factory.forClassDeclaration(this.options).printText(
          writer,
          statement,
        );
        break;
      ...
      case StructureKind.Module:
        ensureBlankLine();
        this.factory.forModuleDeclaration(this.options).printText(
          writer,
          statement,
        );
        break;
...
class VariableStatementStructurePrinter extends NodePrinter {
  ...
  printTextInternal(writer, structure) {
    ...
    writer.hangingIndent(() => {
      this.factory.forModifierableNode().printText(writer, structure);
      writer.write(
        `${structure.declarationKind || VariableDeclarationKind.Let} `,
      );
      this.factory.forVariableDeclaration().printTexts(
        writer,
...
function ExtendsClauseableNode(Base) {
  return class extends Base {
    getExtends() {
      var _a;
      const extendsClause = this.getHeritageClauseByKind(
        SyntaxKind.ExtendsKeyword,
      );
      return (_a = extendsClause === null || extendsClause === void 0
              ? void 0
              : extendsClause.getTypeNodes()) !== null && _a !== void 0
        ? _a
        : [];
    }
    addExtends(text) {
    ...
    insertExtends(index, texts) {
      ...
      const writer = this._getWriterWithQueuedChildIndentation();
      const structurePrinter = new CommaSeparatedStructuresPrinter(
        new StringStructurePrinter(),
      );
      structurePrinter.printText(writer, texts);
      index = verifyAndGetIndex(index, originalExtends.length);
      if (originalExtends.length > 0) {
        const extendsClause = this.getHeritageClauseByKindOrThrow(
          SyntaxKind.ExtendsKeyword,
        );
        insertIntoCommaSeparatedNodes({
          parent: extendsClause.getFirstChildByKindOrThrow(
            SyntaxKind.SyntaxList,
          ),
        ...
        const openBraceToken = this.getFirstChildByKindOrThrow(
          SyntaxKind.OpenBraceToken,
        );
        const openBraceStart = openBraceToken.getStart();
        const isLastSpace = /\s/.test(
          this.getSourceFile().getFullText()[openBraceStart - 1],
        );
      ...
      return wasStringInput
        ? newExtends[index]
        : getNodesToReturn(originalExtends, newExtends, index, false);
    }
  ...
  return class extends Base {
    ...
    getAsteriskTokenOrThrow() {
      return errors.throwIfNullOrUndefined(
        ...
        "Expected to find an asterisk token.",
      );
    }
...
function HeritageClauseableNode(Base) {
  return class extends Base {
    getHeritageClauses() {
      ...
      return (_a = heritageClauses === null || heritageClauses === void 0
              ? void 0
              : heritageClauses.map((c) =>
                this._getNodeFromCompilerNode(c)
              )) !== null && _a !== void 0
        ? _a
        : [];
    ...
    getHeritageClauseByKindOrThrow(kind) {
      return errors.throwIfNullOrUndefined(
        ...
        `Expected to have heritage clause of kind ${getSyntaxKindName(kind)}.`,
      );
    }
    getHeritageClauseByKind(kind) {
      return this.getHeritageClauses().find((c) =>
        c.compilerNode.token === kind
      );
    }
  };
}
...
function ImplementsClauseableNode(Base) {
  return class extends Base {
    getImplements() {
      var _a;
      const implementsClause = this.getHeritageClauseByKind(
        SyntaxKind.ImplementsKeyword,
      );
      return (_a = implementsClause === null || implementsClause === void 0
              ? void 0
              : implementsClause.getTypeNodes()) !== null && _a !== void 0
        ? _a
        : [];
    }
    addImplements(text) {
    ...
    insertImplements(index, texts) {
      ...
      if (originalImplements.length > 0) {
        const implementsClause = this.getHeritageClauseByKindOrThrow(
          SyntaxKind.ImplementsKeyword,
        );
        insertIntoCommaSeparatedNodes({
          parent: implementsClause.getFirstChildByKindOrThrow(
            SyntaxKind.SyntaxList,
          ),
        ...
        insertIntoParentTextRange({
          parent: heritageClauses.length === 0
            ? this
            : heritageClauses[0].getParentSyntaxListOrThrow(),
          insertPos: openBraceStart,
          newText: insertText,
        });
      }
      ...
      return wasStringInput
        ? newImplements[0]
        : getNodesToReturn(originalImplements, newImplements, index, false);
    }
  ...
  return class extends Base {
    ...
    getInitializerIfKindOrThrow(kind) {
      return errors.throwIfNullOrUndefined(
        ...
        `Expected to find an initializer of kind '${getSyntaxKindName(kind)}'.`,
      );
    }
    ...
    getInitializerOrThrow() {
      return errors.throwIfNullOrUndefined(
        this.getInitializer(),
        "Expected to find an initializer.",
      );
    }
    getInitializer() {
      return this._getNodeFromCompilerNodeIfExists(
...
function apply(Base) {
  return class extends Base {
    removeInitializer() {
      ...
      const previousSibling = initializer.getPreviousSiblingIfKindOrThrow(
    ...
    setInitializer(textOrWriterFunction) {
      const text = getTextFromStringOrWriter(
      ...
      insertIntoParentTextRange({
        insertPos: semiColonToken != null
          ? semiColonToken.getPos()
          : this.getEnd(),
        parent: this,
        newText: ` = ${text}`,
      });
...
function JSDocableNode(Base) {
  return class extends Base {
    getJsDocs() {
      ...
      return (_a = nodes === null || nodes === void 0
              ? void 0
              : nodes.map((n) => this._getNodeFromCompilerNode(n))) !== null &&
          _a !== void 0
        ? _a
        : [];
    }
    ...
    addJsDocs(structures) {
      return this.insertJsDocs(
        getEndIndexFromArray(this.compilerNode.jsDoc),
        structures,
      );
    }
    ...
    insertJsDocs(index, structures) {
      ...
      const insertPos = index === nodes.length
        ? this.getStart()
        : nodes[index].getStart();
      insertIntoParentTextRange({
        insertPos,
        parent: this,
        newText: code,
...
function ModifierableNode(Base) {
  return class extends Base {
    getModifiers() {
      return this.getCompilerModifiers().map((m) =>
        this._getNodeFromCompilerNode(m)
      );
    }
    getFirstModifierByKindOrThrow(kind) {
      return errors.throwIfNullOrUndefined(
        this.getFirstModifierByKind(kind),
        `Expected a modifier of syntax kind: ${getSyntaxKindName(kind)}`,
      );
    }
    ...
    addModifier(text) {
      ...
      const modifiers = this.getModifiers().filter((m) =>
        m.getKind() !== SyntaxKind.Decorator
      );
      ...
      let startPos;
      let newText;
      const isFirstModifier = modifiers.length === 0 ||
        insertPos === modifiers[0].getStart();
      if (isFirstModifier) {
        newText = text + " ";
        startPos = insertPos;
      } else {
      ...
      insertIntoParentTextRange({
        parent: rawModifiers.length === 0
          ? this
          : rawModifiers[0].getParentSyntaxListOrThrow(),
        insertPos,
        newText,
      });
        ...
        function getInitialInsertPos() {
          ...
          for (const child of node._getChildrenIterator()) {
            if (
              ...
              ts.isJSDocCommentContainingNode(child.compilerNode)
            ) {
              continue;
            }
    ...
    removeModifier(text) {
      ...
      removeChildren({
        children: [
          modifiers.length === 1
            ? modifier.getParentSyntaxListOrThrow()
            : modifier,
        ],
        removeFollowingSpaces: true,
      });
  ...
  return class extends Base {
    ...
    addImportDeclarations(structures) {
      ...
      function getInsertIndex() {
        ...
        for (let i = 0; i < compilerChildren.length; i++) {
          ...
          if (
            wasLastComment && child.kind === SyntaxKind.MultiLineCommentTrivia
          ) {
            insertIndex = i + 1;
          } else {
      ...
      return this._insertChildren({
        ...
        write: (writer, info) => {
          this._standardWrite(writer, info, () => {
            this._context.structurePrinterFactory.forImportDeclaration()
              .printTexts(writer, structures);
          }, {
            previousNewLine: (previousMember) =>
              Node.isImportDeclaration(previousMember) ||
              isComment(previousMember.compilerNode),
    ...
    getImportDeclaration(conditionOrModuleSpecifier) {
      ...
      function getCondition() {
        if (typeof conditionOrModuleSpecifier === "string") {
          return (dec) =>
            dec.getModuleSpecifierValue() === conditionOrModuleSpecifier;
        } else {
    ...
    insertExportDeclarations(index, structures) {
      return this._insertChildren({
        ...
        write: (writer, info) => {
          this._standardWrite(writer, info, () => {
            this._context.structurePrinterFactory.forExportDeclaration()
              .printTexts(writer, structures);
          }, {
            previousNewLine: (previousMember) =>
              Node.isExportDeclaration(previousMember) ||
              isComment(previousMember.compilerNode),
    ...
    insertExportAssignments(index, structures) {
      return this._insertChildren({
        ...
        write: (writer, info) => {
          this._standardWrite(writer, info, () => {
            this._context.structurePrinterFactory.forExportAssignment()
              .printTexts(writer, structures);
          }, {
            previousNewLine: (previousMember) =>
              Node.isExportAssignment(previousMember) ||
              isComment(previousMember.compilerNode),
    ...
    getDefaultExportSymbolOrThrow() {
      return errors.throwIfNullOrUndefined(
        ...
        "Expected to find a default export symbol",
      );
    }
    getExportSymbols() {
      ...
      return symbol == null
        ? []
        : this._context.typeChecker.getExportsOfModule(symbol);
    }
    getExportedDeclarations() {
      ...
      function* getDeclarationHandlingImportsAndExports(declaration) {
        if (Node.isExportSpecifier(declaration)) {
          ...
          if (
            expression == null || expression.getKind() !== SyntaxKind.Identifier
          ) {
            yield expression;
            return;
          }
...
function ReferenceFindableNode(Base) {
  return class extends Base {
    findReferences() {
      return this._context.languageService.findReferences(
        getNodeForReferences(this),
      );
    }
    findReferencesAsNodes() {
      return this._context.languageService.findReferencesAsNodes(
        getNodeForReferences(this),
      );
    }
  };
  ...
  return class extends Base {
    rename(newName, options) {
      ...
      function getNodeToRename(thisNode) {
        if (
          Node.isIdentifier(thisNode) || Node.isPrivateIdentifier(thisNode) ||
          Node.isStringLiteral(thisNode)
        ) {
          return thisNode;
          ...
          if (
            ...
            throw new errors.NotImplementedError(
              `Not implemented renameable scenario for ${node.getKindName()}.`,
            );
          }
          return node;
        } else {
          throw new errors.NotImplementedError(
            `Not implemented renameable scenario for ${thisNode.getKindName()}`,
          );
        }
      }
    }
...
function NameableNodeInternal(Base) {
  return class extends Base {
    ...
    getNameNodeOrThrow() {
      return errors.throwIfNullOrUndefined(
        this.getNameNode(),
        "Expected to have a name node.",
      );
    }
    getName() {
      var _a, _b;
      return (_b = (_a = this.getNameNode()) === null || _a === void 0
              ? void 0
              : _a.getText()) !== null && _b !== void 0
        ? _b
        : undefined;
    }
    getNameOrThrow() {
...
function addNameNode(node, newName) {
  if (Node.isClassDeclaration(node) || Node.isClassExpression(node)) {
    const classKeyword = node.getFirstChildByKindOrThrow(
    ...
    const openParenToken = node.getFirstChildByKindOrThrow(
  ...
  return class extends Base {
    ...
    getOverrideKeywordOrThrow() {
      return errors.throwIfNullOrUndefined(
        ...
        "Expected to find an override keyword.",
      );
    }
...
function ParameteredNode(Base) {
  return class extends Base {
    ...
    getParameterOrThrow(nameOrFindFunction) {
      return errors.throwIfNullOrUndefined(
        this.getParameter(nameOrFindFunction),
        () =>
          getNotFoundErrorMessageForNameOrFindFunction(
            "parameter",
            nameOrFindFunction,
    ...
    getParameters() {
      return this.compilerNode.parameters.map((p) =>
        this._getNodeFromCompilerNode(p)
      );
    }
    ...
    addParameters(structures) {
      return this.insertParameters(
        getEndIndexFromArray(this.compilerNode.parameters),
        structures,
      );
    }
    ...
    insertParameters(index, structures) {
      ...
      const syntaxList = this.getFirstChildByKindOrThrow(
        SyntaxKind.OpenParenToken,
      ).getNextSiblingIfKindOrThrow(SyntaxKind.SyntaxList);
      index = verifyAndGetIndex(index, parameters.length);
      const writer = this._getWriterWithQueuedChildIndentation();
      const structurePrinter = this._context.structurePrinterFactory
  ...
  return class extends Base {
    ...
    getQuestionDotTokenNodeOrThrow() {
      return errors.throwIfNullOrUndefined(
        ...
        "Expected to find a question dot token.",
      );
    }
    setHasQuestionDotToken(value) {
      ...
      function getInsertPos() {
        if (Node.isCallExpression(this)) {
          return this.getFirstChildByKindOrThrow(SyntaxKind.OpenParenToken)
            .getStart();
        }
        if (Node.isElementAccessExpression(this)) {
          return this.getFirstChildByKindOrThrow(SyntaxKind.OpenBracketToken)
            .getStart();
        }
  ...
  return class extends Base {
    ...
    getQuestionTokenNodeOrThrow() {
      return errors.throwIfNullOrUndefined(
        ...
        "Expected to find a question token.",
      );
    }
    setHasQuestionToken(value) {
      ...
      function getInsertPos() {
        ...
        const semicolonToken = this.getLastChildByKind(
  ...
  return class extends Base {
    ...
    getReadonlyKeywordOrThrow() {
      return errors.throwIfNullOrUndefined(
        ...
        "Expected to find a readonly keyword.",
      );
    }
    setIsReadonly(value) {
  ...
  return class extends Base {
    ...
    getReturnTypeNodeOrThrow() {
      return errors.throwIfNullOrUndefined(
        ...
        "Expected to find a return type node.",
      );
    }
    setReturnType(textOrWriterFunction) {
      ...
      function getEndNode(thisNode) {
        if (thisNode.getKind() === SyntaxKind.IndexSignature) {
          return thisNode.getFirstChildByKindOrThrow(
    ...
    removeReturnType() {
      ...
      const colonToken = returnTypeNode.getPreviousSiblingIfKindOrThrow(
        SyntaxKind.ColonToken,
      );
      removeChildren({
        children: [colonToken, returnTypeNode],
        removePrecedingSpaces: true,
      });
      return this;
    }
    ...
    getStructure() {
      ...
      return callBaseGetStructure(Base.prototype, this, {
        returnType: returnTypeNode
          ? returnTypeNode.getText({ trimLeadingIndentation: true })
          : undefined,
      });
    }
  };
  ...
  return class extends Base {
    ...
    getStaticKeywordOrThrow() {
      return errors.throwIfNullOrUndefined(
        ...
        "Expected to find a static keyword.",
      );
    }
    setIsStatic(value) {
...
function TextInsertableNode(Base) {
  return class extends Base {
    ...
    replaceText(range, textOrWriterFunction) {
      ...
      insertIntoParentTextRange({
        insertPos: pos,
        newText: getTextFromStringOrWriter(
          this._getWriter(),
          textOrWriterFunction,
        ),
      ...
      function verifyInRange(i) {
        ...
        throw new errors.InvalidOperationError(
          `Cannot insert or replace text outside the bounds of the node. ` +
            `Expected a position between [${validRange[0]}, ${
              validRange[1]
            }], but received ${i}.`,
        );
      }
...
function getValidRange(thisNode) {
  ...
  const openBrace = Node.isSourceFile(rangeNode)
    ? undefined
    : rangeNode.getPreviousSiblingIfKind(SyntaxKind.OpenBraceToken);
  const closeBrace = openBrace == null
    ? undefined
    : rangeNode.getNextSiblingIfKind(SyntaxKind.CloseBraceToken);
...
function TypeArgumentedNode(Base) {
  return class extends Base {
    getTypeArguments() {
      ...
      return this.compilerNode.typeArguments.map((a) =>
        this._getNodeFromCompilerNode(a)
      );
    }
    ...
    addTypeArguments(argumentTexts) {
      return this.insertTypeArguments(
        this.getTypeArguments().length,
        argumentTexts,
      );
    }
    ...
    insertTypeArguments(index, argumentTexts) {
      ...
      if (typeArguments.length === 0) {
        const identifier = this.getFirstChildByKindOrThrow(
        ...
        insertIntoCommaSeparatedNodes({
          parent: this.getFirstChildByKindOrThrow(SyntaxKind.LessThanToken)
            .getNextSiblingIfKindOrThrow(SyntaxKind.SyntaxList),
    ...
    removeTypeArgument(typeArgOrIndex) {
      ...
      const typeArgToRemove = typeof typeArgOrIndex === "number"
        ? getTypeArgFromIndex(typeArgOrIndex)
        : typeArgOrIndex;
      if (typeArguments.length === 1) {
        const childSyntaxList = typeArguments[0].getParentSyntaxListOrThrow();
        removeChildren({
          children: [
            childSyntaxList.getPreviousSiblingIfKindOrThrow(
      ...
      function getTypeArgFromIndex(index) {
        return typeArguments[
          verifyAndGetIndex(index, typeArguments.length - 1)
        ];
      }
    }
  };
  ...
  return class extends Base {
    ...
    setType(textOrWriterFunction) {
      ...
      if (separatorNode == null) {
        ...
        newText =
          (separatorSyntaxKind === SyntaxKind.EqualsToken ? " = " : ": ") +
          text;
      } else {
      ...
      function getInsertPosWhenNoType(node) {
        const identifier = node.getFirstChildByKindOrThrow(
          SyntaxKind.Identifier,
        );
        const nextSibling = identifier.getNextSibling();
        const insertAfterNode = isQuestionOrExclamation(nextSibling)
          ? nextSibling
          : identifier;
    ...
    removeType() {
      if (this.getKind() === SyntaxKind.TypeAliasDeclaration) {
        throw new errors.NotSupportedError(
          `Cannot remove the type of a type alias. Use ${
            nameof("setType")
          } instead.`,
        );
      }
      ...
      const separatorToken = typeNode.getPreviousSiblingIfKindOrThrow(
        getSeparatorSyntaxKindForNode(this),
      );
      removeChildren({
        children: [separatorToken, typeNode],
        removePrecedingSpaces: true,
      });
      return this;
    }
    getStructure() {
      ...
      return callBaseGetStructure(Base.prototype, this, {
        type: typeNode
          ? typeNode.getText({ trimLeadingIndentation: true })
          : undefined,
      });
    }
  };
  ...
  return class extends Base {
    ...
    addMembers(members) {
      return this.insertMembers(
        getEndIndexFromArray(this.getMembersWithComments()),
        members,
      );
    }
    ...
    insertMembers(index, members) {
      return insertIntoBracesOrSourceFileWithGetChildrenWithComments({
        ...
        write: (writer) => {
          ...
          const memberPrinter = this._context.structurePrinterFactory
    ...
    getConstructSignatures() {
      return this.compilerNode.members.filter((m) =>
        m.kind === SyntaxKind.ConstructSignature
      )
    ...
    addCallSignatures(structures) {
      return this.insertCallSignatures(
        getEndIndexFromArray(this.getMembersWithComments()),
        structures,
      );
    }
    ...
    getCallSignatureOrThrow(findFunction) {
      return errors.throwIfNullOrUndefined(
        ...
        "Expected to find a call signature with the provided condition.",
      );
    }
    getCallSignatures() {
      return this.compilerNode.members.filter((m) =>
        m.kind === SyntaxKind.CallSignature
      )
        .map((m) => this._getNodeFromCompilerNode(m));
    }
    ...
    addIndexSignatures(structures) {
      return this.insertIndexSignatures(
        getEndIndexFromArray(this.getMembersWithComments()),
        structures,
      );
    }
    ...
    getIndexSignatureOrThrow(findFunction) {
      return errors.throwIfNullOrUndefined(
        ...
        "Expected to find a index signature with the provided condition.",
      );
    }
    getIndexSignatures() {
      return this.compilerNode.members.filter((m) =>
    ...
    getMethods() {
      return this.compilerNode.members.filter((m) =>
        m.kind === SyntaxKind.MethodSignature
      )
        .map((m) => this._getNodeFromCompilerNode(m));
    }
    ...
    getMembers() {
      return this.compilerNode.members.map((m) =>
        this._getNodeFromCompilerNode(m)
      );
    }
    getMembersWithComments() {
      const compilerNode = this.compilerNode;
      return ExtendedParser.getContainerArray(
        compilerNode,
        this._sourceFile.compilerNode,
      )
        .map((m) => this._getNodeFromCompilerNode(m));
    ...
    getStructure() {
      return callBaseGetStructure(Base.prototype, this, {
        callSignatures: this.getCallSignatures().map((node) =>
          node.getStructure()
        ),
        constructSignatures: this.getConstructSignatures().map((node) =>
          node.getStructure()
        ),
        indexSignatures: this.getIndexSignatures().map((node) =>
          node.getStructure()
        ),
...
function TypeParameteredNode(Base) {
  return class extends Base {
    ...
    getTypeParameterOrThrow(nameOrFindFunction) {
      return errors.throwIfNullOrUndefined(
        this.getTypeParameter(nameOrFindFunction),
        () =>
          getNotFoundErrorMessageForNameOrFindFunction(
            "type parameter",
            nameOrFindFunction,
    ...
    addTypeParameters(structures) {
      return this.insertTypeParameters(
        getEndIndexFromArray(this.compilerNode.typeParameters),
        structures,
      );
    }
...
function getInsertPos(node) {
  ...
  if (namedNode.getNameNode != null) {
    ...
    Node.isCallSignatureDeclaration(node) || Node.isFunctionTypeNode(node)
  ) {
    return node.getFirstChildByKindOrThrow(SyntaxKind.OpenParenToken)
      .getStart();
  } else {
    throw new errors.NotImplementedError(
      `Not implemented scenario inserting type parameters for node with kind ${node.getKindName()}.`,
    );
  }
}
...
const createBase$D = (ctor) =>
  DotDotDotTokenableNode(InitializerExpressionableNode(BindingNamedNode(ctor)));
...
class BindingElement extends BindingElementBase {
  getPropertyNameNodeOrThrow() {
    return errors.throwIfNullOrUndefined(
      ...
      "Expected to find a property name node.",
    );
  }
  getPropertyNameNode() {
    return this._getNodeFromCompilerNodeIfExists(
  ...
  return class extends Base {
    ...
    getAbstractKeywordOrThrow() {
      return errors.throwIfNullOrUndefined(
        ...
        "Expected to find an abstract keyword.",
      );
    }
...
class ArrayLiteralExpression extends PrimaryExpression {
  ...
  addElements(textsOrWriterFunction, options) {
    return this.insertElements(
      this.compilerNode.elements.length,
  ...
  insertElements(index, textsOrWriterFunction, options = {}) {
    ...
    function insertTexts(node) {
      insertIntoCommaSeparatedNodes({
        ...
        newText: writer.toString(),
        useNewLines,
        useTrailingCommas: useNewLines &&
          node._context.manipulationSettings.getUseTrailingCommas(),
      });
  ...
  removeElement(elementOrIndex) {
    ...
    const elementToRemove = typeof elementOrIndex === "number"
      ? getElementFromIndex(elementOrIndex)
      : elementOrIndex;
...
function ExpressionableNode(Base) {
  return class extends Base {
    ...
    getExpressionOrThrow() {
      return errors.throwIfNullOrUndefined(
        this.getExpression(),
        "Expected to find an expression.",
      );
    }
    ...
    getExpressionIfKindOrThrow(kind) {
      return errors.throwIfNullOrUndefined(
        ...
        `An expression with the kind kind ${
          getSyntaxKindName(kind)
        } was expected.`,
      );
    }
  ...
  return class extends Base {
    ...
    getExpressionIfKind(kind) {
      ...
      return expression.kind === kind
        ? this._getNodeFromCompilerNode(expression)
        : undefined;
    }
    getExpressionIfKindOrThrow(kind) {
      return errors.throwIfNullOrUndefined(
        this.getExpressionIfKind(kind),
        `An expression of the kind ${getSyntaxKindName(kind)} was expected.`,
      );
    }
...
class ElementAccessExpression extends ElementAccessExpressionBase {
  ...
  getArgumentExpressionOrThrow() {
    return errors.throwIfNullOrUndefined(
      ...
      "Expected to find an argument expression.",
    );
  }
}
...
const createBase$z = (ctor) =>
  TypeArgumentedNode(ArgumentedNode(LeftHandSideExpressionedNode(ctor)));
...
class ObjectLiteralExpression extends ObjectLiteralExpressionBase {
  ...
  getProperties() {
    return this.compilerNode.properties.map((p) =>
      this._getNodeFromCompilerNode(p)
    );
  }
  getPropertiesWithComments() {
    const members = ExtendedParser.getContainerArray(
      this.compilerNode,
  ...
  insertProperties(index, structures) {
    ...
    insertIntoCommaSeparatedNodes({
      ...
      newText: writer.toString(),
      useNewLines: true,
      useTrailingCommas: this._context.manipulationSettings
        .getUseTrailingCommas(),
    });
    return getNodesToReturn(
      properties,
  ...
  addShorthandPropertyAssignments(structures) {
    return this.insertShorthandPropertyAssignments(
      this._getAddIndex(),
      structures,
    );
  }
  ...
  _insertProperty(index, structures, createStructurePrinter) {
    ...
    const writer = this._getWriterWithChildIndentation();
    const structurePrinter = new CommaNewLineSeparatedStructuresPrinter(
      createStructurePrinter(),
    );
...
class PropertyAssignment extends PropertyAssignmentBase {
  removeInitializer() {
    ...
    const colonToken = initializer.getPreviousSiblingIfKindOrThrow(
      SyntaxKind.ColonToken,
    );
    const childIndex = this.getChildIndex();
    const sourceFileText = this._sourceFile.getFullText();
    const insertPos = this.getStart();
    const newText = sourceFileText.substring(insertPos, colonToken.getPos()) +
      sourceFileText.substring(initializer.getEnd(), this.getEnd());
    const parent = this.getParentSyntaxList() || this.getParentOrThrow();
    insertIntoParentTextRange({
      insertPos,
      newText,
    ...
    return parent.getChildAtIndexIfKindOrThrow(
      childIndex,
      SyntaxKind.ShorthandPropertyAssignment,
    );
  }
  ...
  getStructure() {
    const initializer = this.getInitializerOrThrow();
    const structure = callBaseGetStructure(
      PropertyAssignmentBase.prototype,
      this,
      {
...
const createBase$x = (ctor) =>
  InitializerExpressionGetableNode(QuestionTokenableNode(NamedNode(ctor)));
...
class ShorthandPropertyAssignment extends ShorthandPropertyAssignmentBase {
  ...
  getObjectAssignmentInitializerOrThrow() {
    return errors.throwIfNullOrUndefined(
      ...
      "Expected to find an object assignment initializer.",
    );
  }
  ...
  getEqualsTokenOrThrow() {
    return errors.throwIfNullOrUndefined(
      this.getEqualsToken(),
      "Expected to find an equals token.",
    );
  }
  getEqualsToken() {
  ...
  getStructure() {
    const structure = callBaseGetStructure(
      ShorthandPropertyAssignmentBase.prototype,
      this,
      {
...
const createBase$w = (ctor) =>
  NamedNode(QuestionDotTokenableNode(LeftHandSideExpressionedNode(ctor)));
...
const SuperElementAccessExpressionBase = SuperExpressionedNode(
...
const SuperPropertyAccessExpressionBase = SuperExpressionedNode(
...
function StatementedNode(Base) {
  return class extends Base {
    getStatements() {
      ...
      const statements =
        (_a = statementsContainer === null || statementsContainer === void 0
              ? void 0
              : statementsContainer.statements) !== null && _a !== void 0
          ? _a
          : [];
      return statements.map((s) => this._getNodeFromCompilerNode(s));
    }
    getStatementsWithComments() {
      return this._getCompilerStatementsWithComments().map((s) =>
        this._getNodeFromCompilerNode(s)
      );
    }
    ...
    getStatementOrThrow(findFunction) {
      return errors.throwIfNullOrUndefined(
        this.getStatement(findFunction),
        "Expected to find a statement matching the provided condition.",
      );
    }
    getStatementByKind(kind) {
      const statement = this._getCompilerStatementsWithComments().find((s) =>
        s.kind === kind
      );
    ...
    getStatementByKindOrThrow(kind) {
      return errors.throwIfNullOrUndefined(
        ...
        `Expected to find a statement with syntax kind ${
          getSyntaxKindName(kind)
        }.`,
      );
    }
    ...
    insertStatements(index, statements) {
      ...
      const writerFunction = (writer) => {
        const statementsPrinter = this._context.structurePrinterFactory
          .forStatement({ isAmbient: isNodeAmbientOrInAmbientContext(this) });
        statementsPrinter.printTexts(writer, statements);
      };
      return getChildSyntaxList.call(this).insertChildText(
        index,
        writerFunction,
      );
    ...
    removeStatement(index) {
      index = verifyAndGetIndex(
        index,
        this._getCompilerStatementsWithComments().length - 1,
      );
      return this.removeStatements([index, index]);
    }
    removeStatements(indexRange) {
      const statements = this.getStatementsWithComments();
      errors.throwIfRangeOutOfRange(
        indexRange,
        [0, statements.length],
        "indexRange",
      );
      removeStatementedNodeChildren(
        statements.slice(indexRange[0], indexRange[1] + 1),
      );
      return this;
    }
    addClass(structure) {
    ...
    insertClasses(index, structures) {
      return this._insertChildren({
        ...
        write: (writer, info) => {
          this._standardWrite(writer, info, () => {
            this._context.structurePrinterFactory.forClassDeclaration({
    ...
    insertEnums(index, structures) {
      return this._insertChildren({
        ...
        write: (writer, info) => {
          this._standardWrite(writer, info, () => {
            this._context.structurePrinterFactory.forEnumDeclaration()
              .printTexts(writer, structures);
          });
        },
      });
    ...
    getFunctions() {
      return this.getStatements().filter(Node.isFunctionDeclaration).filter(
        (f) => f.isAmbient() || f.isImplementation(),
      );
    }
    ...
    insertInterfaces(index, structures) {
      return this._insertChildren({
        ...
        write: (writer, info) => {
          this._standardWrite(writer, info, () => {
            this._context.structurePrinterFactory.forInterfaceDeclaration()
              .printTexts(writer, structures);
          });
        },
      });
    ...
    insertModules(index, structures) {
      return this._insertChildren({
        ...
        write: (writer, info) => {
          this._standardWrite(writer, info, () => {
            this._context.structurePrinterFactory.forModuleDeclaration({
              isAmbient: isNodeAmbientOrInAmbientContext(this),
            })
              .printTexts(writer, structures);
          });
    ...
    insertTypeAliases(index, structures) {
      return this._insertChildren({
        ...
        write: (writer, info) => {
          this._standardWrite(writer, info, () => {
            this._context.structurePrinterFactory.forTypeAliasDeclaration()
              .printTexts(writer, structures);
          }, {
            previousNewLine: (previousMember) =>
              Node.isTypeAliasDeclaration(previousMember),
            nextNewLine: (nextMember) =>
              Node.isTypeAliasDeclaration(nextMember),
          });
        },
      });
    }
    ...
    getVariableStatement(nameOrFindFunction) {
      ...
      function getFindFunction() {
        if (typeof nameOrFindFunction === "string") {
          return (statement) =>
            statement.getDeclarations().some((d) =>
              nodeHasName(d, nameOrFindFunction)
            );
        }
    ...
    insertVariableStatements(index, structures) {
      return this._insertChildren({
        ...
        write: (writer, info) => {
          this._standardWrite(writer, info, () => {
            this._context.structurePrinterFactory.forVariableStatement()
              .printTexts(writer, structures);
          }, {
            previousNewLine: (previousMember) =>
              Node.isVariableStatement(previousMember),
    ...
    getVariableDeclaration(nameOrFindFunction) {
      return getNodeByNameOrFindFunction(
        this.getVariableDeclarations(),
        nameOrFindFunction,
      );
    }
    getVariableDeclarationOrThrow(nameOrFindFunction) {
      return errors.throwIfNullOrUndefined(
        this.getVariableDeclaration(nameOrFindFunction),
        () =>
          getNotFoundErrorMessageForNameOrFindFunction(
    ...
    set(structure) {
      if (
        Node.isBodyable(this) && structure.statements == null &&
        structure.hasOwnProperty(nameof(structure, "statements"))
      ) {
        this.removeBody();
    ...
    _getCompilerStatementsContainer() {
      var _a;
      if (
        Node.isSourceFile(this) || Node.isCaseClause(this) ||
        Node.isDefaultClause(this)
      ) {
        ...
        return (_a = this.getBody()) === null || _a === void 0
          ? void 0
          : _a.compilerNode;
    ...
    _standardWrite(writer, info, writeStructures, opts = {}) {
      if (
        info.previousMember != null &&
        (opts.previousNewLine == null ||
          !opts.previousNewLine(info.previousMember)) &&
      ...
      if (
        ...
        (opts.nextNewLine == null || !opts.nextNewLine(info.nextMember))
      ) {
        writer.blankLine();
      } else {
        writer.newLineIfLastNot();
...
const createBase$t = (ctor) =>
  JSDocableNode(ExpressionedNode(TextInsertableNode(StatementedNode(ctor))));
...
class CatchClause extends CatchClauseBase {
  ...
  getVariableDeclarationOrThrow() {
    return errors.throwIfNullOrUndefined(
      ...
      "Expected to find a variable declaration.",
    );
  }
}
...
class ForStatement extends ForStatementBase {
  ...
  getIncrementorOrThrow() {
    return errors.throwIfNullOrUndefined(
      this.getIncrementor(),
      "Expected to find an incrementor.",
    );
  }
}
...
class TryStatement extends TryStatementBase {
  ...
  getCatchClauseOrThrow() {
    return errors.throwIfNullOrUndefined(
      this.getCatchClause(),
      "Expected to find a catch clause.",
    );
  }
  getFinallyBlock() {
    if (
      this.compilerNode.finallyBlock == null ||
      this.compilerNode.finallyBlock.getFullWidth() === 0
    ) {
      return undefined;
    }
  ...
  getFinallyBlockOrThrow() {
    return errors.throwIfNullOrUndefined(
      ...
      "Expected to find a finally block.",
    );
  }
}
...
class ExportAssignment extends ExportAssignmentBase {
  ...
  setIsExportEquals(value) {
    ...
    if (value) {
      this.getFirstChildByKindOrThrow(SyntaxKind.DefaultKeyword)
        .replaceWithText("=");
    } else {
      this.getFirstChildByKindOrThrow(SyntaxKind.EqualsToken).replaceWithText(
        "default",
      );
    }
    return this;
...
class ExportDeclaration extends ExportDeclarationBase {
  ...
  setIsTypeOnly(value) {
    ...
    if (value) {
      insertIntoParentTextRange({
        parent: this,
        insertPos:
          ((_a = this.getNodeProperty("exportClause")) !== null && _a !== void 0
            ? _a
            : this.getFirstChildByKindOrThrow(SyntaxKind.AsteriskToken))
            .getStart(),
        newText: "type ",
      });
    } else {
      const typeKeyword = this.getFirstChildByKindOrThrow(
  ...
  getNamespaceExport() {
    ...
    return exportClause != null && Node.isNamespaceExport(exportClause)
      ? exportClause
      : undefined;
  }
  getNamespaceExportOrThrow() {
    return errors.throwIfNullOrUndefined(
      ...
      "Expected to find a namespace export.",
    );
  }
  setNamespaceExport(name) {
    ...
    if (exportClause == null) {
      const asteriskToken = this.getFirstChildByKindOrThrow(
  ...
  setModuleSpecifier(textOrSourceFile) {
    const text = typeof textOrSourceFile === "string"
      ? textOrSourceFile
      : this._sourceFile.getRelativePathAsModuleSpecifierTo(textOrSourceFile);
  ...
  getModuleSpecifier() {
    const moduleSpecifier = this._getNodeFromCompilerNodeIfExists(
  ...
  getModuleSpecifierValue() {
    ...
    return moduleSpecifier === null || moduleSpecifier === void 0
      ? void 0
  ...
  getModuleSpecifierSourceFileOrThrow() {
    return errors.throwIfNullOrUndefined(
      ...
      `A module specifier source file was expected.`,
    );
  }
  ...
  removeModuleSpecifier() {
    ...
    removeChildren({
      children: [
        this.getFirstChildByKindOrThrow(SyntaxKind.FromKeyword),
        moduleSpecifier,
      ],
      removePrecedingNewLines: true,
      removePrecedingSpaces: true,
  ...
  hasNamedExports() {
    var _a;
    return ((_a = this.compilerNode.exportClause) === null || _a === void 0
      ? void 0
      : _a.kind) === SyntaxKind.NamedExports;
  }
  ...
  insertNamedExports(index, namedExports) {
    if (
      !(namedExports instanceof Function) &&
      ArrayUtils.isNullOrEmpty(namedExports)
    ) {
      return [];
    }
    const originalNamedExports = this.getNamedExports();
    const writer = this._getWriterWithIndentation();
    const namedExportStructurePrinter = this._context.structurePrinterFactory
    ...
    if (exportClause == null) {
      ...
      insertIntoCommaSeparatedNodes({
        parent: this.getFirstChildByKindOrThrow(SyntaxKind.NamedExports)
          .getFirstChildByKindOrThrow(SyntaxKind.SyntaxList),
        currentNodes: originalNamedExports,
        insertIndex: index,
        newText: writer.toString(),
        surroundWithSpaces: this._context.getFormatCodeSettings()
          .insertSpaceAfterOpeningAndBeforeClosingNonemptyBraces,
        useTrailingCommas: false,
      });
    }
  ...
  setAssertElements(elements) {
    ...
    if (assertClause) {
      ...
      insertIntoParentTextRange({
        parent: this,
        newText: writer.toString(),
        insertPos: this.getSourceFile().getFullText()[this.getEnd() - 1] === ";"
          ? this.getEnd() - 1
          : this.getEnd(),
      });
    }
  ...
  getAssertClause() {
    return this._getNodeFromCompilerNodeIfExists(
      this.compilerNode.assertClause,
    );
  }
  set(structure) {
    ...
    if (structure.namedExports != null) {
      ...
      structure.hasOwnProperty(nameof(structure, "namedExports")) &&
      structure.moduleSpecifier == null
    ) {
  ...
  getStructure() {
    ...
    return callBaseGetStructure(ExportDeclarationBase.prototype, this, {
      ...
      moduleSpecifier: moduleSpecifier === null || moduleSpecifier === void 0
        ? void 0
        : moduleSpecifier.getLiteralText(),
      namedExports: this.getNamedExports().map((node) => node.getStructure()),
      namespaceExport:
        (_a = this.getNamespaceExport()) === null || _a === void 0
          ? void 0
          : _a.getName(),
      assertElements: assertClause
        ? assertClause.getElements().map((e) => e.getStructure())
        : undefined,
    });
  }
}
...
class ExportSpecifier extends ExportSpecifierBase {
  ...
  removeAlias() {
    ...
    removeChildren({
      children: [
        this.getFirstChildByKindOrThrow(SyntaxKind.AsKeyword),
        aliasIdentifier,
      ],
      removePrecedingSpaces: true,
      removePrecedingNewLines: true,
  ...
  getLocalTargetSymbolOrThrow() {
    return errors.throwIfNullOrUndefined(
      ...
      `The export specifier's local target symbol was expected.`,
    );
  }
  ...
  getLocalTargetDeclarations() {
    var _a, _b;
    return (_b = (_a = this.getLocalTargetSymbol()) === null || _a === void 0
            ? void 0
            : _a.getDeclarations()) !== null && _b !== void 0
      ? _b
      : [];
  }
  remove() {
...
class ExternalModuleReference extends ExternalModuleReferenceBase {
  getReferencedSourceFileOrThrow() {
    return errors.throwIfNullOrUndefined(
      ...
      "Expected to find the referenced source file.",
    );
  }
  isRelative() {
    const expression = this.getExpression();
...
class ImportClause extends ImportClauseBase {
  ...
  getDefaultImportOrThrow() {
    return errors.throwIfNullOrUndefined(
      ...
      "Expected to find a default import.",
    );
  }
  getDefaultImport() {
  ...
  getNamedBindingsOrThrow() {
    return errors.throwIfNullOrUndefined(
      ...
      "Expected to find an import declaration's named bindings.",
    );
  }
  getNamedBindings() {
  ...
  getNamespaceImportOrThrow() {
    return errors.throwIfNullOrUndefined(
      ...
      "Expected to find a namespace import.",
    );
  }
  getNamespaceImport() {
...
class ImportDeclaration extends ImportDeclarationBase {
  isTypeOnly() {
    var _a, _b;
    return (_b = (_a = this.getImportClause()) === null || _a === void 0
            ? void 0
            : _a.isTypeOnly()) !== null && _b !== void 0
      ? _b
      : false;
  }
  setIsTypeOnly(value) {
  ...
  getModuleSpecifier() {
    const moduleSpecifier = this._getNodeFromCompilerNode(
      this.compilerNode.moduleSpecifier,
    );
    if (!Node.isStringLiteral(moduleSpecifier)) {
      throw new errors.InvalidOperationError(
  ...
  isModuleSpecifierRelative() {
    return ModuleUtils.isModuleSpecifierRelative(
      this.getModuleSpecifierValue(),
    );
  }
  setDefaultImport(text) {
    ...
    const importKeyword = this.getFirstChildByKindOrThrow(
      SyntaxKind.ImportKeyword,
    );
    const importClause = this.getImportClause();
    if (importClause == null) {
  ...
  getDefaultImport() {
    ...
    return (_b = (_a = this.getImportClause()) === null || _a === void 0
            ? void 0
            : _a.getDefaultImport()) !== null && _b !== void 0
      ? _b
      : undefined;
  }
  setNamespaceImport(text) {
    ...
    insertIntoParentTextRange({
      insertPos: this.getFirstChildByKindOrThrow(SyntaxKind.ImportKeyword)
        .getEnd(),
      parent: this,
    ...
    function getChildrenToRemove() {
      ...
      if (defaultImport == null) {
        return [
          ...
          this.getLastChildByKindOrThrow(SyntaxKind.FromKeyword),
        ];
      } else {
        return [
          defaultImport.getNextSiblingIfKindOrThrow(SyntaxKind.CommaToken),
          namespaceImport,
        ];
      }
    }
  ...
  removeDefaultImport() {
    ...
    if (hasOnlyDefaultImport) {
      removeChildren({
        children: [
          importClause,
          importClause.getNextSiblingIfKindOrThrow(SyntaxKind.FromKeyword),
        ],
        removePrecedingSpaces: true,
        removePrecedingNewLines: true,
      });
  ...
  getNamespaceImport() {
    ...
    return (_b = (_a = this.getImportClause()) === null || _a === void 0
            ? void 0
            : _a.getNamespaceImport()) !== null && _b !== void 0
      ? _b
      : undefined;
  }
  ...
  insertNamedImports(index, namedImports) {
    if (
      !(namedImports instanceof Function) &&
      ArrayUtils.isNullOrEmpty(namedImports)
    ) {
      return [];
    }
    const originalNamedImports = this.getNamedImports();
    const writer = this._getWriterWithQueuedIndentation();
    const namedImportStructurePrinter = this._context.structurePrinterFactory
      .forNamedImportExportSpecifier();
    const importClause = this.getImportClause();
    ...
    if (originalNamedImports.length === 0) {
      ...
      insertIntoCommaSeparatedNodes({
        parent: importClause.getFirstChildByKindOrThrow(SyntaxKind.NamedImports)
          .getFirstChildByKindOrThrow(SyntaxKind.SyntaxList),
  ...
  getNamedImports() {
    ...
    return (_b = (_a = this.getImportClause()) === null || _a === void 0
            ? void 0
            : _a.getNamedImports()) !== null && _b !== void 0
      ? _b
      : [];
  }
  removeNamedImports() {
    ...
    if (
      namedImportsNode == null ||
      namedImportsNode.getKind() !== SyntaxKind.NamedImports
    ) {
      return this;
    }
    const defaultImport = this.getDefaultImport();
    if (defaultImport != null) {
      const commaToken = defaultImport.getNextSiblingIfKindOrThrow(
        SyntaxKind.CommaToken,
      );
      removeChildren({ children: [commaToken, namedImportsNode] });
      return this;
    }
    const fromKeyword = importClause.getNextSiblingIfKindOrThrow(
      SyntaxKind.FromKeyword,
    );
    removeChildren({
      children: [importClause, fromKeyword],
      removePrecedingSpaces: true,
    });
    return this;
  }
  getImportClauseOrThrow() {
    return errors.throwIfNullOrUndefined(
      ...
      "Expected to find an import clause.",
    );
  }
  getImportClause() {
    return this._getNodeFromCompilerNodeIfExists(
...
function setEmptyNamedImport(node) {
  ...
  const namedImportStructurePrinter = node._context.structurePrinterFactory
    .forNamedImportExportSpecifier();
  ...
  if (importClause == null) {
    insertIntoParentTextRange({
      insertPos: node.getFirstChildByKindOrThrow(SyntaxKind.ImportKeyword)
        .getEnd(),
      parent: node,
...
class ImportEqualsDeclaration extends ImportEqualsDeclarationBase {
  isTypeOnly() {
    var _a;
    return (_a = this.compilerNode.isTypeOnly) !== null && _a !== void 0
      ? _a
      : false;
  }
  setIsTypeOnly(value) {
  ...
  setExternalModuleReference(textOrSourceFile) {
    ...
    const moduleReference = this.getModuleReference();
    if (
      Node.isExternalModuleReference(moduleReference) &&
      moduleReference.getExpression() != null
    ) {
      moduleReference.getExpressionOrThrow().replaceWithText((writer) =>
        writer.quote(text)
      );
    } else {
      moduleReference.replaceWithText((writer) =>
        writer.write("require(").quote(text).write(")")
      );
    }
    return this;
  }
...
class ImportSpecifier extends ImportSpecifierBase {
  ...
  getNameNode() {
    var _a;
    return this._getNodeFromCompilerNode(
      (_a = this.compilerNode.propertyName) !== null && _a !== void 0
        ? _a
        : this.compilerNode.name,
    );
  }
  renameAlias(alias) {
    if (StringUtils.isNullOrWhitespace(alias)) {
  ...
  return class extends Base {
    getParentModuleOrThrow() {
      return errors.throwIfNullOrUndefined(
        ...
        "Expected to find the parent module declaration.",
      );
    }
    getParentModule() {
      ...
      while (
        parent.getParentOrThrow().getKind() === SyntaxKind.ModuleDeclaration
      ) {
...
class ModuleDeclaration extends ModuleDeclarationBase {
  ...
  setName(newName) {
    ...
    if (newName.indexOf(".") >= 0) {
      throw new errors.NotImplementedError(
        `Not implemented to set a namespace name to a name containing a period. ${openIssueText}`,
      );
    }
    ...
    if (moduleName instanceof Array) {
      if (moduleName.length > 1) {
        throw new errors.NotImplementedError(
          `Not implemented to set a namespace name that uses dot notation. ${openIssueText}`,
        );
      }
      if (newName !== "global") {
  ...
  rename(newName, options) {
    ...
    if (nameNodes instanceof Array) {
      if (nameNodes.length > 1) {
        throw new errors.NotSupportedError(
          `Cannot rename a namespace name that uses dot notation. Rename the individual nodes via .${
            nameof(this, "getNameNodes")
          }()`,
        );
      }
      if (newName !== "global") {
  ...
  _getInnerBody() {
    ...
    while (
      node != null && Node.isBodyable(node) &&
      node.compilerNode.statements == null
    ) {
      node = node.getBody();
    }
...
class SourceFile extends SourceFileBase {
  ...
  _clearInternals() {
    ...
    function clearTextRanges(textRanges) {
      textRanges === null || textRanges === void 0
        ? void 0
        : textRanges.forEach((r) => r._forget());
    }
  }
  ...
  getDirectoryPath() {
    return this._context.fileSystemWrapper.getStandardizedAbsolutePath(
      FileUtils.getDirPath(this.compilerNode.fileName),
    );
  }
  getFullText() {
    return this.compilerNode.text;
  ...
  copyToDirectory(dirPathOrDirectory, options) {
    const dirPath = typeof dirPathOrDirectory === "string"
      ? dirPathOrDirectory
      : dirPathOrDirectory.getPath();
  ...
  copy(filePath, options = {}) {
    ...
    if (copiedSourceFile.getDirectoryPath() !== this.getDirectoryPath()) {
      copiedSourceFile._updateReferencesForCopyInternal(
  ...
  _copyInternal(fileAbsoluteOrRelativePath, options = {}) {
    ...
    const standardizedFilePath = fileSystemWrapper.getStandardizedAbsolutePath(
      fileAbsoluteOrRelativePath,
      this.getDirectoryPath(),
    );
    ...
    function getCopiedSourceFile(currentFile) {
      try {
        return compilerFactory.createSourceFileFromText(
        ...
        if (err instanceof errors.InvalidOperationError) {
          throw new errors.InvalidOperationError(
            `Did you mean to provide the overwrite option? ` + err.message,
          );
        } else {
          throw err;
        }
      ...
      function getShouldBeInProject() {
        ...
        const destinationFile = compilerFactory
          .getSourceFileFromCacheFromFilePath(standardizedFilePath);
  ...
  _updateReferencesForCopyInternal(literalReferences) {
    for (const reference of literalReferences) {
      reference[0] = this.getChildSyntaxListOrThrow()
        .getDescendantAtStartWithWidth(
          reference[0].getStart(),
          reference[0].getWidth(),
        );
    }
  ...
  _moveInternal(fileRelativeOrAbsolutePath, options = {}) {
    const { overwrite = false } = options;
    const filePath = this._context.fileSystemWrapper
      .getStandardizedAbsolutePath(
        fileRelativeOrAbsolutePath,
        this.getDirectoryPath(),
      );
    ...
    if (overwrite) {
      const existingSourceFile = this._context.compilerFactory
  ...
  _updateReferencesForMoveInternal(sourceFileReferences, oldDirPath) {
    ...
    updateStringLiteralReferences(
      referencingLiterals.map((node) => [node, this]),
    );
  }
  async moveImmediately(filePath, options) {
    const oldFilePath = this.getFilePath();
    const newFilePath = this._context.fileSystemWrapper
      .getStandardizedAbsolutePath(filePath, this.getDirectoryPath());
  ...
  getTypeReferenceDirectives() {
    if (this._typeReferenceDirectives == null) {
      this._typeReferenceDirectives =
        (this.compilerNode.typeReferenceDirectives || [])
          .map((f) => new FileReference(f, this));
    }
  ...
  getLibReferenceDirectives() {
    if (this._libReferenceDirectives == null) {
      this._libReferenceDirectives =
        (this.compilerNode.libReferenceDirectives || [])
          .map((f) => new FileReference(f, this));
    }
  ...
  getReferencedSourceFiles() {
    const entries = this._referenceContainer
      .getLiteralsReferencingOtherSourceFilesEntries();
  ...
  getImportStringLiterals() {
    ...
    return literals.filter((l) => l.pos !== -1).map((l) =>
      this._getNodeFromCompilerNode(l)
    );
  }
  getLanguageVersion() {
  ...
  indent(positionRangeOrPos, times = 1) {
    ...
    const positionRange = typeof positionRangeOrPos === "number"
      ? [positionRangeOrPos, positionRangeOrPos]
      : positionRangeOrPos;
    errors.throwIfRangeOutOfRange(
      positionRange,
      [0, sourceFileText.length],
      "positionRange",
    );
    const startLinePos = getPreviousMatchingPos(
      sourceFileText,
      positionRange[0],
      (char) => char === CharCodes.NEWLINE,
    );
    const endLinePos = getNextMatchingPos(
      sourceFileText,
      positionRange[1],
      (char) =>
        char === CharCodes.CARRIAGE_RETURN || char === CharCodes.NEWLINE,
    );
    const correctedText = StringUtils.indent(
      sourceFileText.substring(startLinePos, endLinePos),
      times,
      {
    ...
    replaceSourceFileTextForFormatting({
      sourceFile: this,
      newText: sourceFileText.substring(0, startLinePos) + correctedText +
  ...
  emitSync(options) {
    return this._context.program.emitSync({
      targetSourceFile: this,
      ...options,
    });
  }
  getEmitOutput(options = {}) {
    return this._context.languageService.getEmitOutput(
      this,
      options.emitOnlyDtsFiles || false,
    );
  }
  formatText(settings = {}) {
    replaceSourceFileTextForFormatting({
  ...
  async refreshFromFileSystem() {
    const fileReadResult = await this._context.fileSystemWrapper
      .readFileOrNotExists(this.getFilePath(), this._context.getEncoding());
    return this._refreshFromFileSystemInternal(fileReadResult);
  }
  refreshFromFileSystemSync() {
    const fileReadResult = this._context.fileSystemWrapper
      .readFileOrNotExistsSync(this.getFilePath(), this._context.getEncoding());
    return this._refreshFromFileSystemInternal(fileReadResult);
  }
  ...
  getRelativePathAsModuleSpecifierTo(sourceFileDirOrFilePath) {
    return this.getDirectory().getRelativePathAsModuleSpecifierTo(
      sourceFileDirOrFilePath,
    );
  }
  ...
  organizeImports(formatSettings = {}, userPreferences = {}) {
    this._context.languageService.organizeImports(
      this,
      formatSettings,
      userPreferences,
    ).forEach((fileTextChanges) => fileTextChanges.applyChanges());
    return this;
  }
  ...
  fixMissingImports(formatSettings = {}, userPreferences = {}) {
    const combinedCodeFix = this._context.languageService.getCombinedCodeFix(
      this,
      "fixMissingImport",
      formatSettings,
      userPreferences,
    ...
    function removeUnnecessaryDoubleBlankLines(changes) {
      ...
      for (let i = 0; i < changes.length - 1; i++) {
        ...
        compilerObject.newText = compilerObject.newText.replace(
          /(\r?)\n\r?\n$/,
          "$1\n",
        );
      }
    ...
    function applyTextChanges(changes) {
      const groups = ArrayUtils.groupBy(
        changes,
        (change) => change.getSpan().getStart(),
      );
      let addedLength = 0;
  ...
  for (const [stringLiteral, sourceFile] of nodeReferences) {
    if (ModuleUtils.isModuleSpecifierRelative(stringLiteral.getLiteralText())) {
      stringLiteral.setLiteralValue(
        stringLiteral._sourceFile.getRelativePathAsModuleSpecifierTo(
          sourceFile,
        ),
      );
    }
...
const createBase$p = (ctor) =>
  ModuleChildableNode(
    JSDocableNode(AmbientableNode(ExportableNode(ModifierableNode(ctor)))),
  );
...
class VariableStatement extends VariableStatementBase {
  ...
  getStructure() {
    return callBaseGetStructure(VariableStatementBase.prototype, this, {
      ...
      declarationKind: this.getDeclarationKind(),
      declarations: this.getDeclarations().map((declaration) =>
...
function FunctionLikeDeclaration(Base) {
  return JSDocableNode(
    TypeParameteredNode(
      SignaturedDeclaration(StatementedNode(ModifierableNode(Base))),
    ),
  );
}

const createBase$o = (ctor) =>
  TextInsertableNode(BodiedNode(AsyncableNode(FunctionLikeDeclaration(ctor))));
...
function OverloadableNode(Base) {
  return class extends Base {
    ...
    getImplementation() {
      ...
      return getOverloadsAndImplementation(this).find((n) =>
        n.isImplementation()
      );
    }
...
function insertOverloads(opts) {
  ...
  const firstIndex = overloads.length > 0
    ? overloads[0].getChildIndex()
    : implementationNode.getChildIndex();
  ...
  const structures = opts.structures.map((structure) =>
    Object.assign(Object.assign({}, thisStructure), structure)
  );
  ...
  return getRangeWithoutCommentsFromArray(
    ...
    mainIndex,
    structures.length,
    opts.expectedSyntaxKind,
  );
}

const createBase$n = (ctor) =>
  UnwrappableNode(
    TextInsertableNode(
      OverloadableNode(
        BodyableNode(
...
const createOverloadBase$2 = (ctor) =>
  UnwrappableNode(
    TextInsertableNode(
      AsyncableNode(
        GeneratorableNode(
...
class FunctionDeclaration extends FunctionDeclarationBase {
  ...
  insertOverloads(index, structures) {
    ...
    const printer = this._context.structurePrinterFactory
  ...
  getStructure() {
    ...
    const basePrototype = isOverload && hasImplementation
      ? FunctionDeclarationOverloadBase.prototype
      : FunctionDeclarationBase.prototype;
      ...
      BodiedNode(
        AsyncableNode(
          GeneratorableNode(
            StatementedNode(
              TypeParameteredNode(
                SignaturedDeclaration(ModifierableNode(NameableNode(ctor))),
              ),
            ),
          ),
        ),
          ...
          ReadonlyableNode(
            ModifierableNode(
              DotDotDotTokenableNode(
                TypedNode(
                  InitializerExpressionableNode(BindingNamedNode(ctor)),
                ),
              ),
            ),
          ),
...
class ParameterDeclaration extends ParameterDeclarationBase {
  ...
  isOptional() {
    return this.compilerNode.questionToken != null || this.isRestParameter() ||
      this.hasInitializer();
  }
  remove() {
    removeCommaSeparatedChild(this);
...
function addParensIfNecessary(parameter) {
  ...
  function isParameterWithoutParens() {
    return Node.isArrowFunction(parent) &&
      ...
      parameter.getParentSyntaxListOrThrow().getPreviousSiblingIfKind(
          SyntaxKind.OpenParenToken,
        ) == null;
  }
  function addParens() {
                ...
                ScopedNode(
                  QuestionTokenableNode(
                    StaticableNode(
                      AsyncableNode(
                        GeneratorableNode(
                          FunctionLikeDeclaration(PropertyNamedNode(ctor)),
                        ),
                      ),
                    ),
                  ),
...
const createOverloadBase$1 = (ctor) =>
  JSDocableNode(
    ChildOrderableNode(
      TextInsertableNode(
        OverrideableNode(
              ...
              AbstractableNode(
                QuestionTokenableNode(
                  StaticableNode(
                    AsyncableNode(
                      ModifierableNode(
                        GeneratorableNode(SignaturedDeclaration(ctor)),
                      ),
                    ),
                  ),
                ),
...
class MethodDeclaration extends MethodDeclarationBase {
  ...
  getStructure() {
    ...
    const isOverload = this.isOverload();
    const basePrototype = isOverload && hasImplementation
      ? MethodDeclarationOverloadBase.prototype
      : MethodDeclarationBase.prototype;
    return callBaseGetStructure(basePrototype, this, getStructure(this));
    function getStructure(thisNode) {
      if (hasImplementation && isOverload) {
        return getOverloadSpecificStructure();
...
function ClassLikeDeclarationBase(Base) {
  return ClassLikeDeclarationBaseSpecific(
    NameableNode(
      TextInsertableNode(
        ImplementsClauseableNode(
          HeritageClauseableNode(
            AbstractableNode(
              JSDocableNode(
                TypeParameteredNode(DecoratableNode(ModifierableNode(Base))),
              ),
            ),
          ),
        ),
...
function ClassLikeDeclarationBaseSpecific(Base) {
  return class extends Base {
    setExtends(text) {
      ...
      if (extendsClause != null) {
        const childSyntaxList = extendsClause.getFirstChildByKindOrThrow(
          SyntaxKind.SyntaxList,
        );
        ...
        if (implementsClause != null) {
          ...
          insertPos = this.getFirstChildByKindOrThrow(SyntaxKind.OpenBraceToken)
            .getStart();
        }
        const isLastSpace = /\s/.test(
          this.getSourceFile().getFullText()[insertPos - 1],
        );
        ...
        insertIntoParentTextRange({
          parent: implementsClause == null
            ? this
            : implementsClause.getParentSyntaxListOrThrow(),
          insertPos,
          newText,
        });
      }
    ...
    insertMembers(index, members) {
      ...
      return insertIntoBracesOrSourceFileWithGetChildrenWithComments({
        ...
        parent: this,
        write: (writer, info) => {
          const previousMemberHasBody = !isAmbient &&
            info.previousMember != null &&
            Node.isBodyable(info.previousMember) &&
            info.previousMember.hasBody();
          const firstStructureHasBody = !isAmbient &&
            members instanceof Array && structureHasBody(members[0]);
          if (
            previousMemberHasBody ||
            info.previousMember != null && firstStructureHasBody
          ) {
            writer.blankLineIfLastNot();
          } else {
            writer.newLineIfLastNot();
          ...
          const memberWriter = this._getWriter();
          const memberPrinter = this._context.structurePrinterFactory
            .forClassMember({ isAmbient });
          memberPrinter.printTexts(memberWriter, members);
          writer.write(memberWriter.toString());
          const lastStructureHasBody = !isAmbient && members instanceof Array &&
            structureHasBody(members[members.length - 1]);
          const nextMemberHasBody = !isAmbient && info.nextMember != null &&
            Node.isBodyable(info.nextMember) && info.nextMember.hasBody();
          if (
            info.nextMember != null && lastStructureHasBody || nextMemberHasBody
          ) {
            writer.blankLineIfLastNot();
          } else {
            writer.newLineIfLastNot();
    ...
    addConstructor(structure = {}) {
      return this.insertConstructor(
        getEndIndexFromArray(this.getMembersWithComments()),
        structure,
      );
    }
    addConstructors(structures) {
      return this.insertConstructors(
        getEndIndexFromArray(this.getMembersWithComments()),
        structures,
      );
    }
    ...
    insertConstructors(index, structures) {
      ...
      return insertChildren(this, {
        ...
        write: (writer, info) => {
          if (
            !isAmbient && info.previousMember != null &&
            !Node.isCommentNode(info.previousMember)
          ) {
            writer.blankLineIfLastNot();
          } else {
          ...
          this._context.structurePrinterFactory.forConstructorDeclaration({
            isAmbient,
          }).printTexts(writer, structures);
    ...
    addStaticBlock(structure = {}) {
      return this.insertStaticBlock(
        getEndIndexFromArray(this.getMembersWithComments()),
        structure,
      );
    }
    addStaticBlocks(structures) {
      return this.insertStaticBlocks(
        getEndIndexFromArray(this.getMembersWithComments()),
        structures,
      );
    }
    ...
    insertStaticBlocks(index, structures) {
      ...
      return insertChildren(this, {
        ...
        write: (writer, info) => {
          ...
          this._context.structurePrinterFactory.forClassStaticBlockDeclaration()
            .printTexts(writer, structures);
          if (!isAmbient && info.nextMember != null) {
            writer.blankLineIfLastNot();
          } else {
    ...
    addGetAccessors(structures) {
      return this.insertGetAccessors(
        getEndIndexFromArray(this.getMembersWithComments()),
        structures,
      );
    }
    ...
    addSetAccessors(structures) {
      return this.insertSetAccessors(
        getEndIndexFromArray(this.getMembersWithComments()),
        structures,
      );
    }
    ...
    insertProperties(index, structures) {
      return insertChildren(this, {
        ...
        write: (writer, info) => {
          if (
            info.previousMember != null && Node.hasBody(info.previousMember)
          ) {
            writer.blankLineIfLastNot();
          } else {
            writer.newLineIfLastNot();
          }
          this._context.structurePrinterFactory.forPropertyDeclaration()
            .printTexts(writer, structures);
    ...
    getInstancePropertyOrThrow(nameOrFindFunction) {
      return errors.throwIfNullOrUndefined(
        this.getInstanceProperty(nameOrFindFunction),
        () =>
          getNotFoundErrorMessageForNameOrFindFunction(
    ...
    getStaticPropertyOrThrow(nameOrFindFunction) {
      return errors.throwIfNullOrUndefined(
        this.getStaticProperty(nameOrFindFunction),
        () =>
          getNotFoundErrorMessageForNameOrFindFunction(
    ...
    getInstanceMethod(nameOrFindFunction) {
      return getNodeByNameOrFindFunction(
        this.getInstanceMethods(),
        nameOrFindFunction,
      );
    }
    getInstanceMethodOrThrow(nameOrFindFunction) {
      return errors.throwIfNullOrUndefined(
        this.getInstanceMethod(nameOrFindFunction),
        () =>
          getNotFoundErrorMessageForNameOrFindFunction(
    ...
    getInstanceMethods() {
      return this.getInstanceMembers().filter((m) =>
    ...
    getStaticMethodOrThrow(nameOrFindFunction) {
      return errors.throwIfNullOrUndefined(
        this.getStaticMethod(nameOrFindFunction),
        () =>
          getNotFoundErrorMessageForNameOrFindFunction(
            "class static method",
            nameOrFindFunction,
    ...
    getStaticMethods() {
      return this.getStaticMembers().filter((m) =>
        m instanceof MethodDeclaration
      );
    }
    getInstanceMember(nameOrFindFunction) {
      return getNodeByNameOrFindFunction(
        this.getInstanceMembers(),
        nameOrFindFunction,
      );
    }
    getInstanceMemberOrThrow(nameOrFindFunction) {
      return errors.throwIfNullOrUndefined(
        this.getInstanceMember(nameOrFindFunction),
        () =>
          getNotFoundErrorMessageForNameOrFindFunction(
    ...
    getStaticMemberOrThrow(nameOrFindFunction) {
      return errors.throwIfNullOrUndefined(
        this.getStaticMember(nameOrFindFunction),
        () =>
          getNotFoundErrorMessageForNameOrFindFunction(
            "class static member",
            nameOrFindFunction,
    ...
    getMembersWithParameterProperties() {
      ...
      const implementationCtors = members.filter((c) =>
        Node.isConstructorDeclaration(c) && c.isImplementation()
      );
    ...
    getMembers() {
      return getAllMembers(this, this.compilerNode.members).filter((m) =>
        isSupportedClassMember(m)
      );
    }
    getMembersWithComments() {
      const compilerNode = this.compilerNode;
      ...
      return getAllMembers(this, members)
        .filter((m) =>
          isSupportedClassMember(m) || Node.isCommentClassElement(m)
        );
    }
    ...
    getBaseClassOrThrow() {
      return errors.throwIfNullOrUndefined(
        this.getBaseClass(),
        `Expected to find the base class of ${this.getName()}.`,
      );
    }
    getBaseClass() {
      const baseTypes = ArrayUtils.flatten(
        this.getBaseTypes().map((t) =>
          t.isIntersection() ? t.getIntersectionTypes() : [t]
        ),
      );
...
function getAllMembers(classDec, compilerMembers) {
  ...
  const members = compilerMembers.map((m) =>
    classDec._getNodeFromCompilerNode(m)
  );
...
function getImmediateDerivedClasses(classDec) {
  ...
  for (const node of nameNode.findReferencesAsNodes()) {
    const nodeParent = node.getParentIfKind(
      SyntaxKind.ExpressionWithTypeArguments,
    );
    if (nodeParent == null) {
      continue;
    }
    const heritageClause = nodeParent.getParentIfKind(
      SyntaxKind.HeritageClause,
    );
    if (
      heritageClause == null ||
      heritageClause.getToken() !== SyntaxKind.ExtendsKeyword
    ) {
      continue;
    }
    const derivedClass = heritageClause.getParentIfKind(
      SyntaxKind.ClassDeclaration,
    );
...
const createBase$j = (ctor) =>
  ModuleChildableNode(
    AmbientableNode(ExportableNode(ClassLikeDeclarationBase(ctor))),
  );
...
class ClassDeclaration extends ClassDeclarationBase {
  ...
  getStructure() {
    ...
    return callBaseGetStructure(ClassDeclarationBase.prototype, this, {
      ...
      ctors: this.getConstructors().filter((ctor) =>
        isAmbient || !ctor.isOverload()
      ).map((ctor) => ctor.getStructure()),
      methods: this.getMethods().filter((method) =>
        isAmbient || !method.isOverload()
      ...
      getAccessors: this.getGetAccessors().map((getAccessor) =>
        getAccessor.getStructure()
      ),
      setAccessors: this.getSetAccessors().map((accessor) =>
        accessor.getStructure()
      ),
    });
  }
  extractInterface(name) {
    const { constructors, properties, methods, accessors } =
      getExtractedClassDetails(this, false);
    const parameterProperties = ArrayUtils.flatten(
      constructors.map((c) =>
        c.getParameters().filter((p) => p.isParameterProperty())
      ),
    )
    ...
    return {
      ...
      properties: [
        ...parameterProperties.map((p) => {
          const jsDocComment = ArrayUtils.flatten(
            p.getParentOrThrow().getJsDocs().map((j) => j.getTags()),
          )
            .filter(Node.isJSDocParameterTag)
            .filter((t) =>
              t.getTagName() === "param" && t.getName() === p.getName() &&
              t.getComment() != null
            )
            .map((t) => t.getCommentText().trim())[0];
          return {
            kind: StructureKind.PropertySignature,
            docs: jsDocComment == null
              ? []
              : [{ kind: StructureKind.JSDoc, description: jsDocComment }],
            name: p.getName(),
  ...
  extractStaticInterface(name) {
    const { constructors, properties, methods, accessors } =
      getExtractedClassDetails(this, true);
...
function getExtractedClassDetails(classDec, isStatic) {
  const constructors = ArrayUtils.flatten(
    classDec.getConstructors().map((c) =>
      c.getOverloads().length > 0 ? c.getOverloads() : [c]
    ),
  );
  const properties = classDec.getProperties().filter((p) =>
    p.isStatic() === isStatic && p.getScope() === Scope.Public
  );
  ...
  function getAccessors() {
    ...
    for (
      const accessor of [
        ...
        ...classDec.getSetAccessors(),
      ]
    ) {
      if (
        accessor.isStatic() === isStatic && accessor.getScope() === Scope.Public
      ) {
...
function getDefaultExtractedName(name, classDec) {
  ...
  return name || classDec.getName() ||
    classDec.getSourceFile().getBaseNameWithoutExtension().replace(
      /[^a-zA-Z0-9_$]/g,
      "",
    );
}
...
function getExtractedInterfaceMethodStructure(method) {
  return {
    ...
    parameters: method.getParameters().map(
      getExtractedInterfaceParameterStructure,
    ),
...
const createBase$i = (ctor) =>
  ChildOrderableNode(
    TextInsertableNode(StatementedNode(JSDocableNode(BodiedNode(ctor)))),
  );
...
const createOverloadBase = (ctor) =>
  TypeParameteredNode(
    JSDocableNode(
      ChildOrderableNode(
        TextInsertableNode(
          ScopedNode(ModifierableNode(SignaturedDeclaration(ctor))),
        ),
      ),
    ),
  );
...
class ConstructorDeclaration extends ConstructorDeclarationBase {
  ...
  getStructure() {
    ...
    const isOverload = this.isOverload();
    const basePrototype = isOverload && hasImplementation
      ? ConstructorDeclarationOverloadBase.prototype
      : ConstructorDeclarationBase.prototype;
    return callBaseGetStructure(basePrototype, this, getStructure(this));
    function getStructure(thisNode) {
      if (hasImplementation && isOverload) {
    ...
    TextInsertableNode(
      DecoratableNode(
        AbstractableNode(
          ScopedNode(
            StaticableNode(
              FunctionLikeDeclaration(BodyableNode(PropertyNamedNode(ctor))),
            ),
          ),
        ),
      ),
...
class GetAccessorDeclaration extends GetAccessorDeclarationBase {
  ...
  getSetAccessor() {
    ...
    return this.getParentOrThrow().forEachChild((sibling) => {
      if (
        Node.isSetAccessorDeclaration(sibling) &&
        sibling.getName() === thisName && sibling.isStatic() === isStatic
      ) {
        return sibling;
      }
      return undefined;
...
class PropertyDeclaration extends PropertyDeclarationBase {
  ...
  remove() {
    ...
    switch (parent.getKind()) {
      ...
      default:
        throw new errors.NotImplementedError(
          `Not implemented parent syntax kind: ${parent.getKindName()}`,
        );
    }
  }
  getStructure() {
...
class SetAccessorDeclaration extends SetAccessorDeclarationBase {
  ...
  getGetAccessor() {
    ...
    return this.getParentOrThrow().forEachChild((sibling) => {
      if (
        Node.isGetAccessorDeclaration(sibling) &&
        sibling.getName() === thisName && sibling.isStatic() === isStatic
      ) {
        return sibling;
      }
...
class Decorator extends DecoratorBase {
      ...
      insertIntoParentTextRange({
        ...
        customMappings: (newParent) => {
          return [{
            ...
            newNode: newParent.expression.expression,
          }];
        },
      });
    } else {
  ...
  getArguments() {
    var _a, _b;
    return (_b = (_a = this.getCallExpression()) === null || _a === void 0
            ? void 0
            : _a.getArguments()) !== null && _b !== void 0
      ? _b
      : [];
  }
  getTypeArguments() {
    ...
    return (_b = (_a = this.getCallExpression()) === null || _a === void 0
            ? void 0
            : _a.getTypeArguments()) !== null && _b !== void 0
      ? _b
      : [];
  }
  addTypeArgument(argumentText) {
  ...
  insertTypeArgument(index, argumentText) {
    return this.getCallExpressionOrThrow().insertTypeArgument(
      index,
      argumentText,
    );
  }
  insertTypeArguments(index, argumentTexts) {
    return this.getCallExpressionOrThrow().insertTypeArguments(
      index,
      argumentTexts,
    );
  }
  ...
  insertArguments(index, argumentTexts) {
    ...
    return this.getCallExpressionOrThrow().insertArguments(
      index,
      argumentTexts,
    );
  }
  ...
  remove() {
    ...
    const previousDecorator = this.getPreviousSiblingIfKind(
      SyntaxKind.Decorator,
    );
    if (
      previousDecorator != null &&
      previousDecorator.getStartLinePos() === thisStartLinePos
    ) {
      removeChildren({
        children: [this],
        removePrecedingSpaces: true,
      ...
      removeChildrenWithFormattingFromCollapsibleSyntaxList({
        children: [this],
        getSiblingFormatting: (parent, sibling) =>
          sibling.getStartLinePos() === thisStartLinePos
  ...
  getStructure() {
    ...
    return callBaseGetStructure(DecoratorBase.prototype, this, {
      ...
      name: this.getName(),
      arguments: isDecoratorFactory
        ? this.getArguments().map((arg) => arg.getText())
        : undefined,
      typeArguments: isDecoratorFactory
        ? this.getTypeArguments().map((arg) => arg.getText())
        : undefined,
    });
  }
}
  ...
  return class extends Base {
    ...
    getTypeExpressionOrThrow() {
      return errors.throwIfNullOrUndefined(
        ...
        `Expected to find a JS doc type expression.`,
      );
    }
    getName() {
      return this.getNameNode().getText();
...
function JSDocTypeExpressionableTag(Base) {
  return class extends Base {
    getTypeExpression() {
      const result = this._getNodeFromCompilerNodeIfExists(
        this.compilerNode.typeExpression,
      );
    ...
    getTypeExpressionOrThrow() {
      return errors.throwIfNullOrUndefined(
        this.getTypeExpression(),
        `Expected to find the JS doc tag's type expression.`,
      );
    }
  };
}
...
function getTextWithoutStars(inputText) {
  ...
  return innerTextWithStars.split(/\n/).map((line) => {
    ...
    const substringStart = line[starPos + 1] === " "
      ? starPos + 2
      : starPos + 1;
...
class JSDoc extends JSDocBase {
  ...
  getTags() {
    var _a, _b;
    return (_b = (_a = this.compilerNode.tags) === null || _a === void 0
            ? void 0
            : _a.map((t) => this._getNodeFromCompilerNode(t))) !== null &&
        _b !== void 0
      ? _b
      : [];
  }
  ...
  getComment() {
    if (this.compilerNode.comment == null) {
      ...
      return this.compilerNode.comment.map((n) =>
        this._getNodeFromCompilerNodeIfExists(n)
      );
    }
  }
  getCommentText() {
  ...
  getDescription() {
    ...
    const endSearchStart =
      (_b = (_a = this.getTags()[0]) === null || _a === void 0
            ? void 0
            : _a.getStart()) !== null && _b !== void 0
        ? _b
        : this.getEnd() - 2;
    const start = getStart(this);
    return getTextWithoutStars(
      sourceFileText.substring(start, Math.max(start, getEndPos())),
    );
  ...
  setDescription(textOrWriterFunction) {
    ...
    function getNewText() {
      ...
      const newLineKind = this._context.manipulationSettings
        .getNewLineKindAsString();
      const rawLines = getTextFromStringOrWriter(
        this._getWriter(),
        textOrWriterFunction,
      ).split(/\r?\n/);
      const startsWithNewLine = rawLines[0].length === 0;
      const isSingleLine = rawLines.length === 1 &&
        ((_b = (_a = this.compilerNode.tags) === null || _a === void 0
                  ? void 0
                  : _a.length) !== null && _b !== void 0
            ? _b
            : 0) === 0;
      const linesText = isSingleLine
        ? rawLines[0]
        : rawLines.map((l) =>
          l.length === 0 ? `${indentationText} *` : `${indentationText} * ${l}`
        )
          .slice(startsWithNewLine ? 1 : 0)
          .join(newLineKind);
      return isSingleLine
        ? " " + linesText + " "
        : newLineKind + linesText + newLineKind + indentationText + " ";
    }
  }
  addTag(structure) {
  ...
  insertTags(index, structures) {
    ...
    if (tags.length === 0 && !this.isMultiLine()) {
      ...
      const structurePrinter = this._context.structurePrinterFactory
        .forJSDocTag({ printStarsOnNewLine: true });
    ...
    function getReplaceStart() {
      const searchStart = index < tags.length
        ? tags[index].getStart()
        : this.getEnd() - 2;
    ...
    if (structure.tags != null) {
      return this.replaceWithText((writer) => {
        ...
        this._context.structurePrinterFactory.forJSDoc().printText(writer, {
          description: (_a = structure.description) !== null && _a !== void 0
            ? _a
            : this.getDescription(),
          tags: structure.tags,
        });
...
class FunctionOrConstructorTypeNodeBase
  extends FunctionOrConstructorTypeNodeBaseBase {
}

const ConstructorTypeNodeBase = AbstractableNode(
  ModifierableNode(FunctionOrConstructorTypeNodeBase),
);
class ConstructorTypeNode extends ConstructorTypeNodeBase {
}

const ExpressionWithTypeArgumentsBase = LeftHandSideExpressionedNode(
  NodeWithTypeArguments,
);
class ExpressionWithTypeArguments extends ExpressionWithTypeArgumentsBase {
}

const FunctionTypeNodeBase = TypeParameteredNode(
...
class ImportTypeAssertionContainer extends Node {
  ...
  isMultiline() {
    var _a;
    return (_a = this.compilerNode.multiLine) !== null && _a !== void 0
      ? _a
      : false;
  }
}
  ...
  setQualifier(text) {
    ...
    if (qualifier != null) {
      ...
      insertIntoParentTextRange({
        ...
        parent: this,
        newText: this._getWriterWithQueuedIndentation().write(".").write(text)
          .toString(),
      });
    }
    return this;
...
class MappedTypeNode extends TypeNode {
  ...
  getTypeNodeOrThrow() {
    return errors.throwIfNullOrUndefined(
      this.getTypeNode(),
      "Type did not exist, but was expected to exist.",
    );
  }
}
...
const createBase$d = (ctor) =>
  TypedNode(
    QuestionTokenableNode(
      DotDotDotTokenableNode(JSDocableNode(NamedNode(ctor))),
    ),
  );
...
class TemplateLiteralTypeNode extends TypeNode {
  ...
  getTemplateSpans() {
    return this.compilerNode.templateSpans.map((s) =>
      this._getNodeFromCompilerNode(s)
    );
  }
  setLiteralValue(value) {
    var _a;
    const childIndex = this.getChildIndex();
    const parent = (_a = this.getParentSyntaxList()) !== null && _a !== void 0
      ? _a
...
class TupleTypeNode extends TypeNode {
  getElements() {
    return this.compilerNode.elements.map((t) =>
      this._getNodeFromCompilerNode(t)
    );
  }
}

const createBase$c = (ctor) =>
  TypeParameteredNode(
    TypedNode(
      JSDocableNode(
...
class TypeParameterDeclaration extends TypeParameterDeclarationBase {
  ...
  getConstraintOrThrow() {
    return errors.throwIfNullOrUndefined(
      this.getConstraint(),
      "Expected to find the type parameter's constraint.",
    );
  }
  setConstraint(text) {
  ...
  getDefaultOrThrow() {
    return errors.throwIfNullOrUndefined(
      this.getDefault(),
      "Expected to find the type parameter's default.",
    );
  }
  setDefault(text) {
    text = this.getParentOrThrow()._getTextWithQueuedChildIndentation(text);
  ...
  remove() {
    ...
    const typeParameters = parentSyntaxList.getChildrenOfKind(
      SyntaxKind.TypeParameter,
    );
    ...
    function removeAllTypeParameters() {
      const children = [
        parentSyntaxList.getPreviousSiblingIfKindOrThrow(
          SyntaxKind.LessThanToken,
        ),
        parentSyntaxList,
        parentSyntaxList.getNextSiblingIfKindOrThrow(
          SyntaxKind.GreaterThanToken,
        ),
      ];
  ...
  getStructure() {
    ...
    return callBaseGetStructure(TypeParameterDeclarationBase.prototype, this, {
      ...
      constraint: constraintNode != null
        ? constraintNode.getText({ trimLeadingIndentation: true })
        : undefined,
      default: defaultNode
        ? defaultNode.getText({ trimLeadingIndentation: true })
        : undefined,
...
function removeConstraintOrDefault(nodeToRemove, siblingKind) {
  ...
  removeChildren({
    children: [
      nodeToRemove.getPreviousSiblingIfKindOrThrow(siblingKind),
      nodeToRemove,
    ],
    removePrecedingSpaces: true,
  });
...
class TypePredicateNode extends TypeNode {
  ...
  getAssertsModifierOrThrow() {
    return errors.throwIfNullOrUndefined(
      ...
      "Expected to find an asserts modifier.",
    );
  }
  getTypeNode() {
    return this._getNodeFromCompilerNodeIfExists(this.compilerNode.type);
...
class JSDocTag extends JSDocTagBase {
  ...
  remove() {
    ...
    function getRemovalStart() {
      return Math.max(
        jsDocBodyStart,
        getPreviousNonWhiteSpacePos(this, this.getStart()),
      );
    }
  ...
  set(structure) {
    ...
    if (structure.text != null || structure.tagName != null) {
      return this.replaceWithText((writer) => {
        var _a;
        this._context.structurePrinterFactory.forJSDocTag({
          printStarsOnNewLine: true,
        }).printText(writer, {
          tagName: (_a = structure.tagName) !== null && _a !== void 0
            ? _a
            : this.getTagName(),
  ...
  replaceWithText(textOrWriterFunction) {
    const newText = getTextFromStringOrWriter(
      this._getWriterWithQueuedIndentation(),
      textOrWriterFunction,
    );
    const parent = this.getParentOrThrow();
    const childIndex = this.getChildIndex();
...
function getText(jsDocTag) {
  ...
  const startPos = Math.min(
    text.charCodeAt(nameEnd) === CharCodes.SPACE ? nameEnd + 1 : nameEnd,
    tagEnd,
  );
...
function getTagEnd(jsDocTag) {
  return getPreviousNonWhiteSpacePos(
    jsDocTag,
    getNextTagStartOrDocEnd(jsDocTag),
  );
}
function getNextTagStartOrDocEnd(jsDocTag, nextJsDocTag) {
  nextJsDocTag = nextJsDocTag !== null && nextJsDocTag !== void 0
    ? nextJsDocTag
    : getNextJsDocTag(jsDocTag);
...
class JSDocTemplateTag extends JSDocTemplateTagBase {
  ...
  getConstraintOrThrow() {
    return errors.throwIfNullOrUndefined(
      this.getConstraint(),
      "Expected to find the JS doc template tag's constraint.",
    );
  }
}
...
class JSDocTypeLiteral extends JSDocType {
  ...
  getPropertyTags() {
    return this.compilerNode.jsDocPropertyTags
      ? this.compilerNode.jsDocPropertyTags.map((t) =>
        this._getNodeFromCompilerNode(t)
      )
      : undefined;
  }
...
const createBase$a = (ctor) =>
  JSDocableNode(InitializerExpressionableNode(PropertyNamedNode(ctor)));
...
class EnumMember extends EnumMemberBase {
  ...
  setValue(value) {
    ...
    if (typeof value === "string") {
      const quoteKind = this._context.manipulationSettings.getQuoteKind();
      text = quoteKind + StringUtils.escapeForWithinString(value, quoteKind) +
        quoteKind;
    } else {
      text = value.toString();
    }
  ...
  set(structure) {
    ...
    if (structure.value != null) {
      ...
      structure.hasOwnProperty(nameof(structure, "value")) &&
      structure.initializer == null
    ) {
      this.removeInitializer();
    }
    return this;
...
class HeritageClause extends Node {
  getTypeNodes() {
    var _a, _b;
    return (_b = (_a = this.compilerNode.types) === null || _a === void 0
            ? void 0
            : _a.map((t) => this._getNodeFromCompilerNode(t))) !== null &&
        _b !== void 0
      ? _b
  ...
  removeExpression(expressionNodeOrIndex) {
    ...
    const expressionNodeToRemove = typeof expressionNodeOrIndex === "number"
      ? getExpressionFromIndex(expressionNodeOrIndex)
      : expressionNodeOrIndex;
    if (expressions.length === 1) {
      const heritageClauses = this.getParentSyntaxListOrThrow().getChildren();
      if (heritageClauses.length === 1) {
        removeChildren({
          children: [heritageClauses[0].getParentSyntaxListOrThrow()],
          removePrecedingSpaces: true,
        });
      } else {
...
const createBase$9 = (ctor) =>
  TypeParameteredNode(
    ChildOrderableNode(JSDocableNode(SignaturedDeclaration(ctor))),
  );
...
class ConstructSignatureDeclaration extends ConstructSignatureDeclarationBase {
  ...
  getStructure() {
    return callBaseGetStructure(
      ConstructSignatureDeclarationBase.prototype,
      this,
      {
        kind: StructureKind.ConstructSignature,
      },
...
const createBase$7 = (ctor) =>
  ReturnTypedNode(
    ChildOrderableNode(JSDocableNode(ReadonlyableNode(ModifierableNode(ctor)))),
  );
  ...
  getBaseDeclarations() {
    return ArrayUtils.flatten(
      this.getType().getBaseTypes().map((t) => {
        var _a, _b;
        return (_b = (_a = t.getSymbol()) === null || _a === void 0
                ? void 0
                : _a.getDeclarations()) !== null && _b !== void 0
          ? _b
          : [];
...
function JsxAttributedNode(Base) {
  return class extends Base {
    getAttributes() {
      return this.compilerNode.attributes.properties.map((p) =>
        this._getNodeFromCompilerNode(p)
      );
    }
    ...
    insertAttributes(index, structures) {
      ...
      const originalChildrenCount =
        this.compilerNode.attributes.properties.length;
      index = verifyAndGetIndex(index, originalChildrenCount);
      const insertPos = index === 0
        ? this.getTagNameNode().getEnd()
        : this.getAttributes()[index - 1].getEnd();
      const writer = this._getWriterWithQueuedChildIndentation();
      const structuresPrinter = new SpaceFormattingStructuresPrinter(
...
class JsxAttribute extends JsxAttributeBase {
  getInitializerOrThrow() {
    return errors.throwIfNullOrUndefined(
      this.getInitializer(),
      `Expected to find an initializer for the JSX attribute '${this.getName()}'`,
    );
  }
  getInitializer() {
    return this._getNodeFromCompilerNodeIfExists(this.compilerNode.initializer);
  ...
  removeInitializer() {
    ...
    removeChildren({
      children: [
        initializer.getPreviousSiblingIfKindOrThrow(SyntaxKind.EqualsToken),
        initializer,
      ],
      removePrecedingSpaces: true,
      removePrecedingNewLines: true,
  ...
  getStructure() {
    ...
    return callBaseGetStructure(JsxAttributeBase.prototype, this, {
      ...
      initializer: initializer === null || initializer === void 0
        ? void 0
        : initializer.getText(),
    });
  }
...
class JsxElement extends JsxElementBase {
  getJsxChildren() {
    return this.compilerNode.children.map((c) =>
      this._getNodeFromCompilerNode(c)
    );
  }
  getOpeningElement() {
  ...
  setBodyText(textOrWriterFunction) {
    const newText = getBodyText(
      this._getWriterWithIndentation(),
      textOrWriterFunction,
    );
...
const JsxExpressionBase = ExpressionableNode(
  DotDotDotTokenableNode(Expression),
);
  ...
  removeTag() {
    ...
    insertIntoParentTextRange({
      customMappings: (
        ...
        newNode: newParent.getChildren(newSourceFile)[index],
      }],
      parent,
      insertPos: this.getStart(),
...
function CommonIdentifierBase(Base) {
  return class extends Base {
    ...
    getDefinitionNodes() {
      return this.getDefinitions().map((d) => d.getDeclarationNode()).filter(
        (d) => d != null,
      );
    }
    getDefinitions() {
...
const IdentifierBase = CommonIdentifierBase(
...
const PrivateIdentifierBase = CommonIdentifierBase(
  ReferenceFindableNode(RenameableNode(Node)),
);
...
class VariableDeclaration extends VariableDeclarationBase {
  remove() {
    ...
    switch (parent.getKind()) {
      ...
      default:
        throw new errors.NotImplementedError(
          `Not implemented for syntax kind: ${parent.getKindName()}`,
        );
    }
    function removeFromDeclarationList(node) {
      const variableStatement = parent.getParentIfKindOrThrow(
  ...
  getVariableStatementOrThrow() {
    return errors.throwIfNullOrUndefined(
      ...
      "Expected the grandparent to be a variable statement.",
    );
  }
...
class VariableDeclarationList extends VariableDeclarationListBase {
  getDeclarations() {
    return this.compilerNode.declarations.map((d) =>
      this._getNodeFromCompilerNode(d)
    );
  }
  getDeclarationKind() {
...
class Signature {
  ...
  getTypeParameters() {
    ...
    return typeParameters.map((t) =>
      this._context.compilerFactory.getTypeParameter(t)
    );
  }
  getParameters() {
    return this.compilerSignature.parameters.map((p) =>
      this._context.compilerFactory.getSymbol(p)
    );
  }
  getReturnType() {
  ...
  getDocumentationComments() {
    const docs = this.compilerSignature.getDocumentationComment(
      this._context.typeChecker.compilerObject,
    );
    return docs.map((d) =>
      this._context.compilerFactory.getSymbolDisplayPart(d)
    );
  }
  getJsDocTags() {
  ...
  getDeclaration() {
    ...
    const compilerSignatureDeclaration = this.compilerSignature
      .getDeclaration();
    return compilerFactory.getNodeFromCompilerNode(
      compilerSignatureDeclaration,
      compilerFactory.getSourceFileForNode(compilerSignatureDeclaration),
    );
  }
}
...
class Symbol {
  ...
  getAliasedSymbolOrThrow() {
    return errors.throwIfNullOrUndefined(
      ...
      "Expected to find an aliased symbol.",
    );
  }
  ...
  getImmediatelyAliasedSymbolOrThrow() {
    return errors.throwIfNullOrUndefined(
      ...
      "Expected to find an immediately aliased symbol.",
    );
  }
  getAliasedSymbol() {
  ...
  getValueDeclaration() {
    ...
    return this._context.compilerFactory.getNodeFromCompilerNode(
      declaration,
      this._context.compilerFactory.getSourceFileForNode(declaration),
    );
  }
  getDeclarations() {
    var _a;
    return ((_a = this.compilerSymbol.declarations) !== null && _a !== void 0
      ? _a
      : [])
      .map((d) =>
        this._context.compilerFactory.getNodeFromCompilerNode(
          d,
          this._context.compilerFactory.getSourceFileForNode(d),
        )
      );
  }
  getExportOrThrow(name) {
    return errors.throwIfNullOrUndefined(
      this.getExport(name),
      `Expected to find export with name: ${name}`,
    );
  }
  getExport(name) {
    ...
    const tsSymbol = this.compilerSymbol.exports.get(
  ...
  getExports() {
    ...
    return ArrayUtils.from(this.compilerSymbol.exports.values()).map((symbol) =>
      this._context.compilerFactory.getSymbol(symbol)
    );
  }
  getGlobalExportOrThrow(name) {
    return errors.throwIfNullOrUndefined(
      this.getGlobalExport(name),
      `Expected to find global export with name: ${name}`,
    );
  }
  getGlobalExport(name) {
    ...
    const tsSymbol = this.compilerSymbol.globalExports.get(
      ts.escapeLeadingUnderscores(name),
    );
    return tsSymbol == null
      ? undefined
  ...
  getGlobalExports() {
    ...
    return ArrayUtils.from(this.compilerSymbol.globalExports.values()).map(
      (symbol) => this._context.compilerFactory.getSymbol(symbol),
    );
  }
  getMemberOrThrow(name) {
    return errors.throwIfNullOrUndefined(
      this.getMember(name),
      `Expected to find member with name: ${name}`,
    );
  }
  getMember(name) {
    ...
    const tsSymbol = this.compilerSymbol.members.get(
      ts.escapeLeadingUnderscores(name),
    );
    return tsSymbol == null
      ? undefined
  ...
  getMembers() {
    ...
    return ArrayUtils.from(this.compilerSymbol.members.values()).map((symbol) =>
      this._context.compilerFactory.getSymbol(symbol)
    );
  }
  getDeclaredType() {
  ...
  getJsDocTags() {
    return this.compilerSymbol.getJsDocTags(
      this._context.typeChecker.compilerObject,
    )
...
class FileTextChanges {
  constructor(context, compilerObject) {
    ...
    const file = context.compilerFactory
      .addOrGetSourceFileFromFilePath(
        context.fileSystemWrapper.getStandardizedAbsolutePath(
          compilerObject.fileName,
        ),
        {
          markInProject: false,
  ...
  applyChanges(options = {}) {
    ...
    let file;
    if (this.isNewFile()) {
      file = this._context.project.createSourceFile(this.getFilePath(), "", {
...
class CodeAction {
  ...
  getChanges() {
    return this.compilerObject.changes.map((change) =>
      new FileTextChanges(this._context, change)
    );
  }
}
...
class DocumentSpan {
  ...
  getNode() {
    ...
    function findBestMatchingNode() {
      let bestNode;
      sourceFile._context.compilerFactory.forgetNodesCreatedInBlock(
        (remember) => {
          let foundNode;
          ...
          while (nextNode != null) {
            ...
            if (
              nextNode.getStart() === start && nextNode.getWidth() === width
            ) {
  ...
  getOriginalTextSpan() {
    ...
    return originalTextSpan == null
      ? undefined
      : new TextSpan(originalTextSpan);
  }
  getOriginalFileName() {
...
class DefinitionInfo extends DocumentSpan {
  ...
  getDeclarationNode() {
    ...
    function findIdentifier(node) {
      if (
        node.getKind() === SyntaxKind.Identifier && node.getStart() === start
      ) {
        return node;
      }
      for (const child of node._getChildrenIterator()) {
...
class Diagnostic {
  ...
  getSourceFile() {
    ...
    return file == null
      ? undefined
      : this._context.compilerFactory.getSourceFile(file, {
        markInProject: false,
      });
  }
  getMessageText() {
...
class EmitOutput {
  ...
  getOutputFiles() {
    return this.compilerObject.outputFiles.map((f) =>
      new OutputFile(this._context, f)
    );
  }
}
__decorate(
...
class EmitResult {
  ...
  getDiagnostics() {
    return this.compilerObject.diagnostics.map((d) =>
      this._context.compilerFactory.getDiagnostic(d)
    );
  }
}
__decorate(
...
class ImplementationLocation extends DocumentSpan {
  ...
  getDisplayParts() {
    return this.compilerObject.displayParts.map((p) =>
      this._context.compilerFactory.getSymbolDisplayPart(p)
    );
  }
}
__decorate(
  ...
  ImplementationLocation.prototype,
  "getDisplayParts",
  null,
);
...
class MemoryEmitResult extends EmitResult {
  ...
  saveFiles() {
    ...
    const promises = this._files.map((f) =>
      fileSystem.writeFile(
        f.filePath,
        f.writeByteOrderMark ? "\uFEFF" + f.text : f.text,
      )
    );
  ...
  saveFilesSync() {
    ...
    for (const file of this._files) {
      fileSystem.writeFileSync(
        file.filePath,
        file.writeByteOrderMark ? "\uFEFF" + file.text : file.text,
      );
    }
  }
}
...
class RefactorEditInfo {
  ...
  getEdits() {
    return this.compilerObject.edits.map((edit) =>
      new FileTextChanges(this._context, edit)
    );
  }
  getRenameFilePath() {
...
class ReferencedSymbol {
  constructor(context, compilerObject) {
    this._context = context;
    this._compilerObject = compilerObject;
    this._references = this.compilerObject.references.map((r) =>
      context.compilerFactory.getReferencedSymbolEntry(r)
    );
  }
  get compilerObject() {
    return this._compilerObject;
...
class TypeChecker {
  ...
  getAmbientModules() {
    return this.compilerObject.getAmbientModules().map((s) =>
      this._context.compilerFactory.getSymbol(s)
    );
  }
  getApparentType(type) {
    return this._context.compilerFactory.getType(
      this.compilerObject.getApparentType(type.compilerType),
    );
  }
  ...
  getTypeAtLocation(node) {
    return this._context.compilerFactory.getType(
      this.compilerObject.getTypeAtLocation(node.compilerNode),
    );
  }
  getContextualType(expression) {
    const contextualType = this.compilerObject.getContextualType(
      expression.compilerNode,
    );
    return contextualType == null
      ? undefined
      : this._context.compilerFactory.getType(contextualType);
  }
  ...
  getDeclaredTypeOfSymbol(symbol) {
    return this._context.compilerFactory.getType(
      this.compilerObject.getDeclaredTypeOfSymbol(symbol.compilerSymbol),
    );
  }
  getSymbolAtLocation(node) {
    const compilerSymbol = this.compilerObject.getSymbolAtLocation(
      node.compilerNode,
    );
    return compilerSymbol == null
      ? undefined
      : this._context.compilerFactory.getSymbol(compilerSymbol);
  }
  getAliasedSymbol(symbol) {
    if (!symbol.hasFlags(SymbolFlags.Alias)) {
      return undefined;
    }
    const tsAliasSymbol = this.compilerObject.getAliasedSymbol(
      symbol.compilerSymbol,
    );
    return tsAliasSymbol == null
      ? undefined
      : this._context.compilerFactory.getSymbol(tsAliasSymbol);
  }
  getImmediatelyAliasedSymbol(symbol) {
    const tsAliasSymbol = this.compilerObject.getImmediateAliasedSymbol(
      symbol.compilerSymbol,
    );
    return tsAliasSymbol == null
      ? undefined
  ...
  getExportSymbolOfSymbol(symbol) {
    return this._context.compilerFactory.getSymbol(
      this.compilerObject.getExportSymbolOfSymbol(symbol.compilerSymbol),
    );
  }
  getPropertiesOfType(type) {
    return this.compilerObject.getPropertiesOfType(type.compilerType).map((p) =>
      this._context.compilerFactory.getSymbol(p)
    );
  }
  getTypeText(type, enclosingNode, typeFormatFlags) {
    ...
    return this.compilerObject.typeToString(
      type.compilerType,
      enclosingNode === null || enclosingNode === void 0
        ? void 0
  ...
  getReturnTypeOfSignature(signature) {
    return this._context.compilerFactory.getType(
      this.compilerObject.getReturnTypeOfSignature(signature.compilerSignature),
    );
  }
  getSignatureFromNode(node) {
    const signature = this.compilerObject.getSignatureFromDeclaration(
      node.compilerNode,
    );
    return signature == null
      ? undefined
      : this._context.compilerFactory.getSignature(signature);
  }
  getExportsOfModule(moduleSymbol) {
    const symbols = this.compilerObject.getExportsOfModule(
  ...
  getExportSpecifierLocalTargetSymbol(exportSpecifier) {
    const symbol = this.compilerObject.getExportSpecifierLocalTargetSymbol(
      exportSpecifier.compilerNode,
    );
    return symbol == null
      ? undefined
      : this._context.compilerFactory.getSymbol(symbol);
  ...
  getResolvedSignature(node) {
    const resolvedSignature = this.compilerObject.getResolvedSignature(
      node.compilerNode,
    );
  ...
  getResolvedSignatureOrThrow(node) {
    return errors.throwIfNullOrUndefined(
      this.getResolvedSignature(node),
      "Signature could not be resolved.",
    );
  }
  getBaseTypeOfLiteralType(type) {
    return this._context.compilerFactory.getType(
      this.compilerObject.getBaseTypeOfLiteralType(type.compilerType),
    );
  }
  ...
  _getDefaultTypeFormatFlags(enclosingNode) {
    ...
    if (
      enclosingNode != null &&
      enclosingNode.getKind() === SyntaxKind.TypeAliasDeclaration
    ) {
...
class Program {
  ...
  async emit(options = {}) {
    if (options.writeFile) {
      const message =
        `Cannot specify a ${
          nameof(options, "writeFile")
        } option when emitting asynchrously. ` +
    ...
    const emitResult = this._emit({
      writeFile: (filePath, text, writeByteOrderMark) => {
        promises
          .push(
            fileSystemWrapper.writeFile(
              fileSystemWrapper.getStandardizedAbsolutePath(filePath),
  ...
  _emit(options = {}) {
    const targetSourceFile = options.targetSourceFile != null
      ? options.targetSourceFile.compilerNode
      : undefined;
    const { emitOnlyDtsFiles, customTransformers, writeFile } = options;
    const cancellationToken = undefined;
    return this.compilerObject.emit(
      targetSourceFile,
      writeFile,
      cancellationToken,
      emitOnlyDtsFiles,
  ...
  getSyntacticDiagnostics(sourceFile) {
    const compilerDiagnostics = this.compilerObject.getSyntacticDiagnostics(
      sourceFile == null ? undefined : sourceFile.compilerNode,
    );
    return compilerDiagnostics.map((d) =>
      this._context.compilerFactory.getDiagnosticWithLocation(d)
    );
  }
  getSemanticDiagnostics(sourceFile) {
    const compilerDiagnostics = this.compilerObject.getSemanticDiagnostics(
  ...
  getDeclarationDiagnostics(sourceFile) {
    const compilerDiagnostics = this.compilerObject.getDeclarationDiagnostics(
      sourceFile === null || sourceFile === void 0
        ? void 0
        : sourceFile.compilerNode,
    );
  ...
  getConfigFileParsingDiagnostics() {
    const compilerDiagnostics = this.compilerObject
      .getConfigFileParsingDiagnostics();
    return compilerDiagnostics.map((d) =>
      this._context.compilerFactory.getDiagnostic(d)
    );
  ...
  getEmitModuleResolutionKind() {
    return getEmitModuleResolutionKind(
...
class LanguageService {
  constructor(params) {
    ...
    const { languageServiceHost, compilerHost } = createHosts({
      ...
      resolutionHost: (_a = params.resolutionHost) !== null && _a !== void 0
        ? _a
        : {},
    ...
    this._compilerObject = ts.createLanguageService(
      languageServiceHost,
      this._context.compilerFactory.documentRegistry,
    );
  ...
  _reset() {
    ...
    this._program._reset(
      Array.from(this._context.compilerFactory.getSourceFilePaths()),
      this._compilerHost,
    );
  }
  getProgram() {
  ...
  getDefinitionsAtPosition(sourceFile, pos) {
    const results = this.compilerObject.getDefinitionAtPosition(
      sourceFile.getFilePath(),
      pos,
    ) || [];
    return results.map((info) =>
      this._context.compilerFactory.getDefinitionInfo(info)
    );
  }
  ...
  getImplementationsAtPosition(sourceFile, pos) {
    const results = this.compilerObject.getImplementationAtPosition(
      sourceFile.getFilePath(),
      pos,
    ) || [];
    return results.map((location) =>
      new ImplementationLocation(this._context, location)
    );
  }
  findReferences(node) {
  ...
  findReferencesAsNodes(node) {
    ...
    function* getReferencingNodes() {
      for (const referencedSymbol of referencedSymbols) {
        const isAlias = referencedSymbol.getDefinition().getKind() ===
  ...
  findReferencesAtPosition(sourceFile, pos) {
    const results =
      this.compilerObject.findReferences(sourceFile.getFilePath(), pos) || [];
    return results.map((s) =>
      this._context.compilerFactory.getReferencedSymbol(s)
    );
  }
  ...
  getSuggestionDiagnostics(filePathOrSourceFile) {
    const filePath = this._getFilePathFromFilePathOrSourceFile(
      filePathOrSourceFile,
    );
    const suggestionDiagnostics = this.compilerObject.getSuggestionDiagnostics(
      filePath,
    );
    return suggestionDiagnostics.map((d) =>
      this._context.compilerFactory.getDiagnosticWithLocation(d)
    );
  }
  ...
  getFormattingEditsForDocument(filePath, formatSettings) {
    const standardizedFilePath = this._context.fileSystemWrapper
      .getStandardizedAbsolutePath(filePath);
    return (this.compilerObject.getFormattingEditsForDocument(
      standardizedFilePath,
      this._getFilledSettings(formatSettings),
    ) || [])
      .map((e) => new TextChange(e));
  ...
  getFormattedDocumentText(filePath, formatSettings) {
    ...
    if (
      formatSettings.ensureNewLineAtEndOfFile && !newText.endsWith(newLineChar)
    ) {
      newText += newLineChar;
    }
  ...
  getIdentationAtPosition(filePathOrSourceFile, position, settings) {
    ...
    return this.compilerObject.getIndentationAtPosition(
      filePath,
      position,
      settings,
    );
  ...
  organizeImports(
    ...
    return this.compilerObject.organizeImports(
      scope,
      this._getFilledSettings(formatSettings),
      this._getFilledUserPreferences(userPreferences),
    )
      .map((fileTextChanges) =>
        new FileTextChanges(this._context, fileTextChanges)
      );
  }
  getEditsForRefactor(
    filePathOrSourceFile,
    ...
    const position = typeof positionOrRange === "number"
      ? positionOrRange
      : { pos: positionOrRange.getPos(), end: positionOrRange.getEnd() };
    ...
    return compilerObject != null
      ? new RefactorEditInfo(this._context, compilerObject)
      : undefined;
  }
  getCombinedCodeFix(
    filePathOrSourceFile,
    ...
    const compilerResult = this.compilerObject.getCombinedCodeFix(
      {
        type: "file",
        fileName: this._getFilePathFromFilePathOrSourceFile(
          filePathOrSourceFile,
        ),
      },
      fixId,
  ...
  getCodeFixesAtPosition(
    ...
    return compilerResult.map((compilerObject) =>
      new CodeFixAction(this._context, compilerObject)
    );
  }
...
class Type {
  ...
  getText(enclosingNode, typeFormatFlags) {
    return this._context.typeChecker.getTypeText(
      this,
      enclosingNode,
      typeFormatFlags,
    );
  ...
  getAliasSymbol() {
    return this.compilerType.aliasSymbol == null
      ? undefined
  ...
  getArrayElementTypeOrThrow() {
    return errors.throwIfNullOrUndefined(
      ...
      "Expected to find an array element type.",
    );
  }
  getArrayElementType() {
    if (!this.isArray()) {
  ...
  getCallSignatures() {
    return this.compilerType.getCallSignatures().map((s) =>
      this._context.compilerFactory.getSignature(s)
    );
  }
  getConstructSignatures() {
    return this.compilerType.getConstructSignatures().map((s) =>
      this._context.compilerFactory.getSignature(s)
    );
  }
  getConstraintOrThrow() {
  ...
  getConstraint() {
    ...
    return constraint == null
      ? undefined
      : this._context.compilerFactory.getType(constraint);
  }
  getDefaultOrThrow() {
    return errors.throwIfNullOrUndefined(
      this.getDefault(),
      "Expected to find a default type.",
    );
  }
  getDefault() {
    ...
    return defaultType == null
      ? undefined
      : this._context.compilerFactory.getType(defaultType);
  }
  getProperties() {
    return this.compilerType.getProperties().map((s) =>
      this._context.compilerFactory.getSymbol(s)
    );
  }
  getPropertyOrThrow(nameOrFindFunction) {
  ...
  getProperty(nameOrFindFunction) {
    return getSymbolByNameOrFindFunction(
      this.getProperties(),
      nameOrFindFunction,
    );
  }
  getApparentProperties() {
    return this.compilerType.getApparentProperties().map((s) =>
      this._context.compilerFactory.getSymbol(s)
    );
  }
  getApparentProperty(nameOrFindFunction) {
    return getSymbolByNameOrFindFunction(
      this.getApparentProperties(),
      nameOrFindFunction,
    );
  }
  isNullable() {
  ...
  getNumberIndexType() {
    ...
    return numberIndexType == null
      ? undefined
      : this._context.compilerFactory.getType(numberIndexType);
  }
  getStringIndexType() {
    const stringIndexType = this.compilerType.getStringIndexType();
    return stringIndexType == null
      ? undefined
      : this._context.compilerFactory.getType(stringIndexType);
  }
  getTargetType() {
    const targetType = this.compilerType.target || undefined;
    return targetType == null
      ? undefined
      : this._context.compilerFactory.getType(targetType);
  }
  getTargetTypeOrThrow() {
    return errors.throwIfNullOrUndefined(
      this.getTargetType(),
      "Expected to find the target type.",
    );
  }
  getTypeArguments() {
  ...
  getUnionTypes() {
    ...
    return this.compilerType.types.map((t) =>
      this._context.compilerFactory.getType(t)
    );
  }
  ...
  getLiteralValue() {
    var _a;
    return (_a = this.compilerType) === null || _a === void 0
      ? void 0
      : _a.value;
  }
  getLiteralValueOrThrow() {
    return errors.throwIfNullOrUndefined(
      ...
      "Type was not a literal type.",
    );
  }
  getLiteralFreshType() {
    var _a;
    const type = (_a = this.compilerType) === null || _a === void 0
      ? void 0
      : _a.freshType;
    return type == null
      ? undefined
      : this._context.compilerFactory.getType(type);
  }
  ...
  isArray() {
    ...
    return (symbol.getName() === "Array" ||
      ...
      this.getTypeArguments().length === 1;
  }
  isReadonlyArray() {
    const symbol = this.getSymbol();
    if (symbol == null) {
...
class TypeParameter extends Type {
  getConstraintOrThrow() {
    return errors.throwIfNullOrUndefined(
      this.getConstraint(),
      "Expected type parameter to have a constraint.",
    );
  }
  getConstraint() {
  ...
  getDefaultOrThrow() {
    return errors.throwIfNullOrUndefined(
      this.getDefault(),
      "Expected type parameter to have a default type.",
    );
  }
  getDefault() {
    const declaration = this._getTypeParameterDeclaration();
...
class Directory {
  ...
  getParent() {
    ...
    return this.addDirectoryAtPathIfExists(
      FileUtils.getDirPath(this.getPath()),
    );
  }
  getDirectoryOrThrow(pathOrCondition) {
    return errors.throwIfNullOrUndefined(
      this.getDirectory(pathOrCondition),
      () => {
        if (typeof pathOrCondition === "string") {
          return `Could not find a directory at path '${
            this._context.fileSystemWrapper.getStandardizedAbsolutePath(
              pathOrCondition,
              this.getPath(),
            )
  ...
  getDirectory(pathOrCondition) {
    if (typeof pathOrCondition === "string") {
      const path = this._context.fileSystemWrapper.getStandardizedAbsolutePath(
        pathOrCondition,
        this.getPath(),
      );
  ...
  getSourceFileOrThrow(pathOrCondition) {
    return errors.throwIfNullOrUndefined(
      this.getSourceFile(pathOrCondition),
      () => {
        if (typeof pathOrCondition === "string") {
          const absolutePath = this._context.fileSystemWrapper
            .getStandardizedAbsolutePath(pathOrCondition, this.getPath());
  ...
  getSourceFiles(globPatterns) {
    ...
    if (typeof globPatterns === "string" || globPatterns instanceof Array) {
      const finalGlobPatterns = typeof globPatterns === "string"
        ? [globPatterns]
        : globPatterns;
    ...
    function* getFilteredSourceFiles(globPatterns) {
      ...
      const matchedPaths = matchGlobs(
        sourceFilePaths,
        globPatterns,
        dir.getPath(),
      );
      for (const matchedPath of matchedPaths) {
        yield compilerFactory.getSourceFileFromCacheFromFilePath(
          fileSystemWrapper.getStandardizedAbsolutePath(matchedPath),
        );
      }
  ...
  *_getDescendantDirectoriesIterator() {
  ...
  addSourceFilesAtPaths(fileGlobs) {
    ...
    return this._context.directoryCoordinator.addSourceFilesAtPaths(fileGlobs, {
      markInProject: this._isInProject(),
    });
  }
  addDirectoryAtPathIfExists(relativeOrAbsoluteDirPath, options = {}) {
    const dirPath = this._context.fileSystemWrapper.getStandardizedAbsolutePath(
      relativeOrAbsoluteDirPath,
      this.getPath(),
    );
  ...
  addDirectoryAtPath(relativeOrAbsoluteDirPath, options = {}) {
    ...
    return this._context.directoryCoordinator.addDirectoryAtPath(dirPath, {
      ...options,
      markInProject: this._isInProject(),
    });
  }
  ...
  createSourceFile(relativeFilePath, sourceFileText, options) {
    const filePath = this._context.fileSystemWrapper
      .getStandardizedAbsolutePath(relativeFilePath, this.getPath());
  ...
  addSourceFileAtPath(relativeFilePath) {
    const filePath = this._context.fileSystemWrapper
      .getStandardizedAbsolutePath(relativeFilePath, this.getPath());
    return this._context.directoryCoordinator.addSourceFileAtPath(filePath, {
      markInProject: this._isInProject(),
    });
  }
  async emit(options = {}) {
    ...
    for (const emitResult of this._emitInternal(options)) {
      if (isStandardizedFilePath(emitResult)) {
        ...
        writeTasks.push(
          fileSystemWrapper.writeFile(emitResult.filePath, emitResult.fileText),
        );
  ...
  _emitInternal(options = {}) {
    ...
    const isDtsFile = options.declarationDir == null && options.outDir == null
      ? undefined
      : /\.d\.ts$/i;
    ...
    const getSubDirPath = (path, dir) =>
      path == null ? undefined : FileUtils.pathJoin(path, dir.getBaseName());
    const hasDeclarationDir =
      this._context.compilerOptions.get().declarationDir != null ||
      options.declarationDir != null;
    return emitDirectory(
      this,
      getStandardizedPath(options.outDir),
      getStandardizedPath(options.declarationDir),
    );
    function* emitDirectory(directory, outDir, declarationDir) {
      for (const sourceFile of directory.getSourceFiles()) {
        ...
        for (const outputFile of output.getOutputFiles()) {
          ...
          if (
            outDir != null &&
            (isJsFile.test(filePath) || isMapFile.test(filePath) ||
              (!hasDeclarationDir && isDtsFile.test(filePath)))
          ) {
            filePath = FileUtils.pathJoin(
              outDir,
      ...
      for (const dir of directory.getDirectories()) {
        yield* emitDirectory(
          dir,
          getSubDirPath(outDir, dir),
          getSubDirPath(declarationDir, dir),
        );
      }
    }
  }
  ...
  copy(relativeOrAbsolutePath, options) {
    ...
    const fileSystem = this._context.fileSystemWrapper;
    const newPath = this._context.fileSystemWrapper.getStandardizedAbsolutePath(
  ...
  async copyImmediately(relativeOrAbsolutePath, options) {
    const fileSystem = this._context.fileSystemWrapper;
    const originalPath = this.getPath();
    const newPath = fileSystem.getStandardizedAbsolutePath(
      relativeOrAbsolutePath,
      originalPath,
    );
    if (originalPath === newPath) {
  ...
  _copyInternal(newPath, options) {
    ...
    const copyingDirectories = [this, ...this.getDescendantDirectories()].map(
      (directory) => ({
        newDirPath: directory === this
          ? newPath
          : fileSystem.getStandardizedAbsolutePath(
            this.getRelativePathTo(directory),
            newPath,
          ),
      }),
    );
    const copyingSourceFiles = this.getDescendantSourceFiles().map(
      (sourceFile) => ({
        sourceFile,
        newFilePath: fileSystem.getStandardizedAbsolutePath(
          this.getRelativePathTo(sourceFile),
          newPath,
        ),
    ...
    for (const { references, newFilePath } of copyingSourceFiles) {
      this.getSourceFileOrThrow(newFilePath)._updateReferencesForCopyInternal(
        references,
      );
    }
  ...
  _moveInternal(newPath, options, preAction) {
    ...
    const existingDir = this._context.compilerFactory
      .getDirectoryFromCacheOnlyIfInCache(newPath);
    ...
    const fileSystem = this._context.fileSystemWrapper;
    const compilerFactory = this._context.compilerFactory;
    const movingDirectories = [this, ...this.getDescendantDirectories()].map(
      (directory) => ({
        directory,
    ...
    const movingSourceFiles = this.getDescendantSourceFiles().map(
      (sourceFile) => ({
        sourceFile,
        newFilePath: fileSystem.getStandardizedAbsolutePath(
          this.getRelativePathTo(sourceFile),
  ...
  async save() {
    ...
    const unsavedSourceFiles = this.getDescendantSourceFiles().filter((s) =>
      !s.isSaved()
    );
  ...
  getRelativePathTo(sourceFileDirOrPath) {
    ...
    function getPath() {
      return sourceFileDirOrPath instanceof SourceFile
        ...
        : thisDirectory._context.fileSystemWrapper.getStandardizedAbsolutePath(
          sourceFileDirOrPath,
          thisDirectory.getPath(),
        );
    }
  }
  getRelativePathAsModuleSpecifierTo(sourceFileDirOrFilePath) {
    const moduleResolution = this._context.program
      .getEmitModuleResolutionKind();
    const thisDirectory = this;
    const moduleSpecifier = FileUtils.getRelativePathTo(
      this.getPath(),
      getPath(),
    ).replace(/((\.d\.ts$)|(\.[^/.]+$))/i, "");
      ...
      function getPathForDirectory(dir) {
        switch (moduleResolution) {
          ...
          case ModuleResolutionKind.NodeNext:
            return FileUtils.pathJoin(dir.getPath(), "index.ts");
          default:
            return errors.throwNotImplementedForNeverValueError(
              moduleResolution,
            );
        }
      }
  ...
  _getReferencesForMove(sourceFile) {
    const { literalReferences, referencingLiterals } = sourceFile
      ._getReferencesForMoveInternal();
    return {
      literalReferences: literalReferences.filter((r) =>
        !this.isAncestorOf(r[1])
      ),
      referencingLiterals: referencingLiterals.filter((l) =>
        !this.isAncestorOf(l._sourceFile)
      ),
    };
  }
...
class DirectoryCoordinator {
  ...
  addDirectoryAtPathIfExists(dirPath, options) {
    const directory = this.compilerFactory.getDirectoryFromPath(
      dirPath,
      options,
    );
    if (directory == null) {
    ...
    if (options.recursive) {
      for (
        const descendantDirPath of FileUtils.getDescendantDirectories(
          this.fileSystemWrapper,
          dirPath,
        )
      ) {
        this.compilerFactory.createDirectoryOrAddIfExists(
          descendantDirPath,
          options,
        );
      }
  ...
  addSourceFilesAtPaths(fileGlobs, options) {
    ...
    for (
      const dirPath of FileUtils.getParentMostPaths(
        Array.from(globbedDirectories),
      )
    ) {
...
class DirectoryCache {
  ...
  *getAllByDepth() {
    ...
    function addToDirLevels(dir) {
      ...
      if (depth > dirDepth) {
        throw new Error(
          `For some reason a subdirectory had a lower depth than the parent directory: ${dir.getPath()}`,
        );
      }
  ...
  *getChildDirectoriesOfDirectory(dirPath) {
    var _a;
    const entries =
      (_a = this.directoriesByDirPath.get(dirPath)) === null || _a === void 0
        ? void 0
        : _a.entries();
    if (entries == null) {
      return;
  ...
  *getChildSourceFilesOfDirectory(dirPath) {
    var _a;
    const entries =
      (_a = this.sourceFilesByDirPath.get(dirPath)) === null || _a === void 0
        ? void 0
        : _a.entries();
    if (entries == null) {
      return;
  ...
  addSourceFile(sourceFile) {
    ...
    const sourceFiles = this.sourceFilesByDirPath.getOrCreate(
      dirPath,
      () =>
        new SortedKeyValueArray(
  ...
  addToDirectoriesByDirPath(directory) {
    ...
    const directories = this.directoriesByDirPath.getOrCreate(
      parentDirPath,
      () =>
        new SortedKeyValueArray(
          (item) => item.getBaseName(),
...
class CompilerFactory {
  constructor(context) {
    ...
    this.context.compilerOptions.onModified(() => {
      const currentSourceFiles = Array.from(
        this.sourceFileCacheByFilePath.values(),
      );
  ...
  *getSourceFilesByDirectoryDepth() {
  ...
  createSourceFile(filePath, sourceFileText, options) {
    sourceFileText = sourceFileText instanceof Function
      ? getTextFromStringOrWriter(this.context.createWriter(), sourceFileText)
      : sourceFileText || "";
  ...
  createSourceFileFromText(filePath, sourceText, options) {
    filePath = this.context.fileSystemWrapper.getStandardizedAbsolutePath(
      filePath,
    );
    if (options.overwrite === true) {
      return this.createOrOverwriteSourceFileFromText(
        filePath,
        sourceText,
        options,
      );
  ...
  throwIfFileExists(filePath, prefixMessage) {
    if (
      !this.containsSourceFileAtPath(filePath) &&
      !this.context.fileSystemWrapper.fileExistsSync(filePath)
    ) {
      return;
    }
    prefixMessage = prefixMessage == null ? "" : prefixMessage + " ";
    throw new errors.InvalidOperationError(
      `${prefixMessage}A source file already exists at the provided file path: ${filePath}`,
    );
  }
  createOrOverwriteSourceFileFromText(filePath, sourceText, options) {
    ...
    const existingSourceFile = this.addOrGetSourceFileFromFilePath(
      filePath,
      options,
    );
    if (existingSourceFile != null) {
      ...
      this.replaceCompilerNode(
        existingSourceFile,
        this.createCompilerSourceFileFromText(
          filePath,
          sourceText,
          options.scriptKind,
        ),
  ...
  addOrGetSourceFileFromFilePath(filePath, options) {
    ...
    if (sourceFile == null) {
      const fileText = this.context.fileSystemWrapper.readFileIfExistsSync(
        filePath,
        this.context.getEncoding(),
      );
      if (fileText != null) {
  ...
  containsDirectoryAtPath(dirPath) {
    dirPath = this.context.fileSystemWrapper.getStandardizedAbsolutePath(
      dirPath,
    );
  ...
  getNodeFromCompilerNode(compilerNode, sourceFile) {
    ...
    function createNode() {
      if (isCommentNode(compilerNode)) {
        ...
        if (CommentNodeParser.isCommentClassElement(compilerNode)) {
          return new CommentClassElement(
            this.context,
            compilerNode,
            sourceFile,
          );
        ...
        if (CommentNodeParser.isCommentObjectLiteralElement(compilerNode)) {
          return new CommentObjectLiteralElement(
            this.context,
            compilerNode,
            sourceFile,
          );
    ...
    function initializeNode(node) {
      if (compilerNode.parent != null) {
        const parentNode = this.getNodeFromCompilerNode(
          compilerNode.parent,
          sourceFile,
        );
  ...
  createSourceFileFromTextInternal(filePath, text, options) {
    ...
    const sourceFile = this.getSourceFile(
      this.createCompilerSourceFileFromText(filePath, text, options.scriptKind),
      options,
    );
    if (hasBom) {
  ...
  createCompilerSourceFileFromText(filePath, text, scriptKind) {
    return this.documentRegistry.createOrUpdateSourceFile(
      filePath,
      this.context.compilerOptions.get(),
  ...
  getSourceFile(compilerSourceFile, options) {
    ...
    let wasAdded = false;
    const sourceFile =
      (_a = this.sourceFileCacheByFilePath.get(compilerSourceFile.fileName)) !==
          null && _a !== void 0
        ? _a
        : this.nodeCache.getOrCreate(compilerSourceFile, () => {
          const createdSourceFile = new SourceFile(
            this.context,
            compilerSourceFile,
          );
          if (!options.markInProject) {
            this.context.inProjectCoordinator.setSourceFileNotInProject(
              createdSourceFile,
            );
          }
  ...
  getDirectoryFromPath(dirPath, options) {
    ...
    if (
      directory == null &&
      this.context.fileSystemWrapper.directoryExistsSync(dirPath)
    ) {
  ...
  getSymbolDisplayPart(compilerObject) {
    return this.symbolDisplayPartCache.getOrCreate(
      compilerObject,
      () => new SymbolDisplayPart(compilerObject),
    );
  }
  getType(type) {
  ...
  getTypeParameter(typeParameter) {
    return this.typeParameterCache.getOrCreate(
      typeParameter,
      () => new TypeParameter(this.context, typeParameter),
    );
  }
  getSignature(signature) {
    return this.signatureCache.getOrCreate(
      signature,
      () => new Signature(this.context, signature),
    );
  }
  getSymbol(symbol) {
    return this.symbolCache.getOrCreate(
      symbol,
      () => new Symbol(this.context, symbol),
    );
  }
  getDefinitionInfo(compilerObject) {
    return this.definitionInfoCache.getOrCreate(
      compilerObject,
      () => new DefinitionInfo(this.context, compilerObject),
    );
  }
  getDocumentSpan(compilerObject) {
    return this.documentSpanCache.getOrCreate(
      compilerObject,
      () => new DocumentSpan(this.context, compilerObject),
    );
  }
  getReferencedSymbolEntry(compilerObject) {
    return this.referencedSymbolEntryCache.getOrCreate(
      compilerObject,
      () => new ReferencedSymbolEntry(this.context, compilerObject),
    );
  }
  getReferencedSymbol(compilerObject) {
    return this.referencedSymbolCache.getOrCreate(
      compilerObject,
      () => new ReferencedSymbol(this.context, compilerObject),
    );
  }
  getReferencedSymbolDefinitionInfo(compilerObject) {
    return this.referencedSymbolDefinitionInfoCache.getOrCreate(
      compilerObject,
  ...
  getDiagnosticWithLocation(diagnostic) {
    return this.diagnosticCache.getOrCreate(
      diagnostic,
      () => new DiagnosticWithLocation(this.context, diagnostic),
    );
  }
  getDiagnosticMessageChain(compilerObject) {
    return this.diagnosticMessageChainCache.getOrCreate(
      compilerObject,
      () => new DiagnosticMessageChain(compilerObject),
    );
  }
  getJSDocTagInfo(jsDocTagInfo) {
    return this.jsDocTagInfoCache.getOrCreate(
      jsDocTagInfo,
      () => new JSDocTagInfo(jsDocTagInfo),
    );
  }
  replaceCompilerNode(oldNode, newNode) {
    const nodeToReplace = oldNode instanceof Node
      ? oldNode.compilerNode
      : oldNode;
    const node = oldNode instanceof Node
      ? oldNode
      : this.nodeCache.get(oldNode);
    if (
      nodeToReplace.kind === SyntaxKind.SourceFile &&
      nodeToReplace.fileName !== newNode.fileName
    ) {
  ...
  removeCompilerNodeFromCache(compilerNode) {
    ...
    if (compilerNode.kind === SyntaxKind.SourceFile) {
      ...
      const standardizedFilePath = this.context.fileSystemWrapper
        .getStandardizedAbsolutePath(sourceFile.fileName);
      this.directoryCache.removeSourceFile(standardizedFilePath);
      const wrappedSourceFile = this.sourceFileCacheByFilePath.get(
        standardizedFilePath,
      );
...
class InProjectCoordinator {
  ...
  markSourceFilesAsInProjectForResolution() {
    ...
    function shouldMarkInProject(sourceFile) {
      ...
      const nodeModulesPath = filePath.substring(
        0,
        index + nodeModulesSearchName.length - 1,
      );
      const nodeModulesDir = compilerFactory.getDirectoryFromCacheOnlyIfInCache(
        nodeModulesPath,
      );
      ...
      while (directory != null && directory.getPath() !== nodeModulesPath) {
        ...
        directory = compilerFactory.getDirectoryFromCacheOnlyIfInCache(
          FileUtils.getDirPath(directory.getPath()),
        );
      }
      return false;
    }
  ...
  markDirectoryAsInProject(directory) {
    ...
    function markAncestorDirs(dir) {
      ...
      if (
        topAncestor == null ||
        !inProjectCoordinator.isDirectoryInProject(topAncestor)
      ) {
        return;
      }
    ...
    function* getAncestorsUpToOneInProject(dir) {
      ...
      const parentDir = compilerFactory.getDirectoryFromCacheOnlyIfInCache(
        parentDirPath,
      );
      if (parentDir == null) {
        return;
...
__decorate(
  ...
  StructurePrinterFactory.prototype,
  "forInitializerExpressionableNode",
  null,
);
__decorate(
...
class ProjectContext {
  constructor(params) {
    ...
    this.structurePrinterFactory = new StructurePrinterFactory(() =>
      this.manipulationSettings.getFormatCodeSettings()
    );
    this.lazyReferenceCoordinator = new LazyReferenceCoordinator(
      this.compilerFactory,
    );
    this.directoryCoordinator = new DirectoryCoordinator(
      this.compilerFactory,
      params.fileSystemWrapper,
    );
    this._languageService = params.createLanguageService
      ? new LanguageService({
        ...
        resolutionHost: params.resolutionHost &&
          params.resolutionHost(this.getModuleResolutionHost(), () =>
  ...
  createWriter() {
    ...
    return new CodeBlockWriter({
      ...
      indentNumberOfSpaces: indentationText === IndentationText.Tab
        ? undefined
        : indentationText.length,
      useTabs: indentationText === IndentationText.Tab,
      useSingleQuote:
        this.manipulationSettings.getQuoteKind() === QuoteKind.Single,
    });
  }
  getPreEmitDiagnostics(sourceFile) {
    const compilerDiagnostics = ts.getPreEmitDiagnostics(
      this.program.compilerObject,
      sourceFile === null || sourceFile === void 0
        ? void 0
        : sourceFile.compilerNode,
    );
    return compilerDiagnostics.map((d) =>
      this.compilerFactory.getDiagnostic(d)
    );
  }
    ...
    return {
      addOrGetSourceFileFromFilePath: (filePath, opts) => {
        ...
        return Promise.resolve(
          (_a = this.compilerFactory.addOrGetSourceFileFromFilePath(
                filePath,
                opts,
              )) === null || _a === void 0
            ? void 0
      ...
      addOrGetSourceFileFromFilePathSync: (filePath, opts) => {
        var _a;
        return (_a = this.compilerFactory.addOrGetSourceFileFromFilePath(
                filePath,
                opts,
              )) === null || _a === void 0
          ? void 0
      ...
      containsDirectoryAtPath: (dirPath) =>
        this.compilerFactory.containsDirectoryAtPath(dirPath),
      containsSourceFileAtPath: (filePath) =>
        this.compilerFactory.containsSourceFileAtPath(filePath),
      getSourceFileFromCacheFromFilePath: (filePath) => {
        const sourceFile = this.compilerFactory
          .getSourceFileFromCacheFromFilePath(filePath);
        return sourceFile === null || sourceFile === void 0
          ? void 0
          : sourceFile.compilerNode;
      },
      getSourceFilePaths: () => this.compilerFactory.getSourceFilePaths(),
      getSourceFileVersion: (sourceFile) =>
        this.compilerFactory.documentRegistry.getSourceFileVersion(sourceFile),
      getChildDirectoriesOfDirectory: (dirPath) => {
        const result = [];
        for (
          const dir of this.compilerFactory.getChildDirectoriesOfDirectory(
            dirPath,
          )
        ) {
...
class Project {
  constructor(options = {}) {
    ...
    this._context = new ProjectContext({
      ...
      configFileParsingDiagnostics:
        (_a = tsConfigResolver === null || tsConfigResolver === void 0
              ? void 0
              : tsConfigResolver.getErrors()) !== null && _a !== void 0
          ? _a
          : [],
      skipLoadingLibFiles: options.skipLoadingLibFiles,
      libFolderPath: options.libFolderPath,
    ...
    if (
      ...
      this._addSourceFilesForTsConfigResolver(
        tsConfigResolver,
        compilerOptions,
      );
    ...
    function getFileSystem() {
      ...
      return (_a = options.fileSystem) !== null && _a !== void 0
        ? _a
        : new RealFileSystemHost();
    }
    ...
    function getTsConfigCompilerOptions() {
      var _a;
      return (_a = tsConfigResolver === null || tsConfigResolver === void 0
              ? void 0
              : tsConfigResolver.getCompilerOptions()) !== null && _a !== void 0
        ? _a
        : {};
    }
    function getEncoding() {
      ...
      if (options.compilerOptions != null) {
        return (_a = options.compilerOptions.charset) !== null && _a !== void 0
          ? _a
          : defaultEncoding;
      }
  ...
  resolveSourceFileDependencies() {
    ...
    const result = inProjectCoordinator
      .markSourceFilesAsInProjectForResolution();
  ...
  getDirectoryOrThrow(dirPath) {
    return errors.throwIfNullOrUndefined(
      ...
      () =>
        `Could not find a directory at the specified path: ${
          this._context.fileSystemWrapper.getStandardizedAbsolutePath(dirPath)
        }`,
    );
  }
  getDirectory(dirPath) {
    const { compilerFactory } = this._context;
    return compilerFactory.getDirectoryFromCache(
      this._context.fileSystemWrapper.getStandardizedAbsolutePath(dirPath),
    );
  }
  getDirectories() {
  ...
  _addSourceFilesForTsConfigResolver(tsConfigResolver, compilerOptions) {
    ...
    const addedSourceFiles = paths.filePaths.map((p) =>
      this.addSourceFileAtPath(p)
    );
  ...
  createSourceFile(filePath, sourceFileText, options) {
    return this._context.compilerFactory.createSourceFile(
      this._context.fileSystemWrapper.getStandardizedAbsolutePath(filePath),
      sourceFileText !== null && sourceFileText !== void 0
        ? sourceFileText
        : "",
      {
  ...
  getSourceFileOrThrow(fileNameOrSearchFunction) {
    ...
    if (typeof fileNameOrSearchFunction === "string") {
      ...
      if (
        ...
        const errorFileNameOrPath = this._context.fileSystemWrapper
  ...
  getSourceFile(fileNameOrSearchFunction) {
    ...
    return IterableUtils.find(
      this._getProjectSourceFilesByDirectoryDepth(),
      filePathOrSearchFunction,
    );
    function getFilePathOrSearchFunction(fileSystemWrapper) {
      ...
      if (
        ...
        return (def) =>
          FileUtils.pathEndsWith(def.getFilePath(), fileNameOrPath);
      }
    }
  ...
  getSourceFiles(globPatterns) {
    ...
    function* getFilteredSourceFiles() {
      ...
      const matchedPaths = matchGlobs(
        sourceFilePaths,
        globPatterns,
        fileSystemWrapper.getCurrentDirectory(),
      );
      for (const matchedPath of matchedPaths) {
        yield compilerFactory.getSourceFileFromCacheFromFilePath(
          fileSystemWrapper.getStandardizedAbsolutePath(matchedPath),
  ...
  *_getProjectDirectoriesByDirectoryDepth() {
  ...
  getAmbientModuleOrThrow(moduleName) {
    return errors.throwIfNullOrUndefined(
      ...
      () =>
        `Could not find ambient module with name: ${
          normalizeAmbientModuleName(moduleName)
        }`,
    );
  }
  getAmbientModules() {
  ...
  formatDiagnosticsWithColorAndContext(diagnostics, opts = {}) {
    return ts.formatDiagnosticsWithColorAndContext(
      diagnostics.map((d) => d.compilerObject),
      {
        ...
        getNewLine: () => {
          var _a;
          return (_a = opts.newLineChar) !== null && _a !== void 0
            ? _a
...
function createWrappedNode(node, opts = {}) {
  ...
  const wrappedSourceFile = projectContext.compilerFactory.getSourceFile(
    getSourceFileNode(),
    { markInProject: true },
  );
  return projectContext.compilerFactory.getNodeFromCompilerNode(
    node,
    wrappedSourceFile,
  );
...
class Writers {
  ...
  static objectType(structure) {
    return (writer) => {
      ...
      if (anyPropertyHasValue(structure)) {
        writer.indent(() => {
          structurePrinterFactory.forTypeElementMemberedNode().printText(
            writer,
            structure,
          );
        });
  ...
  static unionType(firstType, secondType, ...additionalTypes) {
    return getWriteFunctionForUnionOrIntersectionType("|", [
      firstType,
      secondType,
      ...additionalTypes,
    ]);
...
export {
  ...
  ArrayDestructuringAssignmentBase,
  ...
  AssertEntry,
  AssertEntryBase,
  AssertionKeyNamedNode,
  AssignmentExpression,
  AssignmentExpressionBase,
  AsyncableNode,
  AwaitableNode,
  AwaitExpression,
  AwaitExpressionBase,
  ...
  ClassStaticBlockDeclarationBase,
  CodeAction,
  CodeFixAction,
  CombinedCodeActions,
  CommaListExpression,
  ...
  ConstructorTypeNode,
  ConstructorTypeNodeBase,
  ConstructSignatureDeclaration,
  ConstructSignatureDeclarationBase,
  ContinueStatement,
  createWrappedNode,
  DebuggerStatement,
  ...
  ExpressionWithTypeArgumentsBase,
  ExtendsClauseableNode,
  ...
  FunctionDeclarationOverloadBase,
  FunctionExpression,
  FunctionExpressionBase,
  FunctionLikeDeclaration,
  FunctionOrConstructorTypeNodeBase,
  FunctionOrConstructorTypeNodeBaseBase,
  FunctionTypeNode,
  FunctionTypeNodeBase,
  GeneratorableNode,
  GetAccessorDeclaration,
  GetAccessorDeclarationBase,
  ...
  JSDocOverrideTag,
  JSDocParameterTag,
  JSDocParameterTagBase,
  JSDocPrivateTag,
  JSDocPropertyLikeTag,
  JSDocPropertyTag,
  JSDocPropertyTagBase,
  JSDocProtectedTag,
  JSDocPublicTag,
  JSDocReadonlyTag,
  JSDocReturnTag,
  ...
  JsxFragment,
  JsxOpeningElement,
  JsxOpeningElementBase,
  JsxOpeningFragment,
  JsxSelfClosingElement,
  JsxSelfClosingElementBase,
  JsxSpreadAttribute,
  JsxSpreadAttributeBase,
  JsxTagNamedNode,
  JsxText,
  JsxTextBase,
  LabeledStatement,
  ...
  NonNullExpression,
  NonNullExpressionBase,
  NoSubstitutionTemplateLiteral,
  NoSubstitutionTemplateLiteralBase,
  NotEmittedStatement,
  NotEmittedStatementBase,
  NotImplementedError,
  NotSupportedError,
  NullLiteral,
  NullLiteralBase,
  ...
  OmittedExpression,
  OmittedExpressionBase,
  OutputFile,
  OverloadableNode,
  OverrideableNode,
  ParameterDeclaration,
  ParameterDeclarationBase,
  ParameteredNode,
  ...
  PostfixUnaryExpressionBase,
  PrefixUnaryExpression,
  PrefixUnaryExpressionBase,
  PrimaryExpression,
  printNode,
  PrivateIdentifier,
  PrivateIdentifierBase,
  Program,
  Project,
  PropertyAccessExpression,
  PropertyAccessExpressionBase,
  PropertyAssignment,
  PropertyAssignmentBase,
  PropertyDeclaration,
  PropertyDeclarationBase,
  PropertyNamedNode,
  PropertySignature,
  PropertySignatureBase,
  QualifiedName,
  ...
  SuperElementAccessExpressionBase,
  SuperExpression,
  SuperExpressionBase,
  ...
  VariableDeclarationListBase,
  VariableStatement,
  VariableStatementBase,
  VoidExpression,
  VoidExpressionBase,
  WhileStatement,
  WhileStatementBase,
