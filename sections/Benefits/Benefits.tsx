...
export default function Benefits(
  ...
  const listOfBenefits = benefits.map((benefit, index) => {
    ...
    return (
        <div class={`${reverse ? "bg-primary text-primary-content p-4 lg:p-8" : ""} flex gap-4 ${benefitLayout == "piledup" ? "flex-col items-center text-center" : ""} ${showDivider && benefitLayout !== "piledup" ? "border-b border-neutral-300" : ""} ${showDivider ? "pb-4 lg:pr-8 lg:border-r lg:border-b-0" : ""} ${showDivider && !reverse ? "lg:pb-0" : "lg:py-4"}`}>
            <div class="flex-none">
            <Icon
                id={benefit.icon}
            ...
            <h3 class={`text-base lg:text-xl leading-7 ${reverse ? "text-base-100" : "text-base-content"}`}>
                {benefit.label}
            </h3>
