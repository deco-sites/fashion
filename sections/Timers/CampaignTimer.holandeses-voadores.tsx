export interface Props {
  ...
  export default function CampaignTimer({width, title, labelLink, url, timer, labelButton }: Props) {
    const containerClass = width === "full" ? "w-full" : "w-1/2";

    return (
      <div className={`flex flex-row items-center justify-between flex-wrap gap-4 px-12 py-6 ${containerClass} bg-red-400`}>
        <div className="flex flex-row gap-1 text-gray-100 text-xl">
            {title} <a href={url} className="underline text-gray-100 text-xl">{labelLink}</a>
        </div>
