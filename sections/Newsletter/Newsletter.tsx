...
export default function Newsletter(
  { title, description, form, layout }: Props,
) {
  const headerLayout = (
    <Header
      title={title}
      description={description}
      alignment={!layout?.content?.alignment ||
  ...
  const formLayout = (
    <form action="/" class="flex flex-col gap-4">
      <div class="flex flex-col lg:flex-row gap-3">
        <input
          class="input input-bordered lg:w-[320px]"
          type="text"
          placeholder={form.placeholder}
        />
        <button
          class={`btn ${
            layout?.content?.bgColor === "Reverse" ? "btn-accent" : ""
          }`}
          type="submit"
        >
          {form.buttonText}
      ...
      <p class="text-sm" dangerouslySetInnerHTML={{ __html: form.helpText }}>
      </p>
    </form>
  );

  const bgLayout = layout?.content?.bgColor === "Reverse"
  ...
  const borderLayout = `
        ...
        ${
    layout?.content?.border && layout?.content?.bgColor !== "Reverse"
      ? "border-secondary"
      : ""
  }
        ${
  ...
  return (
    <>
      {(!layout?.content?.alignment ||
        layout?.content?.alignment === "Center") && (
        <div
          class={`flex flex-col p-4 gap-6 lg:p-16 lg:gap-12 ${bgLayout} ${borderLayout}`}
        >
          {headerLayout}
      ...
      {layout?.content?.alignment === "Side to side" && (
        <div
          class={`flex flex-col lg:flex-row p-4 gap-6 lg:p-16 lg:gap-12 ${bgLayout} ${borderLayout}`}
        >
          <div class="lg:w-1/2">
            {headerLayout}
          </div>
